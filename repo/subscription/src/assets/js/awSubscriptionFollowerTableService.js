 ...
 */
export let loadFollowerContext = function( columnDefs, subscriptionObject ) {
    _followerId = 1;
 ...
 */
export let applyFollowerChange = function( dataProvider, followerObjects, columnDefs, selectedFollowers ) {
    _.forEach( selectedFollowers.handler_parameters, function( val ) {
        ...
        var found = _.some( dataProvider.viewModelCollection.loadedVMObjects, function( object ) {
            return object.props.handler_parameters.displayValues[ 0 ] === val.handler_parameters.displayValues[ 0 ];
        } );
        if( !found ) {
            followerObjects.push( _createOrUpdateFollowerObject( columnDefs, val ) );
        }
    } );
    dataProvider.update( followerObjects, followerObjects.length );
};

/**
 * Remove selected object from follower object list.
 */
export let removeFollower = function( dataProvider, followerObjects, selectedObj ) {
 ...
 * @return {Array} An array of Follower VMOs.
 */
export let resetFollowerTable = function( dataProvider, followerCtx, subscriptionObject ) {
    // Remove selection
