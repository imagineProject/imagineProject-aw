...
export let setValueForPanel = function( fields, provider ) {
    ...
    fields.propLogicalOperator.setLovVal( { logicOpLovEntry } );
    ...
    return {
        logicOpLovEntry: logicOpLovEntry,
        mathOpLovEntry: mathOpLovEntry,
        attrNameLovEntry: attrNameLovEntry
    };
};
 ...
 */
export let loadCriteriaContext = function( valueTypeDefs, columnDefs, subscriptionObject, selectPropertyStr ) {
    _criteriaId = 1;
        ...
        if( val !== undefined && val !== null && val !== '' ) {
            _criteriaObjectList.push( _createOrUpdateCriteriaObject( criteriaCtx.columnDefs, _.reduce( criteriaCtx.columnDefs, function( props, columnInfo ) {
                ...
                if( columnInfo.name === 'attribute_values' && dbVal ) {
                    ...
                    if ( dbVal && dbVal.includes( CST_PREFIX ) ) {
                        let classificationDisplay = appCtx.getCtx( 'cls_display' );
                        if ( classificationDisplay ) {
                            dispVal = [ classificationDisplay.get( dbVal ) ];
                        }
                    } else {
 ...
 * @return {Object} Obj for Edit Panel.
 */
export let loadCriteriaEditAttributeValue = function( prop, value, fields, data ) {
    // For avoid any side effect, keep legacy GWT logic
    // check - value.length is added if user has not added anything in the attribute value from the criteria panel
    var vals = value && value.length > 0 ? value.replace( /\\\|/g, '\\*' ).split( '|' ).map( function( v ) {
        return v.replace( /\\\*/g, '|' );
    } ) : [];
    ...
    if( !prop.hasLov ) {
        let newPropAttributeValue = { ...prop };
        uwPropertySvc.setValue( newPropAttributeValue, [ ...dbValue ] );
        uwPropertySvc.updateViewModelProperty( newPropAttributeValue );
        data.dispatch( { path: 'data.propAttributeValue', value: newPropAttributeValue } );
    }
};
 ...
 */
export let applyCriteriaChange = function( dataProvider, criteriaObjects, panelContext, selectedObject, columnDefs, props ) {
    var vmoProp = _.reduce( props, function( sum, prop, key ) {
        ...
        if( key === 'attribute_values' ) {
            ...
            // check - prop.uiValues.length  is added in case of clearing all the attribute_values in the aw-wigdet using clear btn
            // so that even if the uiValues = [], it should not go to if loop and change dbValue to ""
            if( prop.uiValues && prop.uiValues.length > 0 ) {
    ...
    dataProvider.update( criteriaObjects, criteriaObjects.length );
};

/**
 * Remove selected object from criteria object list.
 */
export let removeCriteria = function( dataProvider, criteriaObjects, selectedObj ) {
 ...
 */
export let moveCriteria = function( dataProvider, criteriaObjects, selectedObj, eventType, isMoveUp ) {
 ...
 */
export let resetCriteriaTable = function( dataProvider, criteriaCtx, subscriptionObject ) {
    // Remove selection
