...
var formatUserName = function( vmData ) {
    ...
    var allObjectUid = [ newvmData.modelPropUser.props.fnd0EvalCtxtUsr.dbValue ];
    let deferred = AwPromiseService.instance.defer();
    var cdmObject = _cdm.getObject( allObjectUid[0] );
    if ( cdmObject !== null ) {
        dmSvc.getProperties( allObjectUid, propToLoad ).then( function() {
            var modelObject = _cdm.getObject( allObjectUid[0] );
            if ( modelObject ) {
 ...
 * @param {*} group fnd0EvalCtxGrp
 * @param {*} role fnd0EvalCtxRole
 * @param {*} proj fnd0EvalCtxProj
 * @param {*} vmData fnd0EvalCtxProj
 */
...
function setInitialProperties( newvmData ) {
    ...
    if ( _cdm ) {
        ...
        if ( newvmData.modelPropUser.props.fnd0EvalCtxtUsr ) {
            newvmData.modelPropUser.props.fnd0EvalCtxtUsr.dbValue = userDbVal;
            newvmData.modelPropUser.props.fnd0EvalCtxtUsr.uiValue = userUiVal;
        }
        if ( newvmData.modelPropUser.props.fnd0EvalCtxtGrp ) {
            newvmData.modelPropUser.props.fnd0EvalCtxtGrp.dbValue = groupDbVal;
            newvmData.modelPropUser.props.fnd0EvalCtxtGrp.uiValue = groupUiVal;
        }
        if ( newvmData.modelPropUser.props.fnd0EvalCtxtRole ) {
            newvmData.modelPropUser.props.fnd0EvalCtxtRole.dbValue = roleDbVal;
            newvmData.modelPropUser.props.fnd0EvalCtxtRole.uiValue = roleUiVal;
        }
        if ( newvmData.modelPropUser.props.fnd0EvalCtxtProj ) {
            newvmData.modelPropUser.props.fnd0EvalCtxtProj.dbValue = projDbVal;
            newvmData.modelPropUser.props.fnd0EvalCtxtProj.uiValue = projUiVal;
        }
    }
    return newvmData;
}
