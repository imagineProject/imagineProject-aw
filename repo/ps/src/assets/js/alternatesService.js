...
const excludeTypeForAlternate = [ 'WorkspaceObject', 'POM_application_object', 'POM_object', 'BusinessObject' ];
 ...
 */
export let getSelectedObject = function( subPanelContext ) {
    //At home folder selected data is present in subPanelContext.selected,
    //If go to add alternate from Add group  selected data is present in subPanelContext.selection
    let selectedObjects = subPanelContext.selected ? subPanelContext.selected : subPanelContext.selection;
    let selectedObjectUid = selectedObjects?.length > 0 && selectedObjects[0].props.awb0UnderlyingObject ? selectedObjects[0].props.awb0UnderlyingObject?.dbValues[0] : selectedObjects[0].uid;
    return selectedObjectUid && cdm.getObject( selectedObjectUid );
};
...
export let getPreferredType = function( selectedObject, revisionObj ) {
    ...
    //when we are trying to add the custom part as a alterante of custom item we are not getting the item on the Pallete and the Search
    //So that's why adding typeHierarchyArray as a filter type by excluding the Types of up hierarchy
    if ( items_tag ) {
        types.preferredType = items_tag.type;
        types.filterTypes = items_tag.type + ',' + selectedObject.modelType.typeHierarchyArray
            .filter( type => !excludeTypeForAlternate.includes( type ) )
            .join( ',' );
    } else {
        types.preferredType = selectedObject.type;
        types.filterTypes = selectedObject.modelType.typeHierarchyArray
            .filter( type => !excludeTypeForAlternate.includes( type ) )
            .join( ',' ) + ',' + revisionObj.revTypeName;
    }
    return types;
};
