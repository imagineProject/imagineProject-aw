...
export let getNameofRevisionBaseline = function( response ) {
    if ( response.created && response.created[0] ) {
        var newObject = cdm.getObject( response.created[0] );
        var newObjectUiValue = newObject.props.object_string.uiValues[0];
    } else {
        newObjectUiValue = appCtxSvc.ctx.sublocation.clientScopeURI === 'Awb0OccurrenceManagement' ? appCtxSvc.ctx.occmgmtContext.rootElement.props.object_string.uiValues[ 0 ] : appCtxSvc.ctx.selected.props.object_string.uiValues[ 0 ];
    }
    return newObjectUiValue;
};
