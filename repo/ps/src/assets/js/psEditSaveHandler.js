...
/**
   * Returns the defined save Handler if effectivity text is set in the active ECN
   * If not, returns null to call default save handler
   * @returns {Object} saveHandler
   */
export let getSaveHandler = async function() {
    const cm0GlobalChangeContext = appCtxSvc.getCtx( 'userSession.props.cm0GlobalChangeContext.dbValue' );
    var modeldObj = cdmSvc.getObject( cm0GlobalChangeContext );
    if ( cm0GlobalChangeContext ) {
        // Check if the props is in ECN object
        // if yes, check effectivity_text is already set in the model object,then return ps edit handler
        if ( modeldObj.props && modeldObj.props.effectivity_text ) {
            if ( modeldObj.props.effectivity_text.dbValues.length > 0 && modeldObj.props.effectivity_text.dbValues[ 0 ] !== '' ) {
                return saveHandler;
            }
            // If the effectivity_text is not set, return null to call default save handler
            return null;
        }

        // If not, call the SOA to get the propertie
        // and check if the effectivity_text is set in the result
        // If it is set, return the saveHandler
        // If not, return null to call default save handler
        // This is to avoid calling the SOA if the effectivity text is already set
        const input = {
            objects: [ modeldObj ],
            attributes:  [ 'effectivity_text' ]
        };

        const result = await soaSvc.post( 'Core-2006-03-DataManagement', 'getProperties', input );
        if ( result.modelObjects[cm0GlobalChangeContext]?.props?.effectivity_text?.dbValues.length > 0 &&
            result.modelObjects[cm0GlobalChangeContext].props.effectivity_text.dbValues[0] !== '' ) {
            return saveHandler;
        }
        return null;
    }
...
saveHandler.saveEdits = function( datasource, inputs ) {
    ...
    // Check if the input objects are all valid facade objects. If even one of them is not a facade, then perform only save and not split.
    let areAllInputsFacadeObj = true;
    for ( const currentInput of inputs ) {
        if( currentInput.obj.modelType.typeHierarchyArray.indexOf( 'Awb0Element' ) <= -1 ) {
            areAllInputsFacadeObj = false;
            break;
        }
    }
    ...
    if( areAllInputsFacadeObj && !appCtxSvc.ctx.aceActiveContext.context.isMarkupEnabled ) {
