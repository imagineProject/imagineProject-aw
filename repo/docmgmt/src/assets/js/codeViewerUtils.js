...
export const load = function( viewModel ) {
    ...
    xhr.onload = function( e ) {
        if( this.status === 200 ) {
            ...
            toggleWordWrap( viewModel ); // turn on word wrap by default
        }
    };
    xhr.send();
...
export const loadSave = function( viewModel, method, callback ) {
    if( viewModel.data.editor.dataset ) {
        ...
        if (method === 'save') {
            const content = viewModel.data.editor.content;
            const chunkSize = 2 * 1024 * 1024; // 2MB chunks
            const delimiter = '|||CHUNK_BOUNDARY|||';
            
            if (content.length > chunkSize) {
                // Multiple chunks - simple splitting
                ...
                compressedContent = compressedChunks.join(delimiter);
            } else {
                // Single chunk
        ...
        var inputData = {
            ...
            action: method,
            content: method === 'save' ? compressedContent : ''
        };
