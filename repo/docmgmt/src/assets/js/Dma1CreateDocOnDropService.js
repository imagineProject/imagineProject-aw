 ...
 * @param {Object} selectionData - the selection controller
 *
 ...
 */
export let createDocAttachFiles = function( targetObject, sourceObjects, relationType, runActionWithViewModel, selectionData ) {
    var pasteFilesInput = [ {
        targetObject,
        sourceObjects,
        relationType,
    ...
    return preferenceService.getStringValue( 'Dma1CreateDocOnDrop' ).then( function( prefSvcResult ) {
        ...
        if( objectTypeToCreate.indexOf( 'ShowCreatePanel' ) === 0 ) {
            ...
            appCtxService.updatePartialCtx( 'state.params.cmdArg', objectType.split( /[&,]/ ) );
            ...
            if( promise ) {
                promise.then( function() {
                    // change the selected object to the target folder
                    let newSelectionData = { ...selectionData.getValue() };
                    newSelectionData.selected = [ targetObject ];
