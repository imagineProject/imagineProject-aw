...
export default class GlobalSearchService extends AwBaseService {
    constructor() {
        ...
        this.PREFILTER2_PREFERENCE_VALUE_FOR_CROSS_DOMAIN = 'Any Source';
    ...
    */
    ...
    populateDataProviders( data ) {
        ...
        if( prefilters === undefined ) {
            ...
            // To show the source prefilter only when preference is set to PREFILTER2_PREFERENCE_VALUE_FOR_CROSS_DOMAIN
            let displayCrossDomainPrefilter = this._preFilters.length > 1 && this._preFilters[ 1 ]?.internalName && this._preFilters[ 1 ].internalName === this.PREFILTER2_PREFERENCE_VALUE_FOR_CROSS_DOMAIN;
            if( !displayCrossDomainPrefilter ) {
                this._dataSourcesReceivedFromServer = [];
            }
            if( this._dataSourcesReceivedFromServer.length === 0  ) {
     ...
     */
    performGlobalSearch2( criteria, ownerPrefilter, categoryPrefilter, anySource, imageURL ) {
        this.performGlobalSearch( criteria, ownerPrefilter, categoryPrefilter, anySource, imageURL );
    }
    /**
     ...
     * @param {Object} categoryPrefilter categoryPrefilter
     */
    performGlobalSearch( criteria, ownerPrefilter, categoryPrefilter, anySource, imageURL ) {
        ...
        this.forceSearch( criteria, filterMap, imageURL );
    }
    ...
    forceSearch( criteria, filterMap, imageURL ) {
    ...
    populatePrefilters2( data ) {
        ...
        if ( preferences && preferences.AWS_SearchPreFilter_Property1_SelectedValue ) {
            selectedPreFilter = preferences.AWS_SearchPreFilter_Property1_SelectedValue[0];
        }
        const { dispatch } = data;
        let newData = { ...data };
        ...
        } else if ( selectedPreFilter && prefilters.ownPrefilters.response ) {
            for ( let i = 0; i < prefilters.ownPrefilters.response.length; i++ ) {
                if ( prefilters.ownPrefilters.response[i].staticElementObject === selectedPreFilter ) {
                    prefilter1Val = prefilters.ownPrefilters.response[i].staticDisplayValue;
                    newData.selectPrefilter1.dbValue = prefilters.ownPrefilters.response[i].staticElementObject;
        ...
        if( this._dataSourcesReceivedFromServer.length > 0  ) {
            let prefilter3Val = [ ...this._dataSourcesReceivedFromServer ];
            ...
            if ( preferences && preferences.AWS_SearchPreFilter_Property2_SelectedValue ) {
                selectedPreFilter = preferences.AWS_SearchPreFilter_Property2_SelectedValue[0];
            }
            ...
            } else if ( prefilters.catPrefilters && selectedPreFilter ) {
                for ( let i = 0; i < prefilters.catPrefilters.response.length; i++ ) {
                    if ( prefilters.catPrefilters.response[i].staticElementObject === selectedPreFilter ) {
                        prefilter2Val = prefilters.catPrefilters.response[i].staticDisplayValue;
                        newData.selectPrefilter2.dbValue = prefilters.catPrefilters.response[i].staticElementObject;
                        break;
                    }
                }
            } else{
    ...
    getDataSourceList() {
        return preferenceSvc.getStringValues( 'XST_cross_domain_sources' ).then( ( prefValue ) => {
            if( !prefValue ) {
                ...
                // When array contains only 1 value then it is 'Teamcenter'
                // So no need to show source prefilter
                // So this._dataSourcesReceivedFromServer set to empty
                if( prefValue && prefValue.length > 1 ) {
    ...
    /**
      ...
      * @returns {ObjectArray} VMOs for input list of items
      */
    ...
    getSelectedDataSources( data ) {
        ...
        preferenceSvc.getStringValues( name ).then( ( prefValue ) => {
            ...
            //Preference will not have any value after Xsearch install.
            //Use cached previous selected sources which is set to all source as selected during initialize.
            if( !prefValue ) {
                myArray = value;
            ...
            viewModelPrefs.dbValues = _.clone( myArray );
            viewModelPrefs.dbValue = _.clone( myArray );
            viewModelPrefs.uiValue = myString;
            viewModelPrefs.uiValues = _.clone( myArray );
            viewModelPrefs.displayValues = _.clone( myArray );
            viewModelPrefs.value = _.clone( myArray );

            this._prevSelectedDataSources = JSON.parse( JSON.stringify( myArray ) );
    ...
    /**
      * updatePrefilter3
      * 1. Set the current selected source to Preference.
      * 2. Cache the current selected source in class variable
      * 3. If source selection is updated from only Teamcenter selected or to only Teamcenter selected
      *    then publich event to change revision rule and disable/enable prefilter 1
      * @function updatePrefilter3
      * @param {Object} data data
      * @param {Object} prefilter3VMP Prefilter3 VMP
      * @returns {Object} updated prefilter1.
      */
    updatePrefilter3( data, prefilter3VMP ) {
        let selectedDataSources = JSON.parse( JSON.stringify( prefilter3VMP.checked ) );
        ...
        this._prevSelectedDataSources = selectedDataSources;
