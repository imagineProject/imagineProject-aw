...
export const AwAdvancedSearchAttributesRenderFunction = ( props ) => {
    ...
    const getAttributeValues = ( attribute ) => {
        ...
        if( attribute.typex === 'STRING' && props && props.enterKeyAction ) {
            return (
                <AwWidget {...attribute} hint={renderHint} action={props.enterKeyAction}></AwWidget>
            );
        }
        if( attribute.typex && attribute.typex.endsWith( 'ARRAY' ) ) {
            attribute.reorder = 'false';
        }
        return (
