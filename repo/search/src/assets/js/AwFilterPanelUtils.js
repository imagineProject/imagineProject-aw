    ...
    if( facetSearchString && facetSearchString.length === 0 || !facetSearchString || hasMoreFacetValues ) {
        ...
        for( let index = 0; index < filterValues.length; index++ ) {
            if( filterValues[ index ].name &&
                ...
                filterValues[ index ].name &&
                AwFilterPanelUtils.removeAccents( filterValues[ index ].name ).toLowerCase().indexOf( AwFilterPanelUtils.removeAccents( actualFacetSearchString ).toLowerCase() ) !== -1 ||
                filterValues[ index ].internalName &&
                AwFilterPanelUtils.removeAccents( filterValues[ index ].internalName ).toLowerCase().indexOf( AwFilterPanelUtils.removeAccents( actualFacetSearchString ).toLowerCase() ) !== -1
            ) {
                filterContentInfo.push( filterValues[ index ] );
            }
        }
...
export const getSelectedFiltersFromCategory = ( category ) => {
    let selectedFilters = [];
...
export let getSearchFilterMapForFacetSearch = ( searchState, category ) => {
    let searchStateContext = { ...searchState.getValue() };
    ...
    let isBulkMode = searchStateContext && searchStateContext.autoApplyFilters === false;
    if( isBulkMode ) {
        activeFilterMapInSearchState = applyPendingFilterMaps( activeFilterMapInSearchState, searchStateContext.negativePendingFilterMap, searchStateContext.positivePendingFilterMap );
    }
    ...
    if( categories && categories.length > 0 ) {
        for( let index = 0; index < categories.length; index++ ) {
            ...
            if( eachCategory.internalName === category.internalName ) {
                selectedFiltersInFacetInputCategory = AwFilterPanelUtils.getSelectedFiltersFromCategory( eachCategory );
                break;
            }
        }
            ...
            for( const [ key, value ] of Object.entries( activeFilterMapInSearchState ) ) {
                if( key === category.internalName ) {
                    ...
                    if( activeFilters && activeFilters.length > 0 && selectedFiltersInFacetInputCategory && selectedFiltersInFacetInputCategory.length > 0 ) {
                        activeFilterMapInSearchState[ key ] = AwFilterPanelUtils.constructActiveFilterMapForFacetSearch( activeFilters, selectedFiltersInFacetInputCategory );
                    }
                }
            }
        }
...
export let applyPendingFilterMaps = ( activeFilterMap, negativePendingFilterMap, positivePendingFilterMap ) => {
    let updatedFilterMap = activeFilterMap;
    if( negativePendingFilterMap ) {
        let filteredMap = {};
        for( const [ key, filterValues ] of Object.entries( updatedFilterMap ) ) {
            const filterValuesToRemove = negativePendingFilterMap[key];
            for( const value of filterValues ) {
                let shouldRemove = false;
                if( filterValuesToRemove ) {
                    for( const toRemove of filterValuesToRemove ) {
                        if( toRemove.searchFilterType === value.searchFilterType &&
                        toRemove.stringValue === value.stringValue &&
                        toRemove.stringDisplayValue === value.stringDisplayValue
                        ) {
                            shouldRemove = true;
                            break;
                        }
        ...
        updatedFilterMap = filteredMap;
    }
    if( positivePendingFilterMap ) {
        for( const [ key, positiveValues ] of Object.entries( positivePendingFilterMap ) ) {
            if( updatedFilterMap.hasOwnProperty( key ) ) {
                updatedFilterMap[key].push( ...positiveValues );
            } else {
                updatedFilterMap[key] = [ ...positiveValues ];
            }
        }
    }
...
export let setMapForFilterValueSearch = ( searchFilterMap, searchStateContext, category, startIndex ) => {
    if( searchFilterMap ) {
        ...
        if( startIndex > 0 ) {
            ...
            if( filterValues && filterValues.length > 0 ) {
                for( let index = 0; index < filterValues.length; index++ ) {
                    ...
                    isDuplicateFilterValueInfo = AwFilterPanelUtils.removeDuplicateSelectedFilters( searchStateContext, category, filterValue );
        ...
        searchStateContext.searchFilterMap[ category.internalName ] = AwFilterPanelUtils.arrangeFilterMap( searchStateContext, category );
    } else {
...
export let setProviderForFacetSearch = ( providerName, searchFromLocation ) => {
    if( providerName === shapeSearchProviderName && searchFromLocation === 'shapeSearch' ) {
        return fullTextSearchProviderName;
    }
    return providerName;
};
...
export let updateCategoriesAfterFacetSearch = ( response, searchState, categoryForFacetSearchInput, category ) => {
    ...
    category.filterValues = filterPanelService.getFiltersForCategory( category,
        searchStateContext.searchFilterMap, searchStateContext.objectsGroupedByProperty.internalPropertyName, searchStateContext.colorToggle, undefined, searchStateContext.hideFilterCount );
...
export const updateCategoryToChartBy = ( currentChartByCategoryName, oldChartByCategoryName, searchState, doNotModifySession ) => {
...
export let shiftSelectFilterCallBackAction = ( filter, shiftSelectFilterAction ) => {
    shiftSelectFilterAction( filter );
};

export let shiftSelectFilterAction = ( filter, category, searchState ) => {
    let isBulkFilteringMode = searchState.bulkFiltering;
    let lastSelectedFilterAndCategoryInfo = searchState.lastSelectedFilterAndCategoryInfo;
    let newSearchState = { ...searchState.getValue() };
    let filtersToAdd = [];
    if( isBulkFilteringMode && lastSelectedFilterAndCategoryInfo
         && lastSelectedFilterAndCategoryInfo.lastAction === 'selectFilter'
         && lastSelectedFilterAndCategoryInfo.lastSelectedFilter.categoryName === filter.categoryName
         && lastSelectedFilterAndCategoryInfo.lastSelectedFilter.internalName !== filter.internalName ) {
        let lastSelectedFilterKey = lastSelectedFilterAndCategoryInfo.lastSelectedFilter.key;
        let currentSelectedFilterKey = filter.key;
        let filterMapCategory = newSearchState.searchFilterMap[ category.internalName ];
        let newCategory;
        if( lastSelectedFilterKey < currentSelectedFilterKey ) {
            for( let i = lastSelectedFilterKey; i <= currentSelectedFilterKey; i++ ) {
                if( filterMapCategory[ i ].selected === false ) {
                    filterMapCategory[ i ].selected = true;
                }
            }
            newSearchState.categories = filterPanelService.getCategories3( newSearchState, !newSearchState.hideRange );
            newCategory = newSearchState.categories.find( arrayCategory => {
                return arrayCategory.internalName === category.internalName;
            } );
            for( let i = lastSelectedFilterKey; i <= currentSelectedFilterKey; i++ ) {
                let event = {
                    target: {
                ...
                category.filterValues[ i ].selected.onChange( event );
                filtersToAdd.push( newCategory.filterValues[ i ] );
            }
        } else {
            for( let i = lastSelectedFilterKey; i >= currentSelectedFilterKey; i-- ) {
                if( filterMapCategory[ i ].selected === false ) {
    ...
    _.forEach( filtersToAdd, function( filter ) {
        updateInputSearchFilterMap( filter, category, newSearchState, false );
    } );
    searchState.update( newSearchState );
};
...
export let updateInputSearchFilterMap = ( filter, category, searchState, updateSearchState ) => {
    let newSearchState = searchState;
    let newFilter = filter;
    if( updateSearchState ) {
        newSearchState = { ...searchState.getValue() };
...
export const setFocusForFilter = ( filter, isFilterByCategorySearchStringValid ) => {
    if( filter && filter.autoFocus ) {
        filter.selected.autoFocus = filter.autoFocus && !isFilterByCategorySearchStringValid ? filter.autoFocus : false;
    }
    return filter;
};

/**
 * setCriteriaForFacetSearch
 * @function setCriteriaForFacetSearch
 * @param {Object}searchState - search state object
 * @param {String}categoryName - category to facet on
 * @param {String}facetSearchString - the string entered for facet search
 * @param {Boolean}isRangeSearch - is the category having range based search
 * @return searchCriteria - combined searchCriteria which also includes additionalCriteria set in search state object.
 */
export const setCriteriaForFacetSearch = ( searchState, categoryName, facetSearchString, isRangeSearch ) => {
    let searchCriteria = searchState.criteria ? searchState.criteria : {};
    let additionalCriteria = searchState.additionalCriteria ? searchState.additionalCriteria : {};
    searchCriteria.categoryForFacetSearch = categoryName;
    searchCriteria.facetSearchString = facetSearchString;
    searchCriteria.isRangeSearch = isRangeSearch;
    searchCriteria = awSearchService.getShapeSearchCriteria( searchCriteria );
    awSearchService.processNaturalLanguage( searchState, searchCriteria );
    searchCriteria = { ...searchCriteria, ...additionalCriteria };
    searchCriteria.forceThreshold = AwFilterPanelUtils.getForceThresholdValue( searchState.providerName, searchState.forceThreshold );
    return searchCriteria;
};

export let removeAccents = ( inputString ) => {
    return inputString.normalize( 'NFD' ).replace( /[\u0300-\u036f]/g, '' );
};

export let validateNumericValue = ( inputValue ) => {
    let value = inputValue.replace( /[^0-9.-]/g, '' );
    if( value === '.' ) { // Do not allow only decimal to be entered
        value = '';
    } else if ( value.indexOf( '.' ) !== -1 ) {
        const parts = value.split( '.' );
        if ( parts.length > 2 ) {
            value = parts[0] + '.' + parts[1]; // Keep only one decimal point, remove extra decimal points entered
        }
    }
    return value;
};

export let getForceThresholdValue = ( providerName, currentForceThresholdValue ) => {
    if( providerName === 'Awp0FullTextSearchProvider' || providerName === 'SS1ShapeSearchDataProvider' ) {
        return 'false';
    }
    return currentForceThresholdValue ? currentForceThresholdValue : 'false';
};
...
const AwFilterPanelUtils = {
    ...
    shiftSelectFilterCallBackAction,
    ...
    setCriteriaForFacetSearch,
    removeAccents,
    setProviderForFacetSearch,
    validateNumericValue,
    getForceThresholdValue
};
