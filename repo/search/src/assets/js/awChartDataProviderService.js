 ...
 */
export let triggerSearchFilterTargetCategoryEvent = function( targetFilterCategory, searchState, doNotModifySessionChartByCategory ) {
    ...
    if( targetFilterCategory !== newSearchState.objectsGroupedByProperty.internalPropertyName ) {
        ...
        newSearchState = AwFilterPanelUtils.updateCategoryToChartBy( targetFilterCategory, newSearchState.objectsGroupedByProperty.internalPropertyName, newSearchState, doNotModifySessionChartByCategory );
    }
...
export let selectCat = function( chartListboxPropData, searchState ) {
    let searchContext = { ...searchState.getValue() };
    var label = searchContext.chartTitle;
    if( !label || !searchContext.forceChart && !searchContext.chartBy ) {
        return;
    }
...
export let buildChartProvider = function( searchFilterTargetCategory, searchState, columnCount, chartProvider ) {
    let searchData = searchState.getValue();
    ...
    chartProvider.onSelect = function( column ) {
        try {
            ...
            AwFilterPanelUtils.updateInputSearchFilterMap( filters[ 0 ], chartProvider.category, searchState, true );
        } catch ( e ) {
            console.info( e );
        }
    };
...
export let updateChartListboxPropData = ( chartListboxPropData, searchState )=> {
    ...
    if( searchState.searchFilterCategories && searchState.chartBy ) {
        ...
        if( selCategory ) {
            ...
            nwChartListboxPropData.dbValues = [ selCategory.displayName ];
            nwChartListboxPropData.dispValue = [ selCategory.displayName ];
            nwChartListboxPropData.checked = selCategory.internalName;
            nwChartListboxPropData.value = selCategory.internalName;
        }
    }
