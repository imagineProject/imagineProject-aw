...
export let openCloseFilterPanelActionWhenCategoriesFound = ( autoOpenFilterPanel, openSearchFilterPanel, activationNavigationCommand, isNarrowMode, activationToolsAndInfoCommand, searchResultDialogAction, searchState  ) => {
    let markUpOptions = {
        view: 'Awp0SearchFilter',
    ...
    if( !autoOpenFilterPanel && activationNavigationCommand
        ...
        dialogService.closeDialog( 'INFO_PANEL_CONTEXT', searchState.searchFilterPopupId );
        ...
        // eslint-disable-next-line max-len
        // this is for the use case where executing a search and getting back categories should result in filter panel opening up if it is already not open this usecase is for shapesearch location, to account for sliders size
        if ( searchState.shapeSearchPopupId ) {
            markUpOptions.width = 'SMALL';
        }
        searchResultDialogAction.show( markUpOptions );
    }
};

export let openCloseFilterPanelAction = ( searchState, searchResultDialogAction ) => {
    ...
    if( searchState.totalLoaded !== undefined
        ...
        awSearchLocationFilterPanelService.openCloseFilterPanelActionWhenCategoriesFound(
            autoOpenFilterPanel, openSearchFilterPanel, activationNavigationCommand, isNarrowMode, activationToolsAndInfoCommand, searchResultDialogAction, searchState  );
    }
};
...
export let setFilterPanelStateSessionVariableForCloseButton = ( searchState ) => {
