...
import searchTabPolicies from 'policies/searchObjectPropertyPolicy';
...
const searchStateAttributes = [ 'totalFound', 'totalLoaded', 'endIndex', 'startIndex', 'searchFilterCategories', 'additionalSearchInfoMap', 'unpopulatedSearchFilterCategories', 'searchFilterMap',
    ...
    'searchString', 'searchFilterCategories', 'searchFilterMap', 'filterMap', 'referencingSavedQuery', 'advancedSearchJSONString', 'savedQuery', 'savedQueryAttributes', 'searchCriteriaMap', 'cursorInfoString'
];
...
export let getColumns = function( columns ) {
    ...
    for ( var i = 0; i < columns.length; i++ ) {
        tempCols.push( {
            columnOrder: columns[i].columnOrder,
            hiddenFlag: columns[i].hiddenFlag,
            pixelWidth: columns[i].pixelWidth,
            propertyName: columns[i].propertyName,
...
export const initializeSearchState = ( searchState, sourceSearchFilterMap, subPanelContext ) => {
    let newSearchState;
    try {
        newSearchState = subPanelContext.searchState ? subPanelContext.searchState.getValue() : { ...searchState.value };
    } catch( err ) {
        console.log( err );
        console.log( 'initializeSearchState' );
    }
    ...
    if( !subPanelContext.searchState ) {
        ...
        newSearchState.policy = searchTabPolicies;
...
export const createActiveFiltersFromActiveFilterMap = ( activeFilterMap ) => {
    return awSearchSublocationService.createActiveFiltersFromActiveFilterMap( activeFilterMap );
};
...
const constructActiveFilterMapFromOutputSearchFilterMap = ( searchState ) => {
    let newSearchState;
    try {
        newSearchState = _.cloneDeep( searchState );
    } catch( e ) {
        console.log( e );
        console.log( 'constructActiveFilterMapFromOutputSearchFilterMap' );
    }
    let categories = filterPanelService.getCategories3( newSearchState, !searchState.hideRange );
    let activeSelectedFiltersMap = awSearchSublocationService.getSelectedFiltersMap( categories );
...
export const updateSearchStateAfterFilterAction = ( searchState, showPrefilter ) => {
    ...
    try {
        ...
        console.log( e );
        console.log( 'updateSearchStateAfterFilterAction' );
    }
...
export const processOutputSearchFilterCategories = ( searchState, outputSearchFilterCategories ) => {
    ...
    let newSearchFilterMap = searchState.searchFilterMap;
    ...
    } else if( searchState.immutableSearchInfo && searchState.immutableSearchInfo.immutableSearchFilterMap && outputSearchFilterCategories.length > 0 ) {
        for( const [ key, value ] of Object.entries( searchState.immutableSearchInfo.immutableSearchFilterMap ) ) {
            if( value.length === newSearchFilterMap[key]?.length ) {
                var index = outputSearchFilterCategories.findIndex( item=> item.internalName === key );
                if( index !== -1 ) {
                    outputSearchFilterCategories.splice( index, 1 );
                }
            }
        }
    } else if( searchState.removeProjectsCategoryFromSearchCategories ) {
        for( let indexOfCategory = 0; indexOfCategory < outputSearchFilterCategories.length; indexOfCategory++ ) {
            if( outputSearchFilterCategories[ indexOfCategory ].internalName !== 'WorkspaceObject.project_list' ) {
                updatedSearchFilterCategories.push( outputSearchFilterCategories[ indexOfCategory ] );
            }
        }
        return updatedSearchFilterCategories;
...
export const processOutputFilterMap = ( newSearchState, outputSearchFilterMap ) => {
    let updatedSearchFilterMap = outputSearchFilterMap ? _.cloneDeep( outputSearchFilterMap ) : {};
    let onlySelectedTypeFilters = [];
    let nwUserSelectedCategories = [];
    if( newSearchState.immutableCategories && newSearchState.immutableCategories.length > 0 && Object.keys( updatedSearchFilterMap ).length > 0 ) {
        let immutableCategories = newSearchState.immutableCategories.split( ',' );
        _.forEach( immutableCategories, ( eachImmutableCategory ) => {
            delete updatedSearchFilterMap[ eachImmutableCategory ];
        } );
    } else if( newSearchState.immutableSearchInfo && newSearchState.immutableSearchInfo.immutableSearchFilterMap && Object.keys( updatedSearchFilterMap ).length > 1 ) {
        for( const [ searchFilterMapKey, searchFilterMapValues ] of Object.entries( updatedSearchFilterMap ) ) {
            if ( newSearchState.immutableSearchInfo.immutableSearchFilterMap[searchFilterMapKey] ) {
                _.forEach( searchFilterMapValues, ( filter ) => {
                    var filterIndex = newSearchState.immutableSearchInfo.immutableSearchFilterMap[searchFilterMapKey].findIndex( item => item.stringValue === filter.stringValue );
                    if( filterIndex !== -1 ) {
    ...
    if( newSearchState.applyPresetTypeFilters && !newSearchState.userSelectedTypeFiltersApplied &&
    ...
    } else if( newSearchState.applyPresetTypeFilters && newSearchState.userSelectedTypeFiltersApplied &&
        updatedSearchFilterMap[ filterPanelUtils.PRESET_CATEGORY ] &&
        updatedSearchFilterMap[ filterPanelUtils.PRESET_CATEGORY ].length > 0  ) {
        for( let index = 0; index < updatedSearchFilterMap[ filterPanelUtils.PRESET_CATEGORY ].length; index++ ) {
            if( updatedSearchFilterMap[ filterPanelUtils.PRESET_CATEGORY ][ index ].selected ) {
        ...
        var savedPresetCategoryMap = newSearchState.presetCategoryMap2;
        for( let j = 0; j < onlySelectedTypeFilters.length; j++ ) {
            for( let index = 0; index < savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ].length; index++  ) {
                if( onlySelectedTypeFilters[ j ].stringValue === savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ][ index ].stringValue ) {
                    savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ][ index ] = onlySelectedTypeFilters[ j ];
                    nwUserSelectedCategories =  [  ...nwUserSelectedCategories, ...savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ].splice( index, 1 ) ];
                }
            }
        }
        for( let index = 0; index < savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ].length; index++  ) {
            if( savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ][ index ].selected  ) {
                savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ][ index ].selected = false;
            }
        }
        for( let j = 0; j < savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ].length; j++  ) {
            nwUserSelectedCategories.push( savedPresetCategoryMap[ filterPanelUtils.PRESET_CATEGORY ][ j ] );
            updatedSearchFilterMap[ filterPanelUtils.PRESET_CATEGORY ] = nwUserSelectedCategories;
        }
        if( nwUserSelectedCategories.length === onlySelectedTypeFilters.length ) {
            updatedSearchFilterMap[ filterPanelUtils.PRESET_CATEGORY ] = nwUserSelectedCategories;
        }
    }
    updatedSearchFilterMap = searchCommonUtils.updateSelectedFlagForImmutableSearchFiltersInOutputSearchFilterMap( newSearchState, updatedSearchFilterMap );
    newSearchState.presetCategoryMap2 = updatedSearchFilterMap;
...
export let constructAppliedFiltersAndCategoriesWhenEmptyCategoriesAndZeroResults = ( searchState, currentActiveFilters ) => {
    ...
    if( searchState && currentActiveFilters && searchState.categories &&
        ...
        const activeSelectedFiltersMap = awSearchSublocationService.getSelectedFiltersMap( categories );
...
export const toggleSearchView = ( activeView, activeViewState, selectionModel, searchState ) => {
    try {
        ...
        activeViewState.update( { ...activeViewState.getValue(), name: activeView } );
    } catch( e ) {
        console.log( e );
        console.log( 'toggleSearchView' );
    }
};
...
export const resetSearchState = ( searchState, selectionModel ) => {
    ...
    try {
        searchStateValue = { ...searchState.getValue() };
    } catch( e ) {
        console.log( e );
        console.log( 'resetSearchState' );
    }

    for( let key in searchStateValue ) {
        if( searchStateAttributes.includes( key ) && !searchStateValue.excludeKeysFromResetSearchState?.includes( key ) ) {
 ...
 * @returns {Object} object that contains showSearchFilter, selectedSearchFilters and searchFilterMap if context.isIncludeSubTypes is false otherwise return null
 */
function getSearchFilters( context ) {
    if ( context.isIncludeSubTypes === 'false' ) {
        let typeNames = [];
        let showSearchFilter = true;
        let selectedSearchFilters = [];
        let typeFilter = context.typeFilter.split( ',' );
        for( let type in typeFilter ) {
            let searchFilter = {
                searchFilterType: 'StringFilter',
                stringValue: typeFilter[type].trim()
            };
...
export let findSubBusinessObjectsAndInvokeSearch = function( data, context ) {
    ...
    if( context.typeFilter ) {
        ...
        let searchFilter = getSearchFilters( context );
        if ( searchFilter ) {
            data.searchFilterMap = searchFilter.searchFilterMap;

            return AwPromiseService.instance.resolve( searchFilter );
        }
...
export let updateSearchStateWithIncontentCriteria = ( searchState, incontentSearchBoxString ) => {
    ...
    try {
        ...
        console.log( e );
        console.log( 'updateSearchStateWithIncontentCriteria' );
    }
    let oldSearchString = newSearchState?.criteria?.searchString;
...
export let validateRevisionRuleLOVValueSelections = function( lovEntries, viewProp, operationName, viewModelObj, owningObjUid ) {
    if( viewModelObj ) {
        return lovService.validateLOVValueSelections( lovEntries, viewProp, operationName, viewModelObj, owningObjUid );
    }
};
...
const searchCommonUtils = {
    ...
    updateSearchString,
    validateRevisionRuleLOVValueSelections
};
