...
export let buildTitle = function( searchObject ) {
    if( searchObject ) {
        ...
        if( searchObject.criteria && searchObject.criteria.searchString ) {
            if ( searchObject.immutableSearchInfo && searchObject.immutableSearchInfo.immutableSearchString  ) {
                let searchStr = searchObject.criteria.searchString.split( 'AND' ).map( part => part.trim() );
                searchCriteria = searchStr.filter( item => item !== searchObject.immutableSearchInfo.immutableSearchString ).join( ' AND ' );
            } else {
...
/**
 * initializeQueryType
 * @function initializeQueryType
 * @param {Object} queriesTypeSelector - the query switch which contains the selected query type
 * @param {Object} searchState - search state object
 * @param {String} allText - localized string for "All"
 */
export let initializeQueryType = function( queriesTypeSelector, searchState, allText ) {
    let newSearchstate = searchState ? { ...searchState.getValue() } : {};
    newSearchstate.selectedQueryType = '';
    ...
    queriesTypeSelector.value = 'all';
    queriesTypeSelector.uiValue = allText;
    return queriesTypeSelector;
};
 ...
 * @param {Object} searchState - search state object
 */
export let updateQueryType = function( queriesTypeSelector, searchState ) {
    let newSearchstate = searchState ? { ...searchState.getValue() } : {};
    let selectedQueryType = queriesTypeSelector.dbValue === 'all' ? '' : queriesTypeSelector.dbValue;
    newSearchstate.selectedQueryType = selectedQueryType;
    if( !selectedQueryType || selectedQueryType === '' ) {
        newSearchstate.label = {
            key: 'queriesText',
            source: '/i18n/SearchMessages'
        };
    } else if( selectedQueryType === 'public' ) {
        newSearchstate.label = {
            key: 'publicQueriesText',
            source: '/i18n/SearchMessages'
        };
    } else if( selectedQueryType === 'private' ) {
        newSearchstate.label = {
            key: 'privateQueriesText',
            source: '/i18n/SearchMessages'
        };
    }

    //Clear the search string
    newSearchstate.searchStringSecondary = '';
    if( newSearchstate && newSearchstate.criteria ) {
