...
/**
  ...
  * @return {Object}data - updated view model data
  */
...
export let getChartByDisplayValue = function( chartOnProperty, chartByValue, searchFilterCategories ) {
    let chartBy = _.cloneDeep( chartOnProperty );
    let chartByDispValue = '';

    if( !searchFilterCategories || searchFilterCategories.length === 0 ) {
        chartByDispValue =  chartByValue ? searchFilterSvc.getCategoryDisplayName( chartByValue ) : '';
    } else {
        let category = searchFilterCategories.find( category => category.internalName === chartByValue );
        chartByDispValue = category?.displayName;
    }
...
export const cleanupUnsavedChartProp = ( editInProgress, chartListboxPropData, searchState ) => {
    if( editInProgress ) {
        awChartDataProviderService.updateChartListboxPropData( chartListboxPropData, searchState );
    }
};
