...
/**
  ...
  * @param {Object} data - view model data.
  */
export let getPropertiesForAwp0SearchType = ( objects, attributes, data ) => {
    let searchState = {
        callDataProvider: true,
        clientScopeURI: clientScopeURIFullTextSearch,
        columnConfigId: 'searchResultsColConfig'
    };
    let editInProgress = appCtxService.getCtx( 'editInProgress' );
    if( editInProgress ) {
        return searchState;
    }
    let objectUID = objects[0].uid;
...
export const processOutput = ( data, dataCtxNode, searchData ) => {
    ...
    if( dataCtxNode && dataCtxNode.searchState && dataCtxNode.searchState.awp0SearchType === '1' ) {
        newSearchData.provider = 'Awp0FullTextSearchProvider';
    } else if( dataCtxNode && dataCtxNode.searchState &&
        ( dataCtxNode.searchState.awp0SearchType === '2' || dataCtxNode.searchState.awp0SearchType === '4' ) ) {
        newSearchData.showIntermediateResultCount = true;
        newSearchData.provider = 'Awp0SavedQuerySearchProvider';
...
export let setColumnConfig = function( response ) {
    ...
    if( response ) {
        ...
        if( response.additionalSearchInfoMap ) {
            let nonIndexedPrefs = response.additionalSearchInfoMap.nonIndexedColumns;
            if( nonIndexedPrefs ) {
...
export let getStartIndexForSearchFolder = ( dataProvider, searchFolder ) => {
    if( searchFolder && searchFolder.endIndex && searchFolder.provider === 'Awp0SavedQuerySearchProvider' && dataProvider.startIndex !== 0 ) {
        return parseInt( searchFolder.endIndex ) + 1;
    }
    return dataProvider.startIndex;
};
