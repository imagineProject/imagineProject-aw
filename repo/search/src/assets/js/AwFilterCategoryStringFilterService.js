...
export const awFilterCategoryStringFilterRenderFunction = ( props ) => {
    ...
    let disableTypeAheadSearch = ctx.preferences && ctx.preferences.AW_DisableTypeAheadFacetSearch && ctx.preferences.AW_DisableTypeAheadFacetSearch[ 0 ].toLowerCase() === 'true';
    ...
    let validFacetSearchString = facetSearchString && facetSearchString.dbValue && facetSearchString.dbValue.length > 0;
    ...
    const performFacetSearch = ( facetSearchString ) => {
        ...
        if( enableWildcardFiltering ) {
            ...
            categoryForFacetSearchInput.isServerSearch = true;
    ...
    const currentWildcardSelection = ( newSelection ) => {
        ...
        dispatch( { path: 'data', value: { currentWildcardOperator: newSelection } } );
        if( wildcardSelectionChanged ) {
            addWildcardCallBack( category, data.currentWildcardOperator, fields.facetSearchString.value, wildcardSelectionChanged, false );
            if( isBulkMode ) {
                performFacetSearch( fields.facetSearchString.value );
            }
        }
    };
    ...
    const addWildcardCallBackFunction = () => {
        addWildcardCallBack( category, data.currentWildcardOperator, fields.facetSearchString.value, undefined, false );
    };
    ...
    const getWildcardComponent = () => {
        ...
        if( enableWildcardFiltering && (  category.wildcardOperatorList && category.wildcardOperatorList.length > 0  || stringWildcardOperators ) ) {
            if( category.wildcardOperatorList.length > 0 ) {
                wildcardOperatorList = category.wildcardOperatorList;
            } else {
                wildcardOperatorList = stringWildcardOperators;
            }
            return (
    ...
    const getFilterSearchBoxComponent = () => {
        if( ( category.showFilterText || enableWildcardFiltering ) && !disableTypeAheadSearch ) {
            return (
                <AwFlexRow>
                    ...
                    </AwFlexColumn>
                    {
                        ...
                        <AwFlexColumn className='aw-search-addWildcardButton'>
                            <AwButtonEnableWhen iconId={isBulkMode ? 'cmdAdd' : 'cmdApply16'} action={addWildcardCallBackFunction} enableWhen={validFacetSearchString}></AwButtonEnableWhen>
                        </AwFlexColumn>
                    }
                    <AwFlexColumn>
                        {
