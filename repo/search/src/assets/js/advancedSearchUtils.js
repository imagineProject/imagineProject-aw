 ...
 */
export let populateQueryAttributesForSavedSearch = async function( queryAttributesVMO, queryAttributeValues ) {
    if( !clientDataModel.getObject( queryAttributesVMO.uid ) ) {
        await dmSvc.loadObjects( [ queryAttributesVMO.uid ] );
    }
        ...
        _.forEach( queryAttributeValues, function( val, key ) {
            if( keepGoing === true ) {
                if( key === prop.propertyName && prop.type !== 'DATE' ) {
                    ...
                    //If the val is in epoch time, need to first convert it to date
                    val = Number.isFinite( Number( val ) ) ? Number( val ) : val;
                    if( typeof val === 'number' ) {
                        val = _convertDate( val );
                    }
...
export let  _convertDate = function( dateInEpochFormat ) {
    if( !dateInEpochFormat || dateInEpochFormat < 0 ) {
        return dateTimeService.NULLDATE;
    }
    return AwFilterService.instance( 'date' )( dateInEpochFormat, 'yyyy-MM-dd' ) + ' ' +
        AwFilterService.instance( 'date' )( dateInEpochFormat, 'HH:mm' );
};
...
export let clearAllAction = function( attributes ) {
    ...
    _.forEach( newAttributes.props, function( prop ) {
        ...
        if( prop.type === 'DATE' ) {
            ...
            var propName = prop.propertyName;
            var propDisplayName = prop.propertyDisplayName;
            if( propName && propDisplayName && ( prop.dbValue !== undefined || prop.newValue !== undefined ) ) {
                if( prop.propertyDescriptor && prop.propertyDescriptor.lovCategory === 1 ) {
                    prop.dbValue = [];
                    prop.value = [];
...
export let populateSavedQueryParametersMap = function( savedQueryAttributes, savedQueryParametersMap ) {
    _.forEach( savedQueryAttributes, function( prop ) {
        if( prop.type !== 'DATE' && !advancedSearchUtils.isPropEmpty( prop.dbValue ) ) {
            ...
            savedQueryParametersMap[ prop.propertyName ] = prop.dbValue;
        }
    } );
