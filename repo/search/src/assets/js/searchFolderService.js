...
export let getSearchFolderDefinitionIntFulltext = function( searchFolderCtx, params, paramValues, searchFolderCriteria, searchFolderFilterMap, searchFilters ) {
    ...
    while( index < paramValues.length ) {
        ...
        if( param === 'ReportSearchCriteria' ) {
            ...
            ++index;
        } else if( param === 'isProjectDataTemplateType' ) {
            searchFolderCtx.isProjectDataTemplateType = paramValues[ index ];
            updateSearchFolderCtx = true;
            ++index;
        } else if( param === 'projectDataTemplateSavedQuery' ) {
            searchFolderCtx.projectDataTemplateSavedQuery = paramValues[ index ];
            updateSearchFolderCtx = true;
            ++index;
        } else {
...
export let getSearchFolderDefinitionIntAdvanced = function( searchFolderCtx, params, paramValues, searchFolderCriteria, searchCriteriaAdvPopulated, searchFilters ) {
    ...
    while( index < paramValues.length ) {
        ...
        if( param === 'ReportSearchCriteria' ) {
            ...
            let paramValue = paramValues[ ++index ];
            let multiValues = paramValue.split( ';' );
            let filterValue = '';
            if( multiValues && multiValues.length > 1 && paramValue.indexOf( searchFolderCommonService.INTERNAL_KEYWORD ) !== -1 ) {
                filterValue = searchFolderCommonService.splitInternalAndDisplayNameUsingInternalKeyword( multiValues, false, false );
            } else {
...
export let processSearchFolderDefinitionInt_Fulltext = function( searchFolderCtx, searchFolderCriteria, params, paramValues, searchFolderFilterMap, searchFilters ) {
    ...
    searchFolderService.setPropsForPropertySpecificSearch( paramValues, searchFolderCriteria );
...
export let getSearchFolderInt_procKnownType = function( searchFolderCtx ) {
    ...
    } else if( awp0SearchType && awp0SearchType.dbValues[ 0 ] === '2' || awp0SearchType && awp0SearchType.dbValues[ 0 ] === '4' ) {
...
export let editSearchFolderRule = function( searchFolder ) {
    const newSearchFolder = { ...searchFolder.getValue() };
...
export let initRuleType = function( ruleType, searchFolder ) {
    ...
    const newSearchFolder = { ...searchFolder.getValue() };
    if( searchFolder.isParentProjectTemplate ) {
        if( searchFolder.awp0SearchType === FULLTEXT_PROVIDER ) {
            updatedRuleType.dbValue = 'projectSearchProvider';
        } else {
            updatedRuleType.dbValue = ADVANCED_PROVIDER;
        }
        searchFolder.update( newSearchFolder );
    } else if( searchFolder.useRule === CURRENT_RULE ) {
...
export let setRuleType = function( ruleType, searchFolder ) {
    ...
    if( ruleType.dbValue === ADVANCED_PROVIDER ) {
        ...
        newSearchFolder.useRule = NEW_RULE;
    } else if( ruleType.dbValue === FULLTEXT_PROVIDER || ruleType.dbValue === 'projectSearchProvider' ) {
        newSearchFolder.awp0SearchType = FULLTEXT_PROVIDER;
        newSearchFolder.useRule = NEW_RULE;
    } else {
...
export let cancelEditSearchFolderRule = function( searchFolder ) {
    ...
    newSearchFolder.onRulePage = true;
    newSearchFolder.useRule = CURRENT_RULE;
    newSearchFolder.totalFound = undefined;
    newSearchFolder.totalLoaded = undefined;
    return newSearchFolder;
};
...
/**
  ...
  */
export let updateNonEditState = ( searchFolder, newSearchFolder ) => {
...
export let updateCriteriaAndFilters = function( data, searchFolderCtx ) {
    ...
    searchFolderService.updateNonEditState( searchFolderCtx, newSearchFolder );
    return data;
};
/**
...
export let getQueryParametersMap = function( savedSearchObject ) {
    ...
    for( let j = 0; j < savedSearchAttributeNames.length; j++ ) {
        ...
        if( multipleValues && multipleValues.length > 0 && value.indexOf( searchFolderCommonService.INTERNAL_KEYWORD ) !== -1 ) {
            filterValue = searchFolderCommonService.splitInternalAndDisplayNameUsingInternalKeyword( multipleValues, true, true );
...
export let getFulltextReportDefinitionCriteriaForSave = function( searchFolderCtx ) {
    ...
    if( searchFolderCtx.isProjectDataTemplateType ) {
        let isProjectDataTemplateType = {
            criteriaName: 'isProjectDataTemplateType',
            criteriaValues: [ searchFolderCtx.isProjectDataTemplateType ]
        };
        criteria.push( isProjectDataTemplateType );
        delete searchFolderCtx.activeFilterMap[ 'WorkspaceObject.project_list' ];
    }

    if( searchFolderCtx.projectDataTemplateSavedQuery ) {
        let projectDataTemplateSavedQuery = {
            criteriaName: 'projectDataTemplateSavedQuery',
            criteriaValues: [ searchFolderCtx.projectDataTemplateSavedQuery ]
        };
        criteria.push( projectDataTemplateSavedQuery );
    }
...
export let getAdvancedReportDefinitionCriteriaForSave = function( searchFolderCtx ) {
    ...
    if( searchFolderCtx.isProjectDataTemplateType ) {
        ...
        searchFolderCriteria.push( isProjectDataTemplateType );
    }
    ...
    if( searchFolderCtx.projectDataTemplateSavedQuery ) {
        ...
        searchFolderCriteria.push( projectDataTemplateSavedQuery );
    }
    let criteria = {};
...
export let setPropsForPropertySpecificSearch = function( paramValues, searchFolderCriteria ) {
    //(V_A_L_0:34) OR (V_A_L_1:ItemRevision|V_A_L_2:ItemRevision|...|V_A_L_9:ItemRevision)
    let propObjects = {};

    // if the paramValues length is more than one, we are dealing with prop specific search so get properties
    if( paramValues.length > 1 ) {
        // start at index 2 since this is where the 'V_A_L_#:Property' strings start
        for( let i = 2; i < paramValues.length; i++ ) {
            let valPropString = paramValues[ i ];
            let valPropArray = valPropString.split( ':' );
            if( valPropArray.length === 2 ) {
                propObjects[ valPropArray[ 0 ] ] = valPropArray[ 1 ];
            }
        }

        let searchStringProperties = [ ...paramValues[ 0 ].matchAll( /"([^"]+)"|([^:()\s\[\]{}<>]+)(?=:)/gu ) ].map( match => match[ 1 ] || match[ 2 ] );
        let termExpressionTokens = paramValues[ 1 ].split( /\sAND\s|\sOR\s/ );
        let termExpressionPropMap = getMapOfTermExpressionsToProps( searchStringProperties, termExpressionTokens, propObjects );

        searchFolderCriteria.propsForPropertySpecificSearch = JSON.stringify( termExpressionPropMap );
    }
};

let getMapOfTermExpressionsToProps = function( searchStringProperties, termExpressionTokens, propObjects ) {
    let termExpressionPropMap = {};

    let termCount = 0;
    _.forEach( termExpressionTokens, function( termExpressionToken ) {
        // check that the term is not in the form "hdd Type:ItemRevision". if it is, only get the property
        let match = termExpressionToken.match( /\(.*V_A_L_\d+:.*\)/ );
        if( !match ) {
            return;
        }
        
        termExpressionToken = match[ 0 ];

        // strip parenthesis and whitespace from token
        termExpressionToken = termExpressionToken.replace( /[()]/g, '' );
        termExpressionToken = termExpressionToken.trim();

        // split token by the | to get the individual props
        let propsInToken = termExpressionToken.split( '|' );

        // get the V_A_L_# from each of the props
        let valArray = propsInToken.map( val => val.match( /^V_A_L_\d+/ )[ 0 ] );

        // get the actual property name from the propObjects and add it to the termExpressionPropMap
        termExpressionPropMap[ searchStringProperties[ termCount ] ] = [];
        valArray.map( val => termExpressionPropMap[ searchStringProperties[ termCount ] ].push( propObjects[ val ] ) );

        termCount = termCount + 1;
    } );

    return termExpressionPropMap;
};

export let clearPropsForPropSpecificSearch = function( searchState ) {
    let newSearchState = { ...searchState.getValue() };
    if( newSearchState.searchFolderCriteria && newSearchState.searchFolderCriteria.propsForPropertySpecificSearch ) {
        newSearchState.searchFolderCriteria.propsForPropertySpecificSearch = null;
    }
...
const searchFolderService = {
    ...
    setPropsForPropertySpecificSearch,
    clearPropsForPropSpecificSearch
};
