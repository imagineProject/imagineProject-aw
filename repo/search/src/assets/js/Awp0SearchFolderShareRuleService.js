 ...
 * @returns {Array} finalObjects - the view model objects in the SOA response to be consumed by the data provider.
 */
export let getSearchFolderAccessors = function( selectedSearchFolderUID, searchString, searchFolderShareRuleState ) {
    ...
    return soaService.post( searchFolderServiceName, searchFolderAccessorSOAName, {
        ...
        let newSearchFolderShareRuleState = { ...searchFolderShareRuleState.getValue() };
        newSearchFolderShareRuleState.accessorsString = getSearchFolderAccessorsString( finalObjects );
...
export let createInputForRemovingSearchFolderAccessors = ( selectedObjects, searchFolderShareRuleState ) => {
    ...
    const newSearchFolderShareRuleState = { ...searchFolderShareRuleState.getValue() };
...
const removeSelectedAccessorsFromAccessorsString = ( selectedAccessorsToRemove, currentAccessorsString ) => {
    ...
    let accessorsUidsToRemove = [];
    for( let accessorToRemove of selectedAccessorsToRemove ) {
        accessorsUidsToRemove.push( accessorToRemove.uid );
    }
    let updatedAccessorsUids = currentAccessorsUids.filter( accessor => !accessorsUidsToRemove.includes( accessor ) );
    return updatedAccessorsUids.join( ',' );
};

export let processRemoveAccessorOutput = ( searchFolderShareRuleState, accessorTableSelectionData, orgTreeSelectionData, projectTableSelectionData ) => {
    const newSearchFolderShareRuleState = { ...searchFolderShareRuleState.getValue() };
    const newAccessorTableSelectionData = { ...accessorTableSelectionData.getValue() };
    newSearchFolderShareRuleState.disableAddButton = true;
    newSearchFolderShareRuleState.disableRemoveButton = true;
    if( newAccessorTableSelectionData.selected && newAccessorTableSelectionData.selected.length > 0 ) {
        newSearchFolderShareRuleState.accessorsString = removeSelectedAccessorsFromAccessorsString( newAccessorTableSelectionData.selected, newSearchFolderShareRuleState.accessorsString );
    } else {
        newSearchFolderShareRuleState.accessorsString = '';
    }
    const newOrgTreeSelectionData = { ...orgTreeSelectionData.getValue() };
    const newProjectTableSelectionData = { ...projectTableSelectionData.getValue() };
    newOrgTreeSelectionData.selected = [];
    newProjectTableSelectionData.selected = [];
    newAccessorTableSelectionData.selected = [];
    searchFolderShareRuleState.update( newSearchFolderShareRuleState );
    accessorTableSelectionData.update( newAccessorTableSelectionData );
    orgTreeSelectionData.update( newOrgTreeSelectionData );
    projectTableSelectionData.update( newProjectTableSelectionData );
};

export let processAddAccessorOutput = ( searchFolderShareRuleState, accessorTableSelectionData, selectionData ) => {
    const newSearchFolderShareRuleState = { ...searchFolderShareRuleState.getValue() };
    newSearchFolderShareRuleState.disableAddButton = true;
    newSearchFolderShareRuleState.disableRemoveButton = true;
    if( newSearchFolderShareRuleState.accessorsString && newSearchFolderShareRuleState.accessorsString.length > 0 ) {
        let newAccessorsString = getSearchFolderAccessorsString( selectionData.selected );
        newSearchFolderShareRuleState.accessorsString += ',' + newAccessorsString;
    } else {
        let accessorsString = getSearchFolderAccessorsString( selectionData.selected );
...
/**
 * @param {Object} response - the SOA response from performSearchViewModel
...
export let disableButton2 = ( buttonName, selectionData, searchFolderShareRuleState, selectionDataUpdater ) => {
    let selectedAccessors = selectionData.selected;
    let nonSiteSelectedAccessors = [];
    const newSearchFolderShareRuleState = { ...searchFolderShareRuleState };
    ...
    selectionDataUpdater.searchFolderShareRuleState( newSearchFolderShareRuleState );
};
