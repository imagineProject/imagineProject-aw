...
export const awFilterPanelRenderFunction = ( props ) => {
    ...
    let isCategorySearchStringValid = categorySearchString?.dbValue?.length && categorySearchString.dbValue.length > 0
        || groupAndCategorySearchString?.dbValue?.length && groupAndCategorySearchString.dbValue.length > 0;
    ...
    const numericRangeAction = ( category, startValue, endValue, wildcardOperator ) => {
        let categoryForRangeSearch = {
            category: category,
            startValue: startValue,
            endValue: endValue
        };
        if( numericRangeCallBack ) {
            numericRangeCallBack( categoryForRangeSearch, wildcardOperator );
        } else {
    ...
    const numericRangeFacetAction = ( category, startValue, endValue, wildcardOperator ) => {
        ...
        } else if ( numericRangeFacetCallBack ) {
            numericRangeFacetCallBack( combinedInputData.categoryForFacetSearchInput, combinedInputData.category, wildcardOperator );
        } else {
            dispatch( { path: 'data', value: { ...combinedInputData } } );
    ...
    const selectFilterCallBackAction = ( filter, category ) => {
        if( selectFilterCallBack ) {
            ...
            // if no call back, then the OOTB function will be called which will update the searchState.
            AwFilterPanelUtils.updateInputSearchFilterMap( filter, category, subPanelContext.searchState, true );
        }
    };

    const shiftSelectFilterCallBackAction = ( filter, category ) => {
        AwFilterPanelUtils.shiftSelectFilterAction( filter, category, subPanelContext.searchState );
    };
        ...
        if( categoriesToShowCount && categoriesToShowCount > 0 && index < categoriesToShowCount ||
            ...
            return (
                <AwFilterCategory
                    ...
                    shiftSelectFilterAction={shiftSelectFilterCallBackAction}
                    ...
                    isFilterByCategorySearchStringValid={isCategorySearchStringValid}
                    enableWildcardFiltering={enableWildcardFiltering}
                    enableWildcardFilteringIcon={enableWildcardFilteringIcon}
                    addWildcardCallBack={addWildcardCallBack}
                    numericFacetCallBack={numericFacetCallBack}
                    stringWildcardOperators={stringWildcardOperators}
                    numericWildcardOperators={numericWildcardOperators}
                    partitionWildcardOperators={partitionWildcardOperators}>
                </AwFilterCategory>
            );
        }
    };
