...
import { registerUserInfo } from 'js/AwContextControlService';
import clusterSearchService from 'js/SS1ClusterSearchService';
...
export let getSearchSortCriteria = function( sortCriteria, searchState, usePropertyNameOnly ) {
    ...
    // return sort criteria is already in the form ObjectType.PropertyName
    if( sortCriteria && sortCriteria.length === 1 && sortCriteria[ 0 ].fieldName && sortCriteria[ 0 ].fieldName.indexOf( '.' ) !== -1 ) {
        return sortCriteria;
    }
...
export let setColumnConfig = function( response ) {
    ...
    let nonIndexedPrefs = response.additionalSearchInfoMap && response.additionalSearchInfoMap.nonIndexedColumns ? response.additionalSearchInfoMap.nonIndexedColumns : null;
    if( nonIndexedPrefs ) {
        _.forEach( tempValue.columns, ( val )=>{
            let currentColumn = val.associatedTypeName + '.' + val.propertyName;
            if ( nonIndexedPrefs.indexOf( currentColumn ) > -1 ) {
                val.isDisableFacet = true;
            } else {
                val.isDisableFacet = false;
            }
        } );
    }
...
export const processOutput = ( data, dataCtxNode, searchData ) => {
    ...
    newSearchData.columnProvider = dataCtxNode?.columnProviders?.clientScopeUI;
    ...
    let highlightingAndColorToggleVals = awSearchService.getHighlightingAndColorToggleValues();
    newSearchData.highlighting = highlightingAndColorToggleVals.highlighting;
    newSearchData.colorToggle = highlightingAndColorToggleVals.colorToggle;
    newSearchData.searchColumnFilters = dataCtxNode && dataCtxNode.columnProviders && dataCtxNode.columnProviders.clientScopeUI &&
        dataCtxNode.columnProviders.clientScopeUI.columnFilters ? dataCtxNode.columnProviders.clientScopeUI.columnFilters : [];
    newSearchData.filterFacetCursor = new Map();
    searchData.update( newSearchData );

    //remove this code when arrange panel is refactored to not retrieving from ctx.searchResponseInfo
    let searchResponseInfoCtx = appCtxService.getCtx( 'searchResponseInfo' );
    if( !searchResponseInfoCtx ) {
        searchResponseInfoCtx = {
            searchFilterMap: newSearchData.searchFilterMap
        };
        appCtxService.registerCtx( 'searchResponseInfo', searchResponseInfoCtx );
    } else {
        searchResponseInfoCtx.searchFilterMap = newSearchData.searchFilterMap;
        appCtxService.updateCtx( 'searchResponseInfo', searchResponseInfoCtx );
    }
};

export const getColumnsForExport = ( columns, exportColumns ) => {
    let newColumns = [];
    for( const element of columns ) {
        for( const element2 of exportColumns ) {
            if( element.propertyName === element2.propertyName ) {
...
export const getHighlightingAndColorToggleValues = () => {
    let preferenceCtx = appCtxService.getCtx( 'preferences' );
    return {
        highlighting: preferenceCtx.AW_Highlighting === undefined || preferenceCtx.AW_Highlighting && preferenceCtx.AW_Highlighting.length > 0 && preferenceCtx.AW_Highlighting[0] === 'true',
        colorToggle: preferenceCtx.AWC_ColorFiltering === undefined || preferenceCtx.AWC_ColorFiltering && preferenceCtx.AWC_ColorFiltering.length > 0 && preferenceCtx.AWC_ColorFiltering[0] === 'true'
    };
};
    ...
    if( searchState.immutableSearchInfo && searchState.immutableSearchInfo.immutableSearchFilterMap &&
        ...
        for( const [ key, value ] of Object.entries( searchState.activeFilterMap ) ) {
            ...
            for( const element of value ) {
                values.add( JSON.stringify( element ) );
            }
            let arrayOfValues = values.size > 0 ? Array.from( values ) : [];
            for( const element of arrayOfValues ) {
                filterObjectArray.push( JSON.parse( element ) );
            }
...
export const processSearchCriteria = ( searchState ) => {
    let inputSearchCriteria = searchState.criteria;
    const isImageSearch = appCtxService.getCtx( 'imageFmsTicket' );
    if( isImageSearch ) {
        sessionStorage.setItem( 'lastUploadedImageFileTicket', isImageSearch );
    }
    if( searchState.isGlobalSearch === 'true' ) {
        let oldPrefilterValue = [];
        if( inputSearchCriteria ) {
            for( const [ key ] of Object.entries( inputSearchCriteria ) ) {
                if( key.endsWith( '_PREFILTER' ) ) {
                    oldPrefilterValue.push( key );
                }
            }
            for( let val in oldPrefilterValue ) {
                delete inputSearchCriteria[ oldPrefilterValue[ val ] ];
            }
        }
        ...
        let ownActiveFilter = ownerPrefilter ? searchState.activeFilters[ ownerPrefilter.filterInternalName ] : undefined;
        let catActiveFilter = categeryPrefilter ? searchState.activeFilters[ categeryPrefilter.filterInternalName ] : undefined;

        if( ownerPrefilter ) {
            let ownPrefilterValue = ownerPrefilter.selectedOwner;

            if( ownPrefilterValue === '$ME' ) {
                let userName = appCtxService.getCtx( 'user' ).props.user_name.dbValue;
                let userId = appCtxService.getCtx( 'user' ).props.userid.dbValue;
                ownPrefilterValue = userName + ' ( ' + userId + ' )';
            }
            if( ownActiveFilter && ownActiveFilter[0] === ownPrefilterValue ) {
                inputSearchCriteria[ ownerPrefilter.filterInternalName + '_PREFILTER'] = ownPrefilterValue;
            }
        }
        if( categeryPrefilter ) {
            let catPrefilterValue = categeryPrefilter.selectedCategory;
            if( catActiveFilter && catActiveFilter[0] === catPrefilterValue ) {
                inputSearchCriteria[ categeryPrefilter.filterInternalName + '_PREFILTER'] = categeryPrefilter.selectedCategory;
            }
        } else if( !categeryPrefilter && !catActiveFilter ) {
            let prefilterSources;
            let activeFilters = searchState.activeFilters;
            let filterKeys = Object.keys( activeFilters );
            for( let i = 0; i < filterKeys.length; i++ ) {
                let activeFilterKey = filterKeys[i];

                for( let index = 0; index < activeFilters[filterKeys[i]].length; index++ ) {
                    index === 0 ? prefilterSources = activeFilters[filterKeys[i]][index] : prefilterSources += ',' + activeFilters[filterKeys[i]][index];
                }
                inputSearchCriteria[ activeFilterKey + '_PREFILTER' ] = prefilterSources;
            }
        }
    }
    awSearchService.processNaturalLanguage( searchState, inputSearchCriteria );
    const lastUploadedImageFileTicket = sessionStorage.getItem( 'lastUploadedImageFileTicket' );
    if( isImageSearch || lastUploadedImageFileTicket ) {
        inputSearchCriteria.imageFMSTicket = isImageSearch ? isImageSearch : lastUploadedImageFileTicket;
    }
    return inputSearchCriteria;
};

export let processNaturalLanguage = ( searchState, inputSearchCriteria ) => {
    let NLPPref = appCtxService.getCtx( 'preferences.TC_AI_natural_language_search' );

    if ( NLPPref && NLPPref[0] === 'true' ) {
        if ( searchState.additionalSearchInfoMap && searchState.additionalSearchInfoMap.additionalInfoMessages ) {
            let jsonArrayMess = searchState.additionalSearchInfoMap.additionalInfoMessages[0]
                ? _.cloneDeep( searchState.additionalSearchInfoMap.additionalInfoMessages[0] )
                : {};

            if ( !_.isEmpty( jsonArrayMess ) ) {
                let structuredIntent = 'structuredIntent';
                inputSearchCriteria[structuredIntent] = jsonArrayMess;
            }
        }
    }
};
...
export const processSelectionOfFirstObject = ( response, selectFirstObjectPreference ) => {
    if( selectFirstObjectPreference && selectFirstObjectPreference.length > 0 ) {
        ...
        if( selectionPreferenceValue && selectionPreferenceValue === 'true' &&
              response.totalLoaded > 0 && response.cursor.startReached ||
              response.totalLoaded === 1 && response.cursor.endIndex === 1 ) {
            return true;
        }
    }
    return false;
...
/**
 * Initializes the revision rule.
 * Listens to 'cdm.modified' event and registers user information if any 'UserSession' or 'User' objects are modified.
 *
 * @function
 * @name initRevRule
 * @returns {void} Returns nothing
 */
export function initRevRule() {
    eventBus.subscribe( 'cdm.modified', function( eventData ) {
        const callRegisterUserIinfo = eventData.modifiedObjects.filter( i=>i.type === 'UserSession' || i.type === 'User' );
        if( callRegisterUserIinfo.length ) {
            registerUserInfo( appCtxService.ctx );
        }
    } );
}

export let updateColumnsWithOutliers = ( dataProvider ) => {
    var preferencesCtx = appCtxService.getCtx( 'preferences' );
    var workspaceCtx = appCtxService.getCtx( 'workspace' );
    if( workspaceCtx && workspaceCtx.workspaceId === 'PricingCompanionWorkspace' &&
        preferencesCtx && preferencesCtx.SS1_Mark_Outliers_Properties ) {
        shapeSearchService.markColumnsWithOutliers( dataProvider.columnConfig.columns, dataProvider.searchResults[0].uid );
    }
};

export const processAndUpdateReportColumns = ( dataProvider ) => {
    var workspaceCtx = appCtxService.getCtx( 'workspace' );
    var locationCtx = appCtxService.getCtx( 'locationContext' );
    var preferencesCtx = appCtxService.getCtx( 'preferences' );
    if( workspaceCtx && workspaceCtx.workspaceId === 'PricingCompanionWorkspace' && locationCtx['ActiveWorkspace:SubLocation'] === 'teamcenter.search.clustersearch' &&
        preferencesCtx.SS1_Identity_report_range_properties && dataProvider?.additionalSearchInfoMap?.identicalDataList ) {
        clusterSearchService.processAndUpdateReportColumns( dataProvider.columnConfig.columns, dataProvider.additionalSearchInfoMap.identicalDataList,
            preferencesCtx.SS1_Identity_report_range_properties );
    }
};

export const getDataProviderForSOA = ( ctx, searchState ) => {
    let dataprovider = '';
    if ( ctx?.imageFile || location.href.indexOf( 'imageURL' ) > -1 ) {
        dataprovider = 'Ais1ImageSearchProvider';
        appCtxService.updatePartialCtx( 'imageFile', '' );
    } else {
        dataprovider = searchState.provider;
    }
    return dataprovider;
};
...
const awSearchService = {
    ...
    processOutput,
    getHighlightingAndColorToggleValues,
    ...
    updateColumnsWithOutliers,
    processAndUpdateReportColumns,
    getDataProviderForSOA
};
