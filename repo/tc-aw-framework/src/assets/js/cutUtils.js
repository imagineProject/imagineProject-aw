...
let isSystemFolder = function( secondaryObject ) {
    let type = secondaryObject.type;
    return  type === 'Awp0FavoritesFolder' || type === 'Fnd0HomeFolder' || type === 'Awp0MySearchFolder' || type === 'Awp0ProjectDataFolder' || type === 'Awp0ProjectDataTemplates';
};

let isSystemFolderUnderRootFolder = function( relationInfo ) {
    return relationInfo.primaryObject.type === 'Awp0Folders' && isSystemFolder( relationInfo.secondaryObject );
};

let isValidObjectForCut = function( relationInfo ) {
    return  !isSystemFolderUnderRootFolder( relationInfo ) && relationInfo.primaryObject.type !== 'Awp0FavoritesFolder';
};
...
export let cutInputs = function( isConfRevContext ) {
    ...
    if( !isConfRevContext ) {
        for( var i = 0; i < appCtxService.ctx.relationContext.relationInfo.length; i++ ) {
            ...
            if ( isValidObjectForCut( relationInfo ) ) {
                inputData = {
        ...
        for( var j = 0; j < appCtxService.ctx.relationContext.relationInfo.length; j++ ) {
            ...
            if( isObjectAConfiguredRev( relationInfo.secondaryObject ) && isValidObjectForCut( relationInfo ) ) {
                inputData = {
                    parentObj: relationInfo.primaryObject,
                    childrenObj: [ relationInfo.secondaryObject ],
                    propertyName: relationInfo.relationType
