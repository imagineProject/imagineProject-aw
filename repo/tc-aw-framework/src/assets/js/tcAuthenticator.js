...
async function getSessionVars() {
    ...
    const { data, status } = await $http.get( `${browserUtils.getBaseURL()}getSessionVars?url=${browserUtils.getBaseURL()}` );
    if( status === 200 ) {
...
let sessionWebSocket = function() {
    // In hosting mode, the login session is passed from NX to AW, so auto logout function is not needed.
    // It will help the window.open use case to keep the session alive.
    // As long term fix, we need to have a debounce like mechanism at gateway to handle all window.open cases.

    // LCS-1010929 allow websocket creation for testfixture to avoid session logout when switched form AW to testfixture
    const urlAttrs = browserUtils.getWindowLocationAttributes();
    if ( urlAttrs.ah && urlAttrs.ah.toLowerCase() === 'true' && appCtxSvc.getCtx( 'aw_host_type' ) !== 'JS' ) {
        return;
    }
