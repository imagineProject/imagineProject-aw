...
/**
 * Gets value for prefernce AWV0AWAppConnectMode
 * @return {Promise} Resolved with true if we should be opening in hosted mode.
 */
function determineHostInVis() {
    var deferred = AwPromiseService.instance.defer();

    preferenceSvc.getStringValue( 'AWV0AWAppConnectMode' ).then( function( result ) {
        if( result !== null && result.length > 0 && result.toUpperCase() === 'OFF' ) {
 ...
 */
export var launchProduct = function( isReloadOperation ) {
...
export let downloadVVIFileTicketForProductSnapshotReload = function( data, isReloadOperation ) {
    let arrayContext = _.isArray( data ) ? data : [ data ];
    return determineHostInVis().then( function( isHostInVis ) {
        let isOpenInHost = determineOpenInHost();
        return createLaunchInfoRequest.launchObject( isOpenInHost, isHostInVis, arrayContext, isReloadOperation );
    } );
};
export default exports = {
    ...
    downloadVVIFileTicketForProductSnapshotReload
};
