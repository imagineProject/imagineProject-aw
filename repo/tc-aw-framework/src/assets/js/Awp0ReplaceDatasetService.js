...
export let getDatasetRefObj = function( selectedDataset ) {
    let refObjUid = selectedDataset.props.ref_list.dbValues[ 0 ];
...
export let createInputForDSM = function( selected, data ) {
    let datasetObj = cdm.getObject( selected.uid );
    let imanFileList = cdm.getObjects( datasetObj.props.ref_list.dbValues );
    let imanFile = null;
    let imanFileReference = '';
    if( imanFileList.length === 1 ) {
        imanFile = imanFileList[ 0 ];
        imanFileReference = datasetObj.props.ref_names.dbValues[ 0 ];
    } else if( data?.datasetFileName ) {
        for( var i = 0; i < imanFileList.length; i++ ) {
            let fileName = imanFileList[ i ].props.original_file_name.dbValues[ 0 ];
            if( fileName === data.datasetFileName.uiValue ) {
...
export let fileUploadForReplace = async function( formData ) {
    try {
...
export function cacheFileName( fileName ) {
    return fileName;
}

export const cleanUpReplaceData = ( uploadFileSelectionData ) => {
    addObjectUtils.resetUploadFileSelectionData( uploadFileSelectionData );
    addObjectUtils.resetFileUploadState();
};
