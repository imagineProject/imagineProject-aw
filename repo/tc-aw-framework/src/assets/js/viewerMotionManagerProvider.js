...
const MOVE_PARTS_FLIP_ENABLED = 'isFlipEnabled';
const MOVE_PARTS_FLIP_SELECTED = 'isFlipSelected';
...
const SNAP_TO_PART_OR_FEATURE = window.JSCom.Consts.ManipulatorIDs.snapToPartOrFeature_id;
const ALIGN_TO_AXIS = window.JSCom.Consts.ManipulatorIDs.alignToAxis_id;
const MOVE_PARTS_PICKING_MODE_ENABLED = 'arePickingModesEnabled';
const FLIP_COMMAND = window.JSCom.Consts.ManipulatorIDs.relocateEnableFlip;
const MANIPULATOR_POSITION = 'Manipulator_Position';

const MOVE_MANIPULATOR_POS_TYPES = [
    'PartCoordinateSystem',
    'PartBoundingBoxCenter',
...
class ViewerMotionManager {
    ...
    restoreExplodedViewSlider() {
        this.getExplodeViewPercent().then( function( explodePercent ) {
            if ( explodePercent > 0.0 ) {
                this.setExplodeViewEnabled( true );
                this.setExplodeViewPercent( explodePercent * 100.0 );
                this.viewerContextData.updateViewerAtomicData( viewerContextService.VIEWER_IS_EXPLODE_VIEW_VISIBLE, true );
    ...
    setManipulator() {
        if ( this.viewerView && this.viewerView.motionMgr ) {
            ...
            if( this.isManipulatorEnabled && this.viewerView && this.viewerView.sectionMgr ) {
                this.viewerContextData.getSectionManager().disableSectionSelectionWhenMoveManipulatorSelected().then( ()=> {
                    this.setOrUnsetManipulator();
                } );
            }else{
                this.setOrUnsetManipulator();
    ...
    setFlip() {
        if ( this.viewerView && this.viewerView.motionMgr ) {
            this.isFlipSelected = !this.isFlipSelected;
            this.viewerContextData.updateViewerAtomicData( MOVE_PARTS + '.' + MOVE_PARTS_FLIP_SELECTED, this.isFlipSelected );
            this.viewerView.motionMgr.showAttachedManipulator( this.isFlipSelected );
        }
    }

    setPickingModeToParts() {
        this.viewerContextData.updateViewerAtomicData( MOVE_PARTS + '.' + MOVE_PARTS_PICKING_MODE_ENABLED, false );
    ...
    /**
    * handler for escape keyboard listener for Move-align
    */
    handleEscInMoveAlign() {
        this.setFeatureAlignment();
        this.viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( null );
    }

    registerorUnregisterForJSComUIEvent( isRegister ) {
        this.enableOrDisablePickingMode = function( notificationObj ) {
            if ( notificationObj.id === SNAP_TO_PART_OR_FEATURE || notificationObj.id === ALIGN_TO_AXIS ) {
                if ( notificationObj.value === true ) {
                    this.viewerContextData.updateViewerAtomicData( MOVE_PARTS + '.' + MOVE_PARTS_PICKING_MODE_ENABLED, true );
                    this.setMovePartsPickMode();
                } else {
                    this.setPickingModeToParts();
                }
            } else if ( notificationObj.id === FLIP_COMMAND ) {
                this.viewerContextData.updateViewerAtomicData( MOVE_PARTS + '.' + MOVE_PARTS_FLIP_ENABLED, true );
            } else if( notificationObj.id === MANIPULATOR_POSITION ) {
                viewerPreferenceService.setMoveManipulatorPos( MOVE_MANIPULATOR_POS_TYPES[notificationObj.value], this.viewerContextData );
            }
        }.bind( this );
        if ( this.viewerView && this.viewerView.motionMgr ) {
            if ( isRegister ) {
                this.viewerView.motionMgr.registerSgoUIChangeEventNotification( this.enableOrDisablePickingMode );
            } else {
                this.viewerView.motionMgr.unregisterSgoUIChangeEventNotification( this.enableOrDisablePickingMode );
            }
        }
    }
    ...
    setOrUnsetManipulator() {
        if ( this.viewerView && this.viewerView.motionMgr ) {
            ...
            let movManipulatorPositionValue = viewerPreferenceService.getMoveManipulatorPos( this.viewerContextData );
            let movManipulatorPositionIndex = MOVE_MANIPULATOR_POS_TYPES.indexOf( movManipulatorPositionValue );
            this.viewerView.motionMgr.enablePartsManipulator( this.isManipulatorEnabled, movManipulatorPositionIndex ).then( function() {
                if( this.isManipulatorEnabled ) {
                    this.registerorUnregisterForJSComUIEvent( true );
                } else {
                    this.registerorUnregisterForJSComUIEvent( false );
                    this.setPickingModeToParts();
                }
            }.bind( this ) );
        }
    ...
    setOrUnsetFeatureAlign() {
        if ( this.viewerView && this.viewerView.motionMgr ) {
            ...
            this.viewerView.motionMgr.enableFeatureAlignment( this.isFeatureAlignEnabled ).then( function() {
                if( this.isFeatureAlignEnabled ) {
                    this.setMovePartsPickMode();
                    this.viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( this.handleEscInMoveAlign.bind( this ) );
                    this.registerorUnregisterForJSComUIEvent( true );
                } else {
                    let escapeKeyFncName = this.viewerContextData.getViewerKeyboardMgr().getEscapeKeyFunctionName();
                    if( !_.isNull( escapeKeyFncName ) && escapeKeyFncName.includes( 'handleEscInMoveAlign' ) ) {
                        this.viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( null );
                    }
                    this.setPickingModeToParts();
                    this.viewerContextData.updateViewerAtomicData( MOVE_PARTS + '.' + MOVE_PARTS_FLIP_ENABLED, false );
                    if( this.isFlipSelected ) {
                        this.setFlip();
                    }
                    this.registerorUnregisterForJSComUIEvent( false );
                }
                    ...
                    getResponse.then( function( info ) {
                        ...
                        setResponse.then( function( info ) {
                            ...
                            if ( error.cause.partialErrors ) {
                                soaSvc.post( 'Cad-2008-06-StructureManagement', 'saveBOMWindows', blWindowObject ).then( function( response ) {
                                    //commenting the message code for now, as we might need to revert back once we have a solution from BOM
                                    /*let finalMessage = '';
                                    ...
                                    messagingService.showInfo( i18n.somePartsSaved );
                                }, function( error ) {
                                    messagingService.showInfo( error.message );
                                } );
                            } else {
    ...
    /**
     * Get explode view percent
     *
     * @returns {Number} Exploded view percentage
     */
    getExplodeViewPercent() {
        if ( this.viewerView && this.viewerView.motionMgr ) {
            return this.viewerView.motionMgr.getExplodedViewPercent();
        }
        return 0.0;
    }
     ...
     *
     */
    resetExplodeView( explodedViewPercentReset ) {
        // we leave the exploded persentage in VisServer unchanged for Update case as the
        // VisServer could save a bookmark, causing wrong values being captured in the bookmark
        // for this case.
        if ( _.isUndefined( explodedViewPercentReset ) || _.isNull( explodedViewPercentReset ) || explodedViewPercentReset === true ) {
     ...
     */
    closeExplodeViewMode( explodedViewPercentReset ) {
        this.resetExplodeView( explodedViewPercentReset );
     ...
     */
    update( topic, data ) {
        if (  this.isExplodeViewActive && ( topic === viewerContextService.VIEWER_COMMAND_PANEL_LAUNCHED && data && data.commandId !==  'Awv0CaptureGallery' ||
            ...
            topic ===  viewerContextService.VIS_SECTION_PLANE_TRANSLATE_MODE &&  data === true )
        ) {
            // Special Handling for CLEANUP_3D_VIEWER.
            // Case: Viewer panel closes before the server saves the Session/Bookmark during cleanup.
            // The disableExplodedView function call resets the exploded view percentage to 0.0.
            // This value is captured in the Bookmark/Session causing incorrect results.
            // For all other cases the exploded view percentage should be reset to 0.0
            if ( topic === this.viewerContextData.CLEANUP_3D_VIEWER ) {
                this.disableExplodeView( false );
            } else {
                this.disableExplodeView( true );
            }
        }
        ...
        if ( topic === this.viewerContextData.CLEANUP_3D_VIEWER && this.viewerContextData.getValueOnViewerAtomicData( 'renderLocation' ) === 'CSR' || topic ===  viewerContextService.VIEWER_AREA_SELECT ) {
            this.isFreeDragEnabled = false;
            this.isManipulatorEnabled = false;
            this.isFeatureAlignEnabled = false;
            this.setOrUnsetFeatureAlign();
     ...
     */
    disableExplodeView( explodedViewPercentReset ) {
        this.closeExplodeViewMode( explodedViewPercentReset );
    ...
    closeMovePartsToolbar() {
        ...
        if ( this.viewerContextData ) {
            ...
            if ( viewerSubCommandCommand ) {
                let newViewerSubCommandCommand = {
