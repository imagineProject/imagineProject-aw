...
import AwCopyOptions2 from 'viewmodel/AwCopyOptions2ViewModel';
...
export const awXrtRenderFunction = ( { subPanelContext, viewModel, type, objectType, vmo, xrtState, reviseSaveAsInfo,
    activeState, editContextKey, disableDeepCopy, selectionData } ) => {
    ...
    if( data.xrtPages ) {
        ...
        if( showTabContainer ) {
            return <div className={`aw-xrt-summaryContainer${enableResizeClass}`}>
                ...
                <div className={scrollClassName}>
                    <div className={`sw-column aw-layout-wrap${enableResizeClass}`}>
                        <AwXrt2 type={typeIn} callback={reloadCurrentXRT} xrtData={data.xrtData}
                            subPanelContext={subPanelContext} xrtState={xrtState} selectionData={selectionData}
                            objectType={objectType} enableResizeCallback={enableResizeCallback} vmo={vmo}
                            activeState={activeState} objectsetInfo={data.extraInfo} xrtContext={context?.xrtContext}
                            editContextKey={editContextKeyIn}></AwXrt2>
                    </div>
                </div>
            </div>;
        }
        // Saveas and Revise
        if( xrtState.deepCopyDatas && ( typeIn === 'SAVEAS' || typeIn === 'REVISE' ) ) {
            if( disableDeepCopy ) {
                return <div className='aw-xrt-summaryContainer'>
                    <div className={scrollClassName}>
                        <div className='sw-column aw-layout-wrap'>
                            ...
                            <AwCopyOptions type={typeIn} xrtData={data.xrtData} xrtState={xrtState}></AwCopyOptions>
                        </div>
                    </div>
                </div>;
            }
                ...
                <div className={scrollClassName}>
                    <div className='sw-column aw-layout-wrap'>
                        <AwXrt2 type={typeIn} callback={reloadCurrentXRT} xrtData={data.xrtData}
                            ...
                            objectType={objectType} enableResizeCallback={enableResizeCallback}
                            activeState={activeState}></AwXrt2>
                        <AwCopyOptions2 type={typeIn} xrtData={data.xrtData} xrtState={xrtState}></AwCopyOptions2>
                    </div>
                </div>
            </div>;
        }
...
export const loadXrtData = async( selectedObject, activeTab, type, objectType, context = {}, reviseSaveAsInfo, isContributedTab ) => {
    ...
    let typeIn = type;
    let activePageId = activeTab || '';
    if( !typeIn ) {
        typeIn = 'SUMMARY';
    }
    if( typeof activeTab === 'object' ) {
        if( activeTab.page ) {
            activePageId = activeTab.page.titleKey ? activeTab.page.titleKey : activeTab.page.displayTitle;
        } else {
            activePageId = '';
        }
    }
    ...
    if ( context?.defaultToTypeXRTIfNoSublocationXRT || context?.pageContext?.sublocationState?.focusComponent === 'intermediary' ) {
    // This logic can be moved to XRT framework where if this flag is passed then check if sublocation XRT present or not
    // If not present then default to type XRT else return sublocation XRT. This will be useful for SWA where we need to show type XRT if no sublocation XRT present.
    // In that case we can simply pass location/sublocation info in SOA call and get the desired XRT based on flag.
        locationInfo = {
            provider: {
                name: undefined,
                nameToken: undefined
    ...
    const response = await xrtParserService.getDeclStyleSheet( typeIn, activePageId, selectedObject, objectType, locationInfo, xrtContext, reviseSaveAsInfo, isContributedTab );
    ...
    _.forEach( xrtData.dataProviders, function( dataProvider ) {
        if( dataProvider?.action && xrtData.actions[ dataProvider.action ]?.inputData ) {
            let searchInput = tcDataManagementService.getSearchInputFromActionInputData( xrtData.actions[ dataProvider.action ].inputData );
            if ( searchInput ) {
    ...
    // Remove pageNameToken type tabs from Stylesheet response under relationsTree flag.
    // SWA doesnt show pageNameToken type tabs.
    if( appCtxService.ctx?.state?.urlAttributes?.relationsTree ) {
        visibleXrtTabs = visibleXrtTabs.filter( tab => !tab?.page?.pageNameToken );
    }
    let headerProps =  _.get( xrtData, 'data._headers' ) || [];

    let uid = selectedObject ? selectedObject.uid : Object.keys( xrtData.data.objects )[ 0 ];
    let tabContentVMO = [ xrtUtils.prepareXrtViewModelObject( uid, xrtData, xrtData.data.operationName, xrtData.data.owningObjUid, selectedObject, true ) ];
    ...
    return { childTabs: visibleXrtTabs, activeTabContent: xrtData, extraInfo, headerProps : headerProps, tabContentVMO: tabContentVMO };
};

export const populateVisibleXrtTabsAndData = ( selectedObject, activePageId, type, objectType, context, reviseSaveAsInfo, editHandler ) => {
    ...
    if( editHandler && ( type === 'INFO' || type === 'SUMMARY' ) ) {
        return editHandler.leaveConfirmation().then( () => {
            return loadXrtData( selectedObject, activePageId, type, objectType, context, reviseSaveAsInfo );
        } );
    }
