...
var _getLanguageDisplayName = function( locale ) {
    let locationMessages = localeSvc.getLoadedText( 'LoginLocale' );
...
export let processLocaleProps = function( response, data ) {
    ...
    _.forEach( propValues, function( propVal ) {
        if( propVal.statuses[ 0 ] === data.masterTranslationStatus.status ) {
            ...
            if( data.subPanelContext.maxLength ) {
                uwPropertySvc.setLength( masterProp, data.subPanelContext.maxLength );
            }
            masterProp.index = 0;
            ...
            if( languageName ) {
                ...
                _.forEach( propVal.values, function( value, indx ) {
                    ...
                    if( data.subPanelContext.maxLength ) {
                        uwPropertySvc.setLength( viewModelProp, data.subPanelContext.maxLength );
                    }
