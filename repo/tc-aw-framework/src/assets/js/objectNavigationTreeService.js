...
export let loadTreeTableData = function( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, hideRootNode ) {
    assert( searchInput, 'Missing search input' );
    assert( treeLoadInput, 'Missing tree load input' );

    treeLoadInput.displayMode = 'Tree';

    return AwPromiseService.instance.resolve( _buildTreeTableStructure( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, hideRootNode ) );
};

export const selectHomeFolderAfterLoad = async function( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, selectionModel, hideRootNode ) {
    const response = await loadTreeTableData( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, hideRootNode );

    // We only want to do this if top level node is of type 'Awp0Folders'
    // If there is no current selection, or current selection is base 'Folders' object
    const treeLoadResult = response.treeLoadResult;
    const params = AwStateService.instance.params;
    if( treeLoadResult && treeLoadResult.newTopNode?.type === 'Awp0Folders' && ( !params.s_uid || params.s_uid === params.uid ) ) {
        // search through load result for initial load to find home folder and set selection
        for( const child of treeLoadResult.childNodes ) {
            if( child.type === 'Fnd0HomeFolder' ) {
                // set home folder as selected
                selectionModel.setSelection( [ child ] );
                break; // we found home folder, exit loop
            }
        }
    }
 ...
 */
function _buildTreeTableStructure( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, hideRootNode ) {
    var target = {};
    ...
    return getTableSummary( soaSearchInput, columnConfigInput, saveColumnConfigData ).then( function( response ) {
        ...
        let treeLoadResult = exports.createTreeLoadResult( response, treeLoadInput, vmNodes );

        if ( cdmRelatedModifiedInfo?.selectionModel && cdmRelatedModifiedInfo?.createdObjects ) {
            objectNavigationService.handleSelectionPostCreation( cdmRelatedModifiedInfo.selectionModel, cdmRelatedModifiedInfo, treeLoadResult.childNodes );
        }
...
export let removeAlternateIdFromRelatedModified = function( data ) {
    _.forEach( data.relatedModified, function( relatedModified ) {
        if( !treeDPReqRespHelper.isViewModelTreeNode( relatedModified ) ) {
            delete relatedModified.alternateID;
        }
    } );

    _.forEach( data.createdObjects, function( createdObject ) {
        if( !treeDPReqRespHelper.isViewModelTreeNode( createdObject ) ) {
            delete createdObject.alternateID;
        }
    } );

    _.forEach( data.childObjects, function( childObject ) {
        if( !treeDPReqRespHelper.isViewModelTreeNode( childObject ) ) {
...
export const handleSelectionForObjNavTree = ( objNavSelectionModel, isTreeMode ) => {
    ...
    // Since this method is called on mount/unmount, clear the cdmRelatedModified event info
    cdmRelatedModifiedInfo = {};
};

export const updateTreeSelectionInfo = ( { selectionModel, selected, rootNode, hideRootNode, initialVMOsFound } = {}, objNavState, baseSelection = {} ) => {
    if( selectionModel && selected && objNavState ) {
        ...
        if( selection.length === selected.length ) {
            let newState = { ...objNavState.value };
            // Deselect the only selected node in tree ; selection should switch to root node
            if( selected.length === 0 ) {
                if ( hideRootNode ) {
                    newState.totalFound = initialVMOsFound?.length;
                } else {
                    newState.totalFound = rootNode?.totalFound;
                }
            } else {
...
export const handleTreeNodeDelete = ( dataprovider, selectionModel ) => {
    const selection = selectionModel.getSelection();
    const selectionHierarchy = selection && selection[0] ? selection[0].split( ',' ).slice( 1 ).join( ',' ) : '';
    if ( selectionHierarchy.length > 0 ) {
        let newSelection = dataprovider.vmCollectionObj.vmCollection.loadedVMObjects.filter( ( obj ) => obj.alternateID === selectionHierarchy );
        setSelectionForModifiedObjects( selectionModel, newSelection );
    }
};
...
export const handleCdmEventForTree = ( eventData, dataprovider, selectionModel, { dropOnTreeNode, dispatch } = {} ) => {
    // Ensure eventData.relatedModified is an Array
    const relatedModified = Array.isArray( eventData.relatedModified ) ? eventData.relatedModified[ eventData.relatedModified.length - 1 ] : eventData.relatedModified;

    const isProjectFolderHierarchy = relatedModified?.modelType?.typeHierarchyArray?.indexOf( 'Awp0ProjectFolder' ) > -1;
    if( relatedModified?.modelType?.typeHierarchyArray?.indexOf( 'Folder' ) > -1 || isProjectFolderHierarchy ) {
        ...
        //like AutoExpandId, need to keep track of createdObjects for later selections.
        cdmRelatedModifiedInfo.isPinnedFlag = eventData.isPinnedFlag;
        cdmRelatedModifiedInfo.createdObjects = _.cloneDeep( eventData.createdObjects );
        cdmRelatedModifiedInfo.selectionModel = selectionModel;
        ...
        } else if( eventData.createdObjects && !eventData.isPinnedFlag && !isProjectFolderHierarchy && !eventData.skipSelectingNewObjects ) {
        ...
        if ( !eventData.refreshLocationFlag ) {
