...
export const processOutput = ( data, dataCtxNode, objNavState = {}, subPanelContext, enableAccScroll ) => {
    const newObjNavState = { ...objNavState.getValue() };
    ...
    if( subPanelContext && subPanelContext.selectionModel.getSelection() ) {
        ...
        if( selectionsInSM.length === 1 && data.cursor.startIndex === 0 ) { // requried only for first page
            ...
            let matched = [ ...existingSearchResults, ...data.searchResults ].filter( obj => {
                return selectionsInSM[ 0 ] === obj.uid;
            } );
            if( enableAccScroll && matched.length === 0  ) {
                matched = dataCtxNode.dataProviders.gridDataProvider.viewModelCollection.loadedVMObjects.filter( obj => {
                    return selectionsInSM[ 0 ] === obj.uid;
                } );
            }
    ...
    if( objNavState.defaultBaseSelection !== undefined && objNavState.baseSelection === undefined ) {
        ...
        selectionService.updateSelection( objNavState.defaultBaseSelection, objNavState.defaultBaseSelection );
        objNavState.update && objNavState.update( newObjNavState );
    }
};

//Uids are not references to the actual object
let getRealUid = function( uid ) {
...
export const isTreeMode = () => {
    return exports.isTreeViewMode( viewModeService.getViewMode() );
};

export const sortResults = function( parentUid, searchResults ) {
...
export const loadData = function( searchInput, columnConfigInput, saveColumnConfigData, inflateProperties = true, enableAccScroll ) {
    if ( enableAccScroll && typeof searchInput.startIndex === 'number' ) {
        if ( searchInput.pagingType === 'GetPreviousPage' && searchInput.startIndex === 0 ) {
            searchInput.cursor.startIndex = searchInput.startIndex + searchInput.maxToReturn;
        } else if ( searchInput.pagingType === 'GetNextPage' ) {
            searchInput.cursor.endIndex = searchInput.startIndex - 1;
        }
    }
...
export const getSearchContext = function( searchContext = {}, changedParams, baseSelection, selectionModel ) {
    ...
    if( currentSelection[ 0 ] !== changedParams.s_uid ) {
        ...
        } else if( !changedParams.s_uid && !multipleSelections ) {
    ...
    if( localSubPanelContext.searchContext !== null ) {
        ...
        return getSearchContext( localSubPanelContext.searchContext, changedParams, baseSelection, selectionModel ).then( function( updatedSearchContext ) {
            ...
            if( hasContextChanged && isResetRequired ) {
                ...
                let objNavState = objNavStateAtomicDataRef.getAtomicData();
                let pwaSelection = !isTreeMode() && objNavState.value?.pwaSelection;
                ...
                if( !objNavState.baseSelection || objNavState.baseSelection.uid !== newBaseSelection.uid ) {
                    objNavStateUpdater.objNavState( { ...objNavState, pwaSelection: pwaSelection, baseSelection: newBaseSelection } );
...
export const handleSelectionPostCreation = ( selectionModel, cdmRelatedModifiedInfo, loadedVMOs ) => {
    // Initialize showConfiguredRev as a boolean
    let showConfiguredRev = true;
    if( cdmRelatedModifiedInfo?.createdObjects?.length > 0 && loadedVMOs?.length > 0 && !cdmRelatedModifiedInfo.isPinnedFlag ) {
        // Check if the preference exists and has a valid value
        if ( appCtxService.ctx.preferences.AWC_display_configured_revs_for_pwa &&
            appCtxService.ctx.preferences.AWC_display_configured_revs_for_pwa.length > 0 ) {
            showConfiguredRev = appCtxService.ctx.preferences.AWC_display_configured_revs_for_pwa[0] === 'true';
        }
        // Use the boolean value to determine whether to call setSelection
        if ( showConfiguredRev ) {
            // Fetch the evaluated objects
            const evaluatedObjs = getEvaluatedObjsForSelection( cdmRelatedModifiedInfo.createdObjects, loadedVMOs ).evaluatedObjs;
            if( evaluatedObjs.length > 0 && !_.isEqual( evaluatedObjs, cdmRelatedModifiedInfo.createdObjects ) ) {
                selectionModel.setSelection( evaluatedObjs );
            }
        }
    }
};
...
export const handleCdmRelatedModifiedEvent = ( eventData, baseSelection, dataprovider, selectionModel ) => {
    const matches = eventData.relatedModified?.length && eventData.relatedModified.filter( function( mo ) {
        return mo.uid === baseSelection.uid;
    } );
    if( !eventData.refreshLocationFlag && matches?.length ) {
        if ( eventData.createdObjects && dataprovider._enableAccScroll ) {
            dataprovider.pageDispatcher( { type: 'FOCUS_PAGE' } );
