...
/**
 * If Workflow/UOM_TAG is unchanged and empty, we don't want to include it in the create input.
 * This only happens in the special-case where the value starts empty, the user selects a value, and then reverts it back to empty.
 * Alternatively, the downstream client code could better handle this case,
 * but at present we end up with a default wf process template when wf is set to "".
 * Further, there is technical debt in the cfx that could be improved. An lov that is reverted to its original state
 * should probably not be considered modified. See declViewModelService::dirtyCheck() and uwPropertyService::isModified().
 **/
const skipSpecialPropsFromCreateInput = function( vmProp ) {
    const skipPropsFromCreateInput = [ 'awp0ProcessTemplates', 'uom_tag' ];
    const validToOmit = skipPropsFromCreateInput.some( prop => vmProp.propertyName.endsWith( prop ) );
    return validToOmit && !vmProp.value && !vmProp.newValue;
};
    ...
    if( editHandler ) {
        ...
        if( dataSource ) {
            ...
            _.forEach( allEditableProperties, function( vmProp ) {
                // If a prop isModified it will be included in the createInput.
                // But consider special-case for awp0ProcessTemplates so it is also added anytime it has a value,
                // even if not modified (default value case), but not if it's been reverted to empty.
                // This is tech debt and would be better solved by configuring the Workflow prop to always submit
                // when it has a default value or change the server to consider the default create inputs from bmide.
                var propValues = _uwPropSrv.getValueStrings( vmProp );
                var isDCPRefProp = vmProp.propertyName.split( '.' )[0].startsWith( 'REF' );
                if( vmProp && !skipSpecialPropsFromCreateInput( vmProp ) && ( vmProp.isAutoAssignable || isDCPRefProp || _uwPropSrv.isModified( vmProp ) || vmProp.propertyName.endsWith( 'awp0ProcessTemplates' ) && propValues && propValues.length > 0 && propValues[0].length > 0 ) ) {
                    _processPropertyForCreateInput( vmProp.propertyName, vmProp, createInputMap );
                }
            } );

            // this seems unnecessary, as allEditableProperties should include all modified properties above?
            let modifiedViewModelProperties = dataSource.getAllModifiedProperties();
            _.forEach( modifiedViewModelProperties, function( vmProp ) {
                if( vmProp && !skipSpecialPropsFromCreateInput( vmProp ) && ( vmProp.isAutoAssignable || _uwPropSrv.isModified( vmProp ) ) ) {
                    _processPropertyForCreateInput( vmProp.propertyName, vmProp, createInputMap );
                }
            } );
        }
                    ...
                    if( !isValidType ) {
                        ...
                        if( itemRevPastedOnTarget ) {
                            ...
                            if( createdItemObj && createdItemObj.props && createdItemObj.props.revision_list ) {
                                ...
                                const itemObjectObject = response.output[ index ].objects.filter( i=>i.modelType.typeHierarchyArray.indexOf( 'ItemRevision' ) !== -1 );
                                if( itemObjectObject.length ) {
                                    newObject = itemObjectObject[0];
                                }
                            }
                        }
                    }
...
export let getCreatedObjectsForAddACopy = function( response ) {
    return getCreatedObjects( response );
};
...
export let getSelectedFileInfo = ( data, addPanelState ) => {
    if( addPanelState ) {
        let newAddPanelState = { ...addPanelState.getValue() };
        newAddPanelState.dataToBeRelated = {
            attachFiles: data.eventData.fileName
        };
        newAddPanelState.fileInputForms = data.eventData.formData;
...
export const datasetChangeAction = function( datasetInfo, addPanelState, datasetState, fileUploadState ) {
    let filePickerInfoData = {
        fileName:  datasetInfo.fileName && _.isArray( datasetInfo.fileName ) &&
        datasetInfo.fileName.length === 1  ? datasetInfo.fileName[ 0 ] : datasetInfo.fileName,
        formData: getFormDataOfDatasetInfo( datasetInfo ),
        fileNameNoExt:  datasetInfo.fileNameNoExt && _.isArray( datasetInfo.fileNameNoExt ) &&
        datasetInfo.fileNameNoExt.length === 1  ? datasetInfo.fileNameNoExt[ 0 ] : datasetInfo.fileNameNoExt,
        fileExt: datasetInfo.fileExt,
        validFile: datasetInfo.validFile,
        totalFiles:  datasetInfo.fileName && _.isArray( datasetInfo.fileName ) ? datasetInfo.fileName.length : 0
    };
    ...
    if( datasetState ) {
        ...
        newDatasetState.files = _.isArray( datasetInfo.fileName ) ? datasetInfo.fileName.map( ( fileName ) =>  { return { fileName }; } ) : [ { fileName: datasetInfo.fileName } ];
...
export const getFormDataOfDatasetInfo = ( datasetInfo ) => {
    let formData = new FormData();
    if( datasetInfo.formData && _.isArray( datasetInfo.formData ) ) {
        for( let i = 0; i < datasetInfo.formData.length; i++ ) {
            formData.append(  datasetInfo.formData.length === 1  ? 'fmsFile' : i, datasetInfo.formData[i].file );
        }
        return formData;
    }
    return datasetInfo.formData;
};

export const getDatasetTypesWithDefaultRelationOutput = ( response ) => {
    return response;
};

export const getDatasetTypesWithDefaultRelationOutputList = ( datasetTypesWithDefaultRelationResponse, data ) => {
    let datasetTypesWithDefaultRelationOutput = datasetTypesWithDefaultRelationResponse.output;
    let multipleFilesDatasetTypeMap = new Map();
    if( datasetTypesWithDefaultRelationOutput && _.isArray( datasetTypesWithDefaultRelationOutput ) ) {
        datasetTypesWithDefaultRelationOutput.forEach( defaultRelationOutput => {
            const inputDefaultRelationOutput = { output: [ defaultRelationOutput ] };
            const datasetTypeList = getDatasetTypesFromTypesWithRelInfo( inputDefaultRelationOutput, data );
            multipleFilesDatasetTypeMap.set( defaultRelationOutput.fileExtension, {
                datasetTypesWithDefaultRelInfo: defaultRelationOutput.datasetTypesWithDefaultRelInfo,
                fileFormat: defaultRelationOutput.datasetTypesWithDefaultRelInfo[0].refInfos[0].fileFormat,
                datasetTypeList: datasetTypeList
            } );
        } );
    }
    return {
        multipleFilesDatasetTypeMap: multipleFilesDatasetTypeMap
    };
};

export let multipleFilesSetDatasetProperties = async function( data, multipleFilesDatasetTypeMap, datasetInfo, addPanelState, isCustomDatasetAction ) {
    let datasetPropertiesList = [];
    let typeAndReferenceResponseArray = [];
    let datasetTypeArr = [];
    for( let i = 0; i < datasetInfo.fileName.length; i++ ) {
        const fileExt = datasetInfo.fileExt[i];
        const dsType = multipleFilesDatasetTypeMap.get( fileExt ).datasetTypeList;
        const dsTypesWithDefaultRelInfo = multipleFilesDatasetTypeMap.get( fileExt ).datasetTypesWithDefaultRelInfo;

        const dsName = _.clone( data.datasetName );
        dsName.dbValue = datasetInfo.fileNameNoExt[i];

        const dsDesc = _.clone( data.datasetDesc );
        dsDesc.dbValue = null;

        const newDatasetType = _.clone( data.datasetType );
        newDatasetType.dbValue = dsType[0].propInternalValue;
        newDatasetType.uiValue = dsType[0].propDisplayValue;
        ...
        typeAndReferenceResponseArray.push( exports.updateTypeAndReferences( newDatasetType, dsTypesWithDefaultRelInfo,
            data.reference, dsName, dsDesc ) );
    }
    const typeAndReferenceResponseList = await Promise.all( typeAndReferenceResponseArray );
    _.forEach( typeAndReferenceResponseList, function( typeAndReferenceResponse, index ) {
        typeAndReferenceResponse.datasetType = datasetTypeArr[ index ].newDatasetType;
        typeAndReferenceResponse.datasetTypeList = datasetTypeArr[ index ].dsType;
        datasetPropertiesList.push( typeAndReferenceResponse );
    } );
    if( addPanelState && isCustomDatasetAction ) {
        let newAddPanelState = { ...addPanelState.getValue() };
        if( datasetPropertiesList.length > 1 ) {
            newAddPanelState.datasetVMO = datasetPropertiesList;
            newAddPanelState.formData = datasetInfo.formData;
            newAddPanelState.fileExtension = datasetInfo.fileExt;
        } else {
            newAddPanelState.datasetTypeList = datasetPropertiesList[0].datasetTypeList;
            newAddPanelState.references = datasetPropertiesList[0].references;
            newAddPanelState.datasetVMO.props.datasetType = datasetPropertiesList[0].datasetType;
            newAddPanelState.datasetVMO.props.datasetName = datasetPropertiesList[0].datasetName;
        }
        addPanelState.update( newAddPanelState );
    }
...
export const updateDatasetCreationTypeList = function( datasetPropertiesList, addPanelState ) {
    let newAddPanelState = { ...addPanelState.getValue() };
    newAddPanelState.creationType = datasetPropertiesList[0].creationType;
    addPanelState.update( newAddPanelState );
    return datasetPropertiesList[0].creationType;
};

export const getDefaultRelationForMultipleFiles = async function( primaryType, datasetPropertiesList ) {
    const defaultRelationInputList = [];
    _.forEach( datasetPropertiesList, property => {
        defaultRelationInputList.push( {
            primaryType: primaryType,
            secondaryType: property.creationType.props.datasettype_name.dbValues[0]
        } );
    } );
    const inputRequest = {
        input: defaultRelationInputList
    };
    let defaultRelationsResponse = await _soaService.postUnchecked( 'Internal-AWS2-2016-12-DataManagement',
        'getDefaultRelation', inputRequest );
    return {
        defaultRelationsList: defaultRelationsResponse.output
    };
};

export const updateRelationListForMultipleFiles = async function( datasetPropertiesList, relationMap, targetObject, typeName,
    defaultRelationsList, creationRelation, objectSetSource, datasetInfo, addPanelState ) {
    let relationListForMultipleFiles = [];
    let relationResponseArr = [];
    for( let i = 0; i < datasetPropertiesList.length; i++ ) {
        let invalidObjectInfo = {
            fileName: datasetInfo.fileName[i],
            objectSetSource: objectSetSource
        };
        relationResponseArr.push( updateRelationList( datasetPropertiesList[i].creationType, relationMap, targetObject, typeName,
            defaultRelationsList[i].defaultRelation, creationRelation, invalidObjectInfo = null, addPanelState ) );
    }
    relationListForMultipleFiles = await Promise.all( relationResponseArr );
    return {
        relationListForMultipleFiles: relationListForMultipleFiles,
        relationList: relationListForMultipleFiles[0].relationList,
        creationRelation: relationListForMultipleFiles[0].creationRelation
    };
};

export const updateItemIdAsDefaultDatasetNameForMultipleFiles = async function( eventData, datasetName, datasetInfo, addObjectContext ) {
    let datasetInfoIn = datasetInfo;
    let response = {};
    if( !datasetInfoIn ) {
        datasetInfoIn = eventData;
    ...
    if( useItemIdAsDefaultDatasetName && useItemIdAsDefaultDatasetName.length === 1 && useItemIdAsDefaultDatasetName[0] === 'true' ) {
        ...
        if( addObjectContext && addObjectContext.targetObject && addObjectContext.targetObject.modelType
            ...
            itemIdAndRevision = addObjectContext.targetObject.props.item_id.dbValues[ 0 ] + '/' + addObjectContext.targetObject.props.item_revision_id.dbValues[ 0 ];
        }
        datasetInfoIn.fileNameNoExt = [];
        for( let i = 0; i < datasetInfo.fileName.length; i++ ) {
            datasetInfoIn.fileNameNoExt.push( itemIdAndRevision );
        }
    }
    const newDatasetName = _.clone( datasetName );
    newDatasetName.dbValue = datasetInfoIn.fileNameNoExt;
    response.datasetName = newDatasetName;
    response.datasetInfo = datasetInfoIn;
    return response;
};

export const updateDatasetInfo = function( datasetInfo, addPanelState, datasetState ) {
 ...
 */
export const initDSCreateParams = async function( data, datasetInfo, addPanelState, isCustomDatasetAction, addObjectContext ) {
    let datasetInfoIn = datasetInfo;
    let response = {};
    ...
    if( addPanelState && isCustomDatasetAction ) {
        ...
        newAddPanelState.fileExtension = datasetInfoIn.fileExt;
 ...
 */
export const getDatasetTypeFromTypeInfo = function( response ) {
    var datasetTypeList = [];
 ...
 * @returns {ObjectArray} Array of list model objects
 */
export const getReferences = function( response ) {
 ...
 */
export const getFileExtension = function( response ) {
    var refInfos = response.infos[ 0 ].refInfos;
    var tempRefName;
    var finalFileExts;
 ...
 * @param {Object} data - The view model data
 */
export const updateTypeAndReferences = async function( datasetType, datasetTypesWithDefaultRelInfo, reference, datasetName, datasetDesc, creationTypeIn, referencesList,
    addPanelState, isCustomDatasetAction ) {
    if( isCustomDatasetAction && addPanelState ) {
        let currentAddPanelState = { ...addPanelState.getValue() };
        let currentPanelStateDatasetVMOProps = currentAddPanelState?.datasetVMO?.props;
        if( currentPanelStateDatasetVMOProps ) {
            datasetName = currentPanelStateDatasetVMOProps.datasetName;
            datasetDesc = currentPanelStateDatasetVMOProps.datasetDesc;
            datasetType = currentPanelStateDatasetVMOProps.datasetType;
            reference = currentPanelStateDatasetVMOProps.reference;
        }
    }
    // Update creationType
    ...
    if( isCustomDatasetAction && addPanelState ) {
        let newReference2 = _.clone( reference );
        newReference2.dbValue = newReference.dbValue;
        newReference2.uiValue = newReference.uiValue;
        let newAddPanelState = { ...addPanelState.getValue() };
        newAddPanelState.references = references;
        newAddPanelState.datasetVMO.props.reference = newReference2;
        addPanelState.update( newAddPanelState );
    }
...
const getAllRelations = function( typeName, targetObject ) {
    return _soaService.postUnchecked( 'Core-2014-10-DataManagement', 'getPasteRelations2', {
        ...
        .then( response => response.outputs.length > 0 && response.outputs[ 0 ].pasteRelInfo ? response.outputs[ 0 ].pasteRelInfo.map( x => x.pastRelName ) : [] );
};
 ...
 */
export const updateRelationList = function( creationType, relationMap, targetObject, typeName,
...
export const updateTargetObject = function( data ) {
...
export const uploadFilePreinit = function( _uid, _ticket, _filename ) {
    var deferred = AwPromiseService.instance.defer();
...
export let commonAddObjectContext =  function( panelContext ) {
    var targetObjectVMO = panelContext?.targetObject ? panelContext.targetObject : appCtxService.getCtx( 'selected' );
...
export let populateAddObjectContext = async function( panelContext ) {
    ...
    if( addObjectContext.targetObject?.type === PSEUDO_FOLDER ) {
        const targetObjectResponse = xrtObjectSetSvc.getTargetObjectForPseudoFolder( addObjectContext.targetObject );
        addObjectContext.targetObject = targetObjectResponse.targetObjectForPseudoFolder;
        addObjectContext.relationMap = updateRelationMap( addObjectContext.relationMap, targetObjectResponse.relationType );
    }
 ...
 */
export const getDatasetFileExtensions = function( { datasetTypeNames } ) {
    ...
    return datasetTypeNames.includes( 'Dataset' ) ? AwPromiseService.instance.resolve( '' ) : _soaService.postUnchecked( 'Core-2007-06-DataManagement', 'getDatasetTypeInfo', {
        ...
        const validExtensions = response.infos.reduce( ( extensions, { refInfos } ) => {
            const nxtExts = refInfos
                .filter( x => x.fileFormat === 'BINARY' || x.fileFormat === 'TEXT' || x.fileFormat === 'OBJECT' )
                .map( x => x.fileExtension === '*' ? '*' : x.fileExtension.slice( 1 ) ); //*.xlsx -> .xlsx or * -> *
            // If any extension is '*', return only '*'
            if ( nxtExts.includes( '*' ) ) {
                return [ '*' ];
            }
            return [ ...extensions, ...nxtExts ];
        }, [] );
        return _.uniq( validExtensions ).join( ',' );
    } );
};
...
export const onTypeSelected = function( context, addPanelState, datasetState, uploadFileSelectionData ) {
    ...
    if( creationTypeIn ) {
        ...
        const creationModelType = _cmm.getType( response.creationType.uid );
        response.isDatasetCreate = response.creationType && ( creationModelType?.typeHierarchyArray?.indexOf( 'Dataset' ) > -1 || response.creationType.uid.indexOf( 'Dataset' ) > -1 );
...
export const loadAddPanelTabs = function( visibleTabs, data, addPanelState ) {
    const tabChangeCallback = ( pageId, tabTitle ) => {
        ...
        if( addPanelState ) {
            ...
            newAddPanelState.isAddACopy = false;
            if( selectedTab ) {
    ...
    if( defaultTab ) {
        if( addPanelState ) {
            let newAddPanelState = { ...addPanelState.getValue() };
            newAddPanelState.selectedTab = { ...defaultTab };
            addPanelState.update( newAddPanelState );
        }
        return {
...
export const cacheSelection = ( selectionData, pwaSelection, baseSelection ) => {
    let selectedUid;
    if( selectionData?.selected?.length === 1 ) {
        selectedUid = selectionData.selected[ 0 ].uid;
        if( selectionData.source === 'secondary' ) {
            selectedUid = selectionData.pselected.uid;
        }
    } else if ( pwaSelection?.length > 0 ) {
        selectedUid = pwaSelection[ 0 ].uid;
    } else if ( baseSelection ) {
        selectedUid = baseSelection.uid;
    }
    return selectedUid;
};
export const closePanel = ( selectionData, pwaSelection, baseSelection, initialSelection, closeDialogOptions ) => {
    let selectedUid = cacheSelection( selectionData, pwaSelection, baseSelection );
    if( initialSelection && initialSelection !== selectedUid ||
        ...
        if ( closeDialogOptions?.open ) {
            dialogService.closeDialog( '', closeDialogOptions.popupId );
            closeDialogOptions.open = false;
        }
    }
};
...
export const cleanupNewTabSubpage = ( name, policyId ) => {
    appCtxService.unRegisterCtx( name );
    propPolicySvc.unregister( policyId );
};
...
export let autoAssignFileInfo = function( fileName, files, creationType, editHandler, autoAssignProps ) {
    let fileSelectionData = {};
    if( fileName ) {
        let viewModelProps = [ { propertyName: 'object_name', dbValue: fileName } ];
        if( autoAssignProps && autoAssignProps.length > 0 ) {
            viewModelProps = viewModelProps.concat( autoAssignProps );
        }
        assignInitialValues( viewModelProps, creationType, editHandler );
    }
    if( files ) {
        const sourceObjects = files[0].sourceObjects;
        let fileInfo = new FormData();
        let fileNames = [];
        let fileExts = [];
        let fileNameNoExts = [];
        _.forEach( sourceObjects, function( sourceObject ) {
            fileInfo.append( sourceObject.name, sourceObject, sourceObject.name );
            fileNames.push( sourceObject.name );
            fileExts.push( fileSvc.getFileExtension( sourceObject.name ) );
            fileNameNoExts.push( fileSvc.getFileNameWithoutExtension( sourceObject.name ) );
        } );
        fileSelectionData.value = {
            fileNames: fileNames,
            fileNameNoExts: fileNameNoExts,
            fileExts: fileExts,
            formData: fileInfo,
            files: fileInfo
        };
...
export const getXRTSubpanelContext = ( vmDef, props, data ) => {
    return [ new DerivedStateResult( {
        ctxParameters: [],
        additionalParameters: [ props.subPanelContext, data.fileInfo ],
        compute: () => {
            return {
...
export const onRelationSelected = ( addPanelState, relationListSelectionData ) => {
    if( relationListSelectionData && relationListSelectionData.selected && relationListSelectionData.selected.length > 0 ) {
        ...
        // update atomic data
        if( addPanelState && addPanelState.getValue().selectedRelation !== selectedRelation ) {
            let newAddPanelState = { ...addPanelState.getValue() };
...
export const onDatasetTypeSelected = ( creationType, datasetTypeSelectionData ) => {
    if( datasetTypeSelectionData?.selected && datasetTypeSelectionData.selected.length > 0 ) {
        let selectedDatasetType = datasetTypeSelectionData.selected[0].propInternalValue;

        if( selectedDatasetType !== creationType ) {
            return selectedDatasetType;
        }
    }
    return creationType;
};
...
export const setInitialValuesForFile = ( hostedFileContext, uploadFileSelectionData, uploadFileProvider ) => {
    const response = {};
    const hostedFileName = hostedFileContext?.filename;
    const hostedFileNameNoExt = fileSvc.getFileNameWithoutExtension( hostedFileName );
    let hostedFileExt = fileSvc.getFileExtension( hostedFileName );
    if ( hostedFileExt !== '' ) {
        hostedFileExt = _.replace( hostedFileExt, '.', '' );
    }

    const fileInfo = {
        fileNames: [ hostedFileName ],
        fileName: hostedFileName,
        fileNameNoExt: hostedFileNameNoExt,
        fileExt: hostedFileExt,
        name: hostedFileName,
        validFile: true
    };

    let newUploadFileSelectionData = { ...uploadFileSelectionData.getValue() };
    newUploadFileSelectionData.files = [ fileInfo ];
    newUploadFileSelectionData.modelInfo = uploadFileProvider[0];
    response.uploadFileSelectionData = newUploadFileSelectionData;
    response.filePickerInfoData = fileInfo;

    return response;
};

export let getFileExtensionsFromObjectSetSource = async( objectSetSource ) => {
    let fileExtensions = '';
    if( objectSetSource ) {
        const objectTypesSet = getObjectTypesFromObjectSetSource( objectSetSource );
        const objectSetArray = Array.from( objectTypesSet );
        if( objectSetArray && objectSetArray.length > 0 ) {
            fileExtensions = await getDatasetFileExtensions( { datasetTypeNames: objectSetArray } );
        }
    }
    return fileExtensions;
};

export let getObjectTypesFromObjectSetSource = ( objectSetSource ) => {
    const isDCP = ( src ) => {
        return src.search( /\(/ );
    };
    return objectSetSource.split( ',' ).reduce( ( prev, curr ) => {
...
export const isTypeUnSelected = ( data, addPanelState, creationType ) => {
    let updatedCreationType = creationType;
    if( data.selected.length === 0 && addPanelState ) {
        updatedCreationType = '';
        let newAddPanelState = { ...addPanelState.getValue() };
        newAddPanelState.creationType = updatedCreationType;
        addPanelState.update( newAddPanelState );
    }
    return updatedCreationType;
};

export const cleanupAddObject = ( type, path ) => {
    editHandlerService.removeEditHandler( type );
    appCtxService.unRegisterCtx( path );
};

export const updateRelationMap = ( relationMap, relationName ) => {
    let relMap = {};
    for( let i = 0; i < Object.keys( relationMap ).length; ++i ) {
        let typeName = Object.keys( relationMap )[ i ];
        relMap[ typeName ] = [ relationName ];
    }
    return relMap;
};

export const updateReferences = () => {
    // adding this empty action as without this, the references Lov keeps loading because the AwLovValUnifiedService is not rendered when the Type Lov is modified.
    // This is needed for custom dataset action.
};

localeService.getLocalizedTextFromKey( 'UIElementsMessages.failureToAttachFiles' ).then(
...
export default exports = {
    ...
    getProjects,
    getFileFormat,
    updateItemIdAsDefaultDatasetNameForMultipleFiles,
    initDSCreateParams,
    updateDatasetInfo,
    ...
    getFormDataOfDatasetInfo,
    getDatasetTypesWithDefaultRelationOutput,
    getDatasetTypesWithDefaultRelationOutputList,
    multipleFilesSetDatasetProperties,
    updateDatasetCreationTypeList,
    getDefaultRelationForMultipleFiles,
    updateRelationListForMultipleFiles,
    getReferences,
    getFileExtension,
