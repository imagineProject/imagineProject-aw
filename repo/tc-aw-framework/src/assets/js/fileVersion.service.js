...
import saveEditUtils from 'js/saveEditUtils';
 ...
 */
export const getColumns = async function( {
    dataProvider,
    columnsToShow = defaultColumnsToShow
} ) {
    await soa.ensureModelTypesLoaded( [ 'Dataset' ] );
...
export const getFileVersions = function( {
    ...
    return soa.postUnchecked( 'Core-2006-03-DataManagement', 'getProperties', {
        ...
        if ( updatedVmo.props?.revisions_prop?.dbValues?.length > 0 ) {
            searchResults = orderBy(
                updatedVmo.props.revisions_prop.dbValues.map( uid => tcViewModelObjectService.createViewModelObjectById( uid ) ),
    ...
    return handleActiveEdits( target )
        .then( () => {
            return soa.post( 'Internal-AWS2-2017-12-DataManagement', 'loadViewModelForEditing2', {
                ...
                return saveEditUtils.saveEdits(
                    [ {
...
export const setDatasetVMOFromSelection = function( selectionData, popupId ) {
    if( selectionData?.selected?.length !== 1 || selectionData.selected[0].modelType.typeHierarchyArray.indexOf( 'Dataset' ) === -1 ) {
        dialogService.closeDialog( 'INFO_PANEL_CONTEXT', popupId );
        return null;
    }
    const datasetVMO = _.cloneDeep( selectionData.selected[0] );
    appCtxService.updatePartialCtx( 'panelContext', { vmo: selectionData.selected[0], parent: selectionData.pselected } );

    return datasetVMO;
};

export const buildFileVersionsSubPanelContext = ( commandContext ) => {
    if( commandContext.selectionData ) {
        return commandContext;
    }

    const selectedObjects = appCtxService.getCtx( 'mselected' );
    const selectionData = {
        selected: adapterService.getAdaptedObjectsSync( selectedObjects )
    };

    return { ...commandContext, selectionData };
};
...
export default {
    ...
    getColumns,
    getFileVersions,
    setDatasetVMOFromSelection,
    buildFileVersionsSubPanelContext
};
