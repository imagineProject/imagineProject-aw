...
export const awTextViewerOnMount = function( viewerData, viewModel ) {
    ...
    appCtxService.updateCtx( 'chatContext.Summarization', chatInputObj );
...
export const awTextViewerOnUnMount = function( viewModel ) {
    ...
    let chatContextObj = appCtxService.getCtx( 'chatContext.Summarization' );
    if( chatContextObj ) {
        chatContextObj.enabled = false;
        appCtxService.updateCtx( 'chatContext.Summarization', chatContextObj );
    }
 ...
 * @returns {Object} promise
 */
export const awTextViewerRenderFn = function( { elementRefList, viewModel, subPanelContext } ) {
    let headerPropList = [];
    let viewerHeaderElem;
    let loadingDiv;
    let text;
    let txtViewerRef = elementRefList.get( 'awp0TextViewer' );

    if( txtViewerRef.current ) {
        txtViewerRef.current.style.height = viewModel.data.viewerHeight;
    }
    if( viewModel.data.viewerData ) {
        ...
        text =
            <div className='aw-viewerjs-border aw-viewerjs-innerContent aw-markup-overflowHidden'>
                <div id='aw-text-container' className='aw-viewerjs aw-viewerjs-text aw-markup-height100'>
    ...
    return (
        <div id='aw-text-viewer' className='aw-text-viewer' ref={txtViewerRef}>
