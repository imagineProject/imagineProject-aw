...
/**
 * Prepare SOA input for xrt (getDeclarativeStyleSheet) call
 *
...
var _prepareSOAInput = function( type, targetPage, vmo, objectType, locationInfo, xrtContext = {}, reviseSaveAsInfo, isStickyTab ) {
    ...
    if( vmo && vmo.modelType ) {
        ...
        logger.error( 'Failed to prepare SOA input for getDeclarativeStyleSheet' );
        return;
    }
    return inputData;
};
...
export let handleGetDeclStyleSheetResponse = function( response, pageKey, vmo, type, dataCtxNode, ignoreTabModel ) {
    ...
    if( response && response.declarativeUIDefs.length > 0 ) {
        ...
        //Refresh/Register the Policy returned from getDeclarativeStyleSheet
...
/**
 * Calls baseGetDeclarativeStyleSheet with provided input.
 *
...
export let getDeclStyleSheet = function( type, targetPage, vmo, objectType, locationInfo, xrtContext, reviseSaveAsInfo, isStickyTab ) {
    ...
    var promise = tcDataManagementService.baseGetDeclarativeStyleSheets( inputData );
    ...
    return promise.then( function( response ) {
        ...
        tcDataManagementService.processGetDeclarativeStyleSheetResponseToSwf( response );

        if( response && response.ServiceData && response.ServiceData.partialErrors && response.ServiceData.partialErrors.length > 0 ) {
            const partialErrors = response.ServiceData.partialErrors;
            if( partialErrors[ 0 ] && partialErrors[ 0 ].errorValues && partialErrors[ 0 ].errorValues.length > 0 ) {
                ...
                _.forEach( partialErrors[ 0 ].errorValues, function( errorValue ) {
                    if( errorValue.code === 515328 || errorValue.code === 74016 ) {
...
/**
 * Calls baseGetDeclarativeStyleSheets with provided input.
 *
