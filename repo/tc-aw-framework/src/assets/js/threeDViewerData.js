...
export default class ThreeDViewerData {
        ...
        ThreeDViewerData.getSelectedModelObject( subPanelContext ).then( ( returnMO ) => {
            ...
            if( force3DViewerReload || !aw3dViewerService.isSameProductOpenedAsPrevious( selectedObj ) ) {
                ...
                aw3dViewerService.getViewerLoadInputParameterPromise( selectedObj, this.compute3DViewerWidth(), this.compute3DViewerHeight(), null, returnMO ).then( viewerLoadInputParams => {
                    try {
                        ...
                        if( _.includes( betaPrefValues, 'enableServerless' ) ) {
                            viewerLoadInputParams.setBomlineProviderFn( () => {
                ...
                aw3dViewerService.restorePreviousView( viewerAtomicData ).then( viewerData => {
                    ...
                    this.setupViewerAfterLoad( viewerData, selectedObj, true );
                    this.registerForConnectionProblems();
                    returnPromise.resolve( this );
                } ).catch( ( errorMsg ) => {
                    logger.error( 'Failed to load viewer : ' + errorMsg );
     ...
     * @param {Boolean} isRestore boolean indicating if viewer is being restored
     */
    setupViewerAfterLoad( viewerData, viewerContextObj, isRestore ) {
        ...
        if( !this.viewerCtxData.isMMVRendering() && this.viewerCtxData.getThreeDViewManager() ) {
        ...
        if( viewerCtxSvc.isServerless() ) {
            this.viewerCtxData.getViewerAtomicDataSubject().notify( this.viewerCtxData.AUTO_SAVE_VIS_BOOKMARK, 'stop' );//stop listening for part viewer
        }
        /*
        Setting fit after adding parent element into DOM, which make sure clientHeight and clientWidth are available
        ClientHeight and clientWidth are available for structureviewer before adding to DOM and not threeDViewer which needs investigation
        So calling fit for PartViewer only. LCS-1109549
        */
        if( !isRestore ) {
            this.viewerCtxData.getNavigationManager().viewAll();
        }
    }
    ...
    compute3DViewerHeight() {
        ...
        if( !_.isNull( heightElement ) && !_.isUndefined( heightElement ) ) {
            ...
            if( locationCtx[ 'ActiveWorkspace:Location' ] === 'com.siemens.splm.clientfx.tcui.xrt.showObjectLocation' ) {
                ...
                let actualHeight = _.toNumber( window.getComputedStyle( heightElement, null ).getPropertyValue( 'height' ).replace( 'px', '' ) );
                let actualPadTop = _.toNumber( window.getComputedStyle( heightElement, null ).getPropertyValue( 'padding-top' ).replace( 'px', '' ) );
                let actualPadBottom = _.toNumber( window.getComputedStyle( heightElement, null ).getPropertyValue( 'padding-bottom' ).replace( 'px', '' ) );
                let computedHeight = actualHeight;
                try {
                    computedHeight = actualHeight - actualPadTop - actualPadBottom;
                } catch ( err ) {
                    computedHeight = actualHeight;
                }
                parentElement.style.height = computedHeight + 'px';
                //We need to subtract 48 which is the height of the toolbar
                currElement.style.height = computedHeight - 46 + 'px';
                return computedHeight - 46;
            }
            parentElement.style.height = heightElement.clientHeight + 'px';
            //We need to subtract 48 which is the height of the toolbar
            currElement.style.height = heightElement.clientHeight - 46 + 'px';
            return heightElement.clientHeight - 46;
        }
            ...
            this.mvProxySelectionChangedEventListener = eventBus.subscribe(
                ...
                function( eventData ) {
                    if( this.viewerCtxData.getModelViewManager() && eventData && Array.isArray( eventData.selectedObjects ) && eventData.selectedObjects.length > 0 &&
                        eventData.selectedObjects[ 0 ] && eventData.selectedObjects[ 0 ].props && eventData.selectedObjects[ 0 ].props.fnd0DisclosedModelView &&
                        Array.isArray( eventData.selectedObjects[ 0 ].props.fnd0DisclosedModelView.dbValues ) &&
                        eventData.selectedObjects[ 0 ].props.fnd0DisclosedModelView.dbValues.length > 0 ) {
    ...
    registerViewerForParentResize() {
        ...
        if( ContentResizeObserver && heightElement ) {
            this.divResizeobserver = new ContentResizeObserver( () => {
                this.set3DViewerSize();
            } );
            this.divResizeobserver.observe( heightElement );
        }
    }
