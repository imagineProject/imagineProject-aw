    ...
    if( $.isArray( logicalProperties ) ) {
        $.each( logicalProperties, function( pIndex, property ) {
            ...
            if( logicalPropertyDescriptorsMap[ property.propertyName ] ) {
                viewModelProperty = createViewModelPropertyFromLogicalProperty( viewModelObject, property,
...
/**
 * Extracts the objects from response and returns that
 * @param {Object} response - response
 *
 * @return {Objects} objects received in response
 */
function getObjectsInJsonResponse( response ) {
 ...
 * @param {Object} response - response
 */
export let processViewModelObjectsFromJsonResponse = function( viewModelObjects, response ) {
    // update the view model object with the view model properties.
    var objectsInResponse = getObjectsInJsonResponse( response );

    viewModelObjects.forEach( ( viewModelObject )=>{
        var objectUpdated = false;
        if( viewModelObject && objectsInResponse ) {
...
export let getViewModelProperties = function( viewModelObjects, propNames ) {
    // Adding the missing logic from old getViewModelProperties code PLM788497.
    // The below code will not call SOA if input VMO already has the input property in it.
    var objects = [];
    _.forEach( viewModelObjects, function( viewModelObject ) {
        if( viewModelObject ) {
    ...
    if( objects.length > 0 ) {
        ...
        promise = soaSvc.post( 'Internal-AWS2-2017-12-DataManagement', 'getViewModelProperties2', input )
            .then( function( response ) {
                processViewModelObjectsFromJsonResponse( viewModelObjects, response );
                return {
                    objects: viewModelObjects,
                    ServiceData: response.ServiceData,
                    logicalObjects: getObjectsInJsonResponse( response )
                };
            } );
        promise.input = input; // Adding the mising line from old getViewModelProperties code PLM788497.
        return promise;
    }
...
export let getTableViewModelProperties = function getTableViewModelProperties( vmNodes, context, overriddenPropertyPolicy ) {
    ...
    var addPropsToServiceData = context.addPropsToServiceData ? context.addPropsToServiceData : [];
    ...
    if(addPropsToServiceData.length > 0) {
        input.input.requestPreference.addAdditionalPropsToServiceData = addPropsToServiceData;
    }

    if( missingUids.length > 0 ) {
        if ( overriddenPropertyPolicy ) {
            if ( typeof overriddenPropertyPolicy === 'object' ) {
                overriddenPropertyPolicy.override = true;
            } else if ( typeof overriddenPropertyPolicy === 'string' ) {
                overriddenPropertyPolicy = JSON.parse( overriddenPropertyPolicy );
                overriddenPropertyPolicy.override = true;
            }
        }

        return tcDataManagementService.baseGetTableViewModelProperties( input, overriddenPropertyPolicy )
            .then( function( response ) {
                // process DCPs
