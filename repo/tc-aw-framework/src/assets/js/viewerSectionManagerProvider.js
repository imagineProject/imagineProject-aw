...
const GEOANALYSIS_SECTION_PVWID = 'pvwId';
...
class ViewerSectionManager {
    ...
    initialize() {
        ...
        this.sectionPlanePositionChangeListener = {
            ...
            onPlaneSelectionChange: function( isSelected ) {
                ...
                //Move manipulator unset
                if( isSelected && self.viewerContextData ) {
                    let manipulatorEnabled = self.viewerContextData.getValueOnViewerAtomicData( 'moveParts.isManipulatorEnabled' );
                    if( manipulatorEnabled && self.viewerContextData.getMotionManager() ) {
                        self.viewerContextData.getMotionManager().setManipulator();
                    }
                }
            ...
            onPlaneDirectionChange: function( value ) {
                let sectionDir = ViewerSectionManager.getPlaneIdFromNormalVector( value );
                const sectionId = this.visObject.resourceID;
                if( self.debounceCallback ) {
                    AwTimeoutService.instance.cancel( self.debounceCallback );
                }
                self.debounceCallback = AwTimeoutService.instance( () => {
                    var selectedSection = self.sectionIdToSectionObjectMap[ sectionId ];
                    if( selectedSection ) {
                        self.modifySectionData( selectedSection, sectionDir, sectionId );
                    }
                    self.debounceCallback = null;
                }, 250 );
            }
    ...
    setupSectionPlaneTranslateModeChangeEventListener() {
        this.sectionPlaneTranslateModeChangeListener = {
            onSectionPlaneTranslateModeChange:function( sectionPlaneTranslateMode ) {
                this.viewerContextData.updateViewerAtomicData( viewerContextService.VIS_SECTION_PLANE_TRANSLATE_MODE, sectionPlaneTranslateMode );
                if( sectionPlaneTranslateMode ) {
                    let dialogToolAndInfo = appCtxSvc.getCtx( 'activeToolsAndInfoCommand_dialog' );
                    if( dialogToolAndInfo && dialogToolAndInfo.commandId !== 'Awv0GeometricAnalysisSection' ) {
                        this.viewerContextData.closeActiveDialog();
                    }
                    ...
                    this.viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( this.handleEscapeKeyboardSectionIn3d.bind( this ) );
                } else {
                    ...
                    this.viewerContextData.getViewerAtomicDataSubject().unsubscribe( viewerContextService.VIEWER_CREATE_MARKUP_BEGIN, this );
                    this.viewerContextData.getViewerAtomicDataSubject().unsubscribe( viewerContextService.VIEWER_CREATE_SECTION_BEGIN, this );
                    this.viewerContextData.getViewerAtomicDataSubject().unsubscribe( viewerContextService.VIEWER_IS_EXPLODE_VIEW_VISIBLE, this );
                }
            }.bind( this )
        };
        this.viewerView.sectionMgr.addListener( this.sectionPlaneTranslateModeChangeListener );
    }

    handleEscapeKeyboardSectionIn3d() {
        this.enableSectionPlaneTranslateMode( false );
    }
    /**
    ...
    setupAtomicDataTopics() {
        ...
        this.viewerContextData.getViewerAtomicDataSubject().subscribe( viewerContextService.VIEWER_SECTION_CMD_FROM_PVW, this );
    ...
    setupAtomicDataTopicsOnPanelReveal( ) {
        this.viewerContextData.getViewerAtomicDataSubject().subscribe( viewerContextService.PVW_SECTION_CONTEXT_MENU, this );
    }
    ...
    update( topic, data ) {
        ...
        if( ( topic === viewerContextService.VIEWER_VIEW_MODE_TOKEN || topic === viewerContextService.VIEWER_VISIBILITY_TOKEN || topic === viewerContextService.VIEWER_SECTION_CMD_FROM_PVW ) && viewerViewMode === 'VIEWER3D' && isViewerRevealed === true ) {
            this.updateSectionCommandState();
            this.viewerContextData.getViewerAssetsMgr().addViewerAssetsSelectedListeners( this.isSectionSelected.bind( this ) );
            this.setupSectionPlaneTranslateModeChangeEventListener();
            ...
            if( data && data.isActivated || data.commandId !== 'Awv0GeometricAnalysisSection' ) {
                ...
                if(  viewerContextService.isNxGestureSettingEnabled( this.viewerContextData ) ) {
                    viewerContextService.setDefaultActionNavigation( this.viewerContextData );
                }
            }
        }else if( topic === viewerContextService.PVW_SECTION_CONTEXT_MENU ) {
            if( data.pvwContextCmd === 'deleteSectionCmd' ) {
                this.deleteSectionCmdFromPvw( data );
            }else{
                this.updateSelectedSectionFromPvw( data );
            }
        }else if( topic === viewerContextService.VIS_DELETE_FROM_KEYBOARD ) {
            this.deleteSelectedSectionFromKeyboard();
        }else if( topic ===  viewerContextService.VIEWER_AREA_SELECT ) {
            this.disableSectionMode();
        } else if( topic === viewerContextService.VIEWER_CREATE_MARKUP_BEGIN || topic === viewerContextService.VIEWER_IS_EXPLODE_VIEW_VISIBLE ||
    ...
    /**
     * delete selected section from keyboard
     */
    deleteSelectedSectionFromKeyboard() {
        if( this.viewerView && this.viewerView.sectionMgr ) {
            let sectionsPromise = this.viewerView.sectionMgr.getAllSections();
            sectionsPromise.then( ( sectionsListFromServer )=> {
                ...
                if( sectionsListFromServer && sectionsListFromServer.length > 0 ) {
                    ...
                    AwPromiseService.instance.all( getSectionPromises ).then(
                        ( sectionsList )=>{
                            let getSectionSelectionDataPromise = [];
                            if( sectionsList && sectionsList.length > 0 ) {
                                for( let i = 0; i < sectionsList.length; i++ ) {
                                    getSectionSelectionDataPromise.push( sectionsList[ i ].getSelected() );
                                }
                            }
                            AwPromiseService.instance.all( getSectionSelectionDataPromise ).then( ( sectionSelectionData )=>{
                                this.deleteSelectedSection( sectionSelectionData, sectionsList );
                            } );
                        }
                    );
                }
     ...
     * @param {Array} sectionSelectionData all selected/unselected values for section list
     * @param {Array} sectionsList List of section created
     */
    deleteSelectedSection( sectionSelectionData, sectionsList ) {
        let selSectionObj = {};
        let selSectionResourceId = null;
        let sectionSelectedIndex = _.indexOf( sectionSelectionData, true );
        if( sectionSelectedIndex > -1 ) {
            selSectionResourceId = sectionsList[ sectionSelectedIndex ].visObject.resourceID;
            selSectionObj = sectionsList[sectionSelectedIndex ];
        }
        if( !_.isEmpty( this.sectionIdToSectionObjectMap ) && !_.isNull( selSectionResourceId ) ) {
            this.deleteSection( selSectionResourceId ).then( ()=>{
                if( this.sectionsList.length === 0 ) {
                    this.viewerContextData.closeActiveDialog();
                }
            } );
        }else if( !_.isEmpty( selSectionObj ) ) {
            selSectionObj.delete().then(
                () => {
                    AwPromiseService.instance.resolve();
                },
                function( errorMessage ) {
                    logger.error( errorMessage );
                    AwPromiseService.instance.reject( errorMessage );
                } );
        }
    }
    ...
    disableSectionMode() {
        this.viewerContextData.getViewerAtomicDataSubject().unsubscribe( this.viewerContextData.SUB_COMMANDS_TOOLBAR_ACTIVE_STATUS, this );
        this.viewerContextData.getViewerAtomicDataSubject().unsubscribe( viewerContextService.VIEWER_COMMAND_PANEL_LAUNCHED, this );
        this.viewerContextData.getViewerAtomicDataSubject().unsubscribe( viewerContextService.PVW_SECTION_CONTEXT_MENU, this );
        this.viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( null );
        this.setSectionModeEnabled( false );
        this.enableSectionPlaneTranslateMode( false );
    }
    /**
     * delete section from Pvw
     * @param {Object} data data from pvw
     */
    deleteSectionCmdFromPvw( data ) {
        let selSectionObj = null;
        if( this.sectionsList && Array.isArray( this.sectionsList ) && this.sectionsList.length > 0 ) {
            let sectionListLength = this.sectionsList.length;
            for( let i = 0; i < sectionListLength; i++ ) {
                if( this.sectionsList[ i ].pvwId === data.pvwObject.planeId ) {
                    selSectionObj = this.sectionsList[ i ];
                    break;
                }
            }
            if( selSectionObj && selSectionObj.sectionId ) {
                this.deleteSectionFromViewerContext( selSectionObj.sectionId );
            }
            if( this.sectionsList.length === 0 ) {
                this.viewerContextData.closeActiveDialog();
            }
    ...
    /**
     * update Selected section for clip/direction from pvw
     */
    updateSelectedSectionFromPvw( data ) {
        let selSectionObj = null;
        let pvwObject = data.pvwObject;
        if( this.sectionsList && Array.isArray( this.sectionsList ) && this.sectionsList.length > 0 ) {
            let sectionListLength = this.sectionsList.length;
            for( let i = 0; i < sectionListLength; i++ ) {
                if( this.sectionsList[ i ].pvwId === pvwObject.planeId ) {
                    selSectionObj = this.sectionsList[ i ];
                    break;
                }
            }
        }
        let pvwSectionDir = pvwObject.sectionDirection.toUpperCase();
        if( pvwSectionDir !== selSectionObj.sectionPlaneLabel ) {
            this.modifySectionDataForPvw( selSectionObj.sectionId, pvwObject.sectionDirection.toUpperCase() );
        } else if ( !_.isUndefined( pvwObject.awSectionSide ) && pvwObject.awSectionSide !== selSectionObj[GEOANALYSIS_SECTION_CLIP_STATE] ) {
            let selectedSection = this.sectionIdToSectionObjectMap[ selSectionObj.sectionId ];
            selectedSection.getCutLines().then( ( updatedCutLineValue )=>{
                this.updateSectionCutLineState( selSectionObj.sectionId, updatedCutLineValue, false );
                this.updateSectionClipState( selSectionObj.sectionId, pvwObject.awSectionSide );
            } );
        }
    }
    ...
    setSectionModeEnabled( isEnabled ) {
        ...
        if( this.viewerView.sectionMgr ) {
            this.viewerView.sectionMgr.enableSectionCreator( isEnabled );
        }
    }

    /**
     * Set section translate mode
     * @param {Boolean} enable true to enable section translate mode
     */
    enableSectionPlaneTranslateMode( enable ) {
        const isSectionPlaneTranslateToPointModeEnabled = this.viewerContextData.getValueOnViewerAtomicData( viewerContextService.VIS_SECTION_PLANE_TRANSLATE_MODE );
        if( this.viewerView.sectionMgr && this.viewerView.sectionMgr.enableSectionPlaneTranslateMode && isSectionPlaneTranslateToPointModeEnabled ) {
            this.viewerView.sectionMgr.enableSectionPlaneTranslateMode( enable );
            this.viewerContextData.updateViewerAtomicData( viewerContextService.VIS_SECTION_PLANE_TRANSLATE_MODE, false );
        }
    }
    ...
    async setSectionOffsetValue( sectionId, newValue ) {
        ...
        if( selectedSection ) {
            ...
            this.timeoutForSetOffset = AwTimeoutService.instance( () => {
                this.updateSectionOffsetInViewerContext( sectionId, roundedOffsetValue, true );
                this.timeoutForSetOffset = null;
            }, 500 );
    ...
    async moveSection( sectionId, offsetValue ) {
        ...
        if( selectedSection ) {
            ...
            this.timeoutForMoveSection = AwTimeoutService.instance( () => {
                selectedSection.setOffset( convertedOffsetValue );
                this.updateSectionOffsetInViewerContext( sectionId, roundedOffsetValue, true );
    ...
    async updateClipState( sectionId, clipState ) {
        ...
        return selectedSection.setClipState( clipState ).then( () => {
            selectedSection.getCutLines().then( ( updatedCutLineValue )=>{
                this.updateSectionCutLineState( sectionId, updatedCutLineValue, false );
    ...
    /**
     * @param {String} sectionId section's Id
     * @param {Boolean} newCutLineValue new Cut line value
     * @returns {function} Updated viewer atomic data
     */
    updateSectionCutLineState( sectionId, newCutLineValue, updateSectionObj ) {
        for( var i = 0; i < this.sectionsList.length; i++ ) {
            var currentSection = this.sectionsList[ i ];
            if( currentSection[ GEOANALYSIS_SECTION_ID ] === sectionId ) {
                currentSection[ GEOANALYSIS_SECTION_CUT_LINES_STATE ] = newCutLineValue;
                if( updateSectionObj ) {
                    return this.updateSectionListToViewerContext();
                }
                return false;
    ...
    createSectionObject( sectionObj, planeId, oldSectionID ) {
        ...
        return AwPromiseService.instance.all( promises ).then( ( values ) => {
            ...
            sectionValueJso[ GEOANALYSIS_SECTION_PVWID ] = sectionObj.visObject.pvwObject ? sectionObj.visObject.pvwObject._pvwID : '';
    ...
    updateSectionListToViewerContext() {
        if ( this.sectionsList.length === 0 ) {
            ...
            this.updateGeometricSectionState( GEOANALYSIS_IS_EDIT_SECTION_UPDATE_NEEDED, true  );
        }
     ...
     * modify PVW Data
     */
    modifySectionDataForPvw( sectionId, newNormal ) {
        var selectedSection = this.sectionIdToSectionObjectMap[ sectionId ];
        var newPlaneId = ViewerSectionManager.getPlaneIdFromNormal( newNormal );
        if( selectedSection ) {
            if( newPlaneId === '4' ) {
                ...
                if( customSectionData && customSectionData[ GEOANALYSIS_SECTION_OFFSET_VALUE ] ) {
                    ...
                    return selectedSection.setOffset( convertedOffsetValue ).then(
                        ...
                        function( errorMessage ) {
                            logger.error( errorMessage );
                            return AwPromiseService.instance.reject( errorMessage );
                        }
                    );
                }
                return this.modifySectionData( selectedSection, newPlaneId, sectionId );
    ...
    isSectionSelected() {
        if( this.viewerView && this.viewerView.sectionMgr ) {
            let sectionsPromise = this.viewerView.sectionMgr.getAllSections();
            return sectionsPromise.then( ( sectionsListFromServer )=> {
                let getSectionPromises = [];
                if( sectionsListFromServer && sectionsListFromServer.length > 0 ) {
                    for( let i = sectionsListFromServer.length - 1; i >= 0; i-- ) {
                        getSectionPromises.push( sectionsListFromServer.getSection( i ) );
                    }
                    return AwPromiseService.instance.all( getSectionPromises ).then(
                        ( sectionsList )=>{
                            let getSectionSelectionDataPromise = [];
                            if( sectionsList && sectionsList.length > 0 ) {
                                for( let i = 0; i < sectionsList.length; i++ ) {
                            ...
                            return AwPromiseService.instance.all( getSectionSelectionDataPromise ).then( ( sectionSelectionData )=>{
                                let sectionSelectedIndex = _.indexOf( sectionSelectionData, true );
                                if( sectionSelectedIndex > -1 ) {
                                    this.selectedResourcId = sectionsList[ sectionSelectedIndex ].visObject.resourceID;
                                    return true;
                                }
                                return false;
                            } );
    ...
    async getAllSectionsDetails() {
        let serverSectionsList = null;
        if( this.viewerView && this.viewerView.sectionMgr ) {
            let sectionsListFromServer = await this.viewerView.sectionMgr.getAllSections();
            let getSectionPromises = [];
            if( sectionsListFromServer && sectionsListFromServer.length > 0 ) {
                for( let i = sectionsListFromServer.length - 1; i >= 0; i-- ) {
                    getSectionPromises.push( sectionsListFromServer.getSection( i ) );
                }
                let  sectionsFromSectionList = await AwPromiseService.instance.all( getSectionPromises );
                let getSectionNormalPromises = [];
                if( sectionsFromSectionList && sectionsFromSectionList.length > 0 ) {
                    serverSectionsList = sectionsFromSectionList;
                    for( let i = 0; i < sectionsFromSectionList.length; i++ ) {
                    ...
                    let  sectionNormals = await AwPromiseService.instance.all( getSectionNormalPromises );
                    let getSectionObjectPromises = [];
                    if( serverSectionsList && serverSectionsList.length > 0 && sectionNormals.length === serverSectionsList.length ) {
                        for( let i = 0; i < serverSectionsList.length; i++ ) {
                            getSectionObjectPromises.push( this.createSectionObject( serverSectionsList[ i ], ViewerSectionManager.getPlaneIdFromNormalVector( sectionNormals[ i ] ) ) );
                        }
                        let sectionObjectsFromSectionList = await AwPromiseService.instance.all( getSectionObjectPromises );
                        this.sectionIdToSectionObjectMap = {};
                        this.sectionsList.length = 0;
                        for( let i = 0; i < sectionObjectsFromSectionList.length; i++ ) {
                            let sectionData = sectionObjectsFromSectionList[ i ];
    ...
    async disableSectionSelectionWhenMoveManipulatorSelected() {
        let sectionInfo = this.getValueOnGeometricSectionState( GEOANALYSIS_SELECTED_SECTION_IN_3D );
        if( !_.isNull( sectionInfo ) && !_.isUndefined( sectionInfo ) ) {
            this.setSectionSelection( sectionInfo.sectionData.sectionId, false );
        }else {
            let result = await this.isSectionSelected();
            if( result ) {
                let res = await this.getAllSectionsDetails();
                if( res ) {
                    this.setSectionSelection( this.selectedResourcId, false );
                }
            }
        }
    }
