...
import { getObject } from 'soa/kernel/clientDataModel';
...
export const handleTabSelectionChange = ( localSelectionData, previousSelectionData, ctxPselected, currentActivePageTitle, lastActivePageTitle, locationContext ) => {
    ...
    if( previousSelectionData?.value?.selected?.length > 0 ) {
        if( locationContext[ 'ActiveWorkspace:SubLocation' ] === 'showObject' && previousSelectionData.value.selected[ 0 ].uid !== ctxPselected.uid ) {
        ...
        } else if( currentActivePageTitle !== lastActivePageTitle ) {
...
export const checkEdit = ( pageType ) => {
    ...
    if( pageType === 'SUMMARY' && stateParams.edit ) {
        ...
        if( eh && eh.canStartEdit() ) {
            // clean up edit param from url once it is consumed as it causes other tabs to go in edit mode after tab change in SWA.
            AwStateService.instance.go( '.', { edit : undefined } );
            eh.startEdit();
        }
    }
};
...
const _processViewModelProperties = ( vmo ) => {
    const sourceUids = viewModelObjectService.processSourceUids( vmo );
    for ( const [ name, sourceObjectUid ] of Object.entries( sourceUids ) ) {
        if( vmo.props[name] && sourceObjectUid ) {
            const srcObj = getObject( sourceObjectUid );

            const nameTokens = name.split( '.' );
            const propertyName = nameTokens[ nameTokens.length - 1];
            if( srcObj?.props[ propertyName ] && name.startsWith( 'REF' ) ) {
                vmo.props[name].propertyDescriptor = srcObj.props[ propertyName ].propertyDescriptor;
            }
        }
    }
};
...
export const initialize = ( elementRefList, xrtState, xrtData, vmo, type, objectType, subPanelContext ) => {
    if( xrtState ) {
        ...
        if( xrtData ) {
            ...
            const parentUids = xrtUtilities.extractParentUidsFromPageRendering( xrtData );
            let uid = vmo ? vmo.uid : Object.keys( xrtData.data.objects )[ 0 ];
            if( parentUids && parentUids.length === 1 ) {
                const response = xrtData.data.objects[ parentUids[ 0 ] ];
                if( response ) {
                    if( response.uid ) {
                        uid = response.uid;
                    } else if( _.isArray( response ) &&  response.length > 0 && response[0] && response[0].uid ) {
                        uid = response[0].uid;
                    }
                }
            }

            vmoIn = xrtUtilities.prepareXrtViewModelObject( uid, xrtData, operationName, owningObjUid, vmo );
            _processViewModelProperties( vmoIn );
...
export const refreshData = ( xrtState, response, vmo ) => {
    if( vmo && xrtState && xrtState.getValue()?.xrtVMO ) {
        let objs = response.updatedObjects;
        if( !objs ) {
            objs = response.modifiedObjects;
        }
        let existingVMO = xrtState.getValue().xrtVMO;
        let updatedVmo = viewModelObjectService.updateViewModelObjectFromModelObjects( existingVMO, objs );
        if( updatedVmo ) {
            let newXrtState = { ...xrtState.getValue() };
            newXrtState.xrtVMO = updatedVmo;
            xrtState.update( newXrtState );
        }
    }
};
