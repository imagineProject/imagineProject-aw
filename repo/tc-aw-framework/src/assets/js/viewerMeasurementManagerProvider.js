...
class ViewerMeasurementManager {
    ...
    setupAtomicDataTopics() {
        ...
        this.viewerContextData.getViewerAtomicDataSubject().subscribe( this.viewerContextData.VIEWER_VOLUME_SELECT, this );
    ...
    startViewerMeasurement( measurementMode ) {
        ...
        if( renderOption === 'CSR' && isUseTransparency === 'Transparent' ) {
            this.viewerContextData.styleSelection( 'PartColor' );
            _previousTransparancyMode = isUseTransparency;
        }
        this.viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( this.handleEscIn3d.bind( this ) );
    }
    ...
    closeViewerMeasurement() {
        ...
        if( renderOption === 'CSR' && _previousTransparancyMode ) {
            this.viewerContextData.styleSelection( _previousTransparancyMode );
     ...
     *
     * @param {Object} measurement the newly created measurement objects
     */
    ...
    /**
     * Delete selected measurement from AW
     */
    deleteSelectedMeasurementFromAW() {
        this.updateSelectedMeasurementAtomicData().then( ()=>{
            this.currentSelectedMeasurement.delete().then( ()=>{
                this.updateSelectedMeasurementContext( null );
            } );
        } ).catch( function( reason ) {
            logger.warn( 'Could not delete selected measurement: ' + reason );
        } );
    }
    /**
     * Update selected measurement Atomic data.
     * @return {Promise} promise that is resolved after selected measurement context is updated
     */
    updateSelectedMeasurementAtomicData() {
        if( !_.isNull( this.currentSelectedMeasurement ) ) {
    ...
    update( topic, data ) {
        if( topic === this.viewerContextData.SUB_COMMANDS_TOOLBAR_ACTIVE_STATUS ) {
            ...
            if( !_.isNull( this.currentSelectedMeasurement ) ) {
                this.deleteSelectedMeasurementFromAW();
            }else{
                this.deleteSelectedMeasurementFromKeyboard();
            }
        } else if ( ( topic === this.viewerContextData.VIEWER_VOLUME_SELECT || topic === viewerContextService.VIS_SECTION_PLANE_TRANSLATE_MODE ) && data === true ) {
            viewerContextService.closeSubCommandsToolbar( this.viewerContextData );
        }else if(  topic === viewerContextService.VIEWER_VIEW_MODE_TOKEN  ) {
            this.viewerContextData.getViewerAssetsMgr().addViewerAssetsSelectedListeners( this.isMeasurementSelected.bind( this ) );
        }
    }

    /**
     * delete selected measurement from keyboard
     */
    deleteSelectedMeasurementFromKeyboard() {
        if( this.viewerView && this.viewerView.measurementMgr ) {
            this.viewerView.measurementMgr.getAllMeasurements()
                .then( rawMeasurementList => {
                    if( rawMeasurementList && rawMeasurementList.length > 0 ) {
                        let getMeasurementSelectionDataPromise = [];
                        let getMeasurementPromise = [];
                        for( let index = 0; index < rawMeasurementList.length; index++ ) {
                            getMeasurementPromise.push( rawMeasurementList.getMeasurement( index ) );
                        }
                        AwPromiseService.instance.all( getMeasurementPromise ).then( ( rawMeasurement )=>{
                            if( rawMeasurement && rawMeasurement.length > 0 ) {
                                for( let i = 0; i < rawMeasurement.length; i++ ) {
                                    getMeasurementSelectionDataPromise.push( rawMeasurement[ i ].getSelected() );
                                }
                            }
                            AwPromiseService.instance.all( getMeasurementSelectionDataPromise ).then( ( measurementSelectionData )=>{
                                this.deleteSelctedMeasurement( measurementSelectionData, rawMeasurement );
                            } );
                        } );
                    }
                } )
     ...
     * @param {Array} measurementSelectionData all selected/unselected values for measurement list
     * @param {Array} rawMeasurement List of measurement created
     */
    // eslint-disable-next-line class-methods-use-this
    deleteSelctedMeasurement( measurementSelectionData, rawMeasurement ) {
        let selMeasurementObj = {};
        let measurementSelectedIndex = _.indexOf( measurementSelectionData, true );
        if( measurementSelectedIndex > -1 ) {
            selMeasurementObj = rawMeasurement[measurementSelectedIndex ];
        }
        if( !_.isEmpty( selMeasurementObj ) ) {
            selMeasurementObj.delete()
                .catch( ( reason )=>{
                    logger.warn( 'Could not delete selected measurement: ' + reason );
                } );
        }
    ...
    isMeasurementSelected() {
        ...
        return this.viewerView.measurementMgr.getAllMeasurements()
            .then( rawMeasurementList => {
                if( rawMeasurementList && rawMeasurementList.length > 0 ) {
                    let getMeasurementSelectionDataPromise = [];
                    let getMeasurementPromise = [];
                    ...
                    return AwPromiseService.instance.all( getMeasurementPromise ).then( ( rawMeasurement )=>{
                        if( rawMeasurement && rawMeasurement.length > 0 ) {
                            for( let i = 0; i < rawMeasurement.length; i++ ) {
                                getMeasurementSelectionDataPromise.push( rawMeasurement[ i ].getSelected() );
                            }
                        }
                        return AwPromiseService.instance.all( getMeasurementSelectionDataPromise ).then( ( measurementSelectionData )=>{
                            let measurementSelectedIndex = _.indexOf( measurementSelectionData, true );
                            if( measurementSelectedIndex > -1 ) {
                                return true;
                            }
