...
export let getPrimaryCrumb = function( baseSelection ) {
    ...
    if( obj !== null ) {
        ...
        return {
            ...
            showArrow: true,
            primaryCrumb: true,
            onCrumbClick: ( crumb, event ) => onSelectCrumb( crumb )
        };
    }
};
...
const getDuidForFlexibleLayouts = ( selectedObjects ) => {
    //sort the selection based on alternateID length to always start finding duid hiearchy from the closest to root to optimize.
    selectedObjects.sort( ( a, b ) => {
        if ( a.alternateID && b.alternateID ) {
            return a.alternateID.length - b.alternateID.length;
        } else if ( a.alternateID ) {
            return -1;
        } else if ( b.alternateID ) {
            return 1;
        }
        return 0;
    } );
...
export let buildNavigateBreadcrumb = function( totalFound, selectedObjects = [], baseSelection ) {
    ...
    var nSelectedCrumb = {
        clicked: false,
        displayName: selectedObjects.length + ' selected',
        selectedCrumb: true
    };
    ...
    if( selectedObjects.length === 0 ) {
        ...
        localeService.getLocalizedText( 'UIMessages', 'Selected' )
            .then( ( msg ) => {
                nSelectedCrumb.displayName = selectedObjects.length + ' ' + msg;
            } );
    }
    var provider = {
    ...
    let d_uid = getDuidForFlexibleLayouts( selectedObjects );
    if( d_uid ) {
    ...
    if( pwaSelectionInfo.currentSelectedCount === 1 && exports.getPrimaryCrumb( baseSelection ).scopedUid !== selectedObjects[ 0 ].uid ) {
    ...
    } else if ( pwaSelectionInfo.currentSelectedCount > 1 ) {
        provider.crumbs.push( nSelectedCrumb );
    }
    ...
    return exports.ensureObjectString( uids ) //
        ...
        .then( ( resp ) => {
            ...
            if( baseCrumb && baseCrumb.displayName ) {
                var d_uids = getDuidForFlexibleLayouts( selectedObjects );
