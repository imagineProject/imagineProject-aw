...
export const createTablePropertyComponent = function( vmDef, props ) {
    return [ new DerivedStateResult( {
        ctxParameters: [],
        additionalParameters: [ props.tablePropertyData, props.columns, props.firstPageUids, props.objects, props.editContextKey, props.operationName, props.enablePropEdit ],
        compute: ( renderContext, tablePropertyData, columns, firstPageUids, objects, editContextKey, operationName, enablePropEdit ) => {
...
export const awTablePropertyRenderFunction = function( props ) {
    const { tablePropertyData, selectionData, editContextKey, dpRef, elementRefList, ctxMin: { dynamicTable } } = props;
    const AwDynamicTableProperty = dynamicTable[ 0 ];
    const dynamicTableStyleRef = elementRefList.get( 'dynamicTableStyle' );
    return <AwDynamicTableProperty tablePropertyData={tablePropertyData} selectionData={selectionData} editContextKey={editContextKey} dpRef={dpRef}
        dynamicTableStyleRef={dynamicTableStyleRef}></AwDynamicTableProperty>;
};
