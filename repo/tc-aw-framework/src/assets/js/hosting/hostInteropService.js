...
/**
 * {String} Encoded origin site from host application
 */
var _originSite = urlAttributes.origin;
...
/**
 * {String} Specifies what the origin of this window must be for the event to be posted
 */
var targetOriginSite = '/';
...
function _processMessageCb( msgObj ) {
    if( _debug_logHandShakeActivity ) {
        ...
        if ( _isRemoteHostingEnabled ) {
            commType = 'Remote Hosting';
        }else {
            commType = 'Window Post Message';
        }
        exports.log( '_processMessageCb: ' + commType + ' ' + 'Host sent: ' + '\n' +
...
function _logCallActivity( direction, bioFunction, serviceDesc, payload ) {
    ...
    if( _debug_logClientCallsToHostDetails ) {
        if( !_.includes( _debug_FilteredFQNs, serviceDesc.FQN ) ) {
            exports.log( getCurrentTime( ) + direction + ' (' + bioFunction + '):' + '\n' +
...
function _logCallActivityResponse( direction, bioFunction, serviceDesc, response ) {
    if( _debug_logClientCallsToHostDetails ) {
        if( !_.includes( _debug_FilteredFQNs, serviceDesc.FQN ) ) {
            exports.log( getCurrentTime( ) + direction + ' (' + bioFunction + '):' + '*** Response ***' + '\n' +
                JSON.stringify( serviceDesc ) + '\n' +
                'Response:' + '\n' +
...
export let startHostHandShake = function() {
        ...
        return new Promise( ( resolve ) => {
            ...
            window.addEventListener( 'message', ( event ) => {
                ...
                if ( event.source === parent ) {
                    if ( targetOriginSite !== event.origin ) {
                        targetOriginSite = event.origin;
                    }
            ...
            if ( _originSite ) {
                targetOriginSite = hostUtils.decode( _originSite );
                parent.postMessage( msgObj, targetOriginSite );
            } else {
                parent.postMessage( msgObj, parent.location.origin );
            }
            resolve();
        } );
    ...
    return _getHostWindowAsync().then( function( hostWnd ) {
        ...
        if( _debug_logHandShakeActivity ) {
            exports.log( getCurrentTime( ) + 'startHostHandShake: ' + 'Found host window' );
        }
        ...
        if( _debug_logHandShakeActivity ) {
            exports.log( getCurrentTime( ) + 'startHostHandShake: ' + 'Host Returned: ' + result );
        }
...
/**
 * Get timestamp for performance measurement.
 *
 * @memberof hostInteropService
 *
 * @returns {String} current timestamp in 'yyyy-mm-dd hh:mm:ss.xxx' format
 */
export let getCurrentTime = function( _date ) {
    let date;
    if ( _date ) {
        date = _date;
    } else {
    ...
    const months = ( date.getMonth( ) + 1 < 10 ? '0' : '' ) + ( date.getMonth( ) + 1 );
    ...
    return `${years}-${months}-${dates} ${hours}:${minutes}:${seconds}.${m} `;
};
...
export let callHostMethodAsync = function( serviceDesc, payload ) {
    ...
    if( !_isRemoteHostingEnabled && !_isPostMessageEnabled ) {
        if ( appCtxSvc.ctx.aw_hosting_browser === hostConfigValues.HOST_BROWSER_TYPE_WEBVIEW2 && window.externalAsync ) {
            if ( hostUtils.isNil( payload ) ) {
                payload = '';
            }
            const descriptor = JSON.stringify( serviceDesc );
            return window.externalAsync.splmHostMethod( descriptor, payload );
        }

        var hostWnd = _getHostWindowSync( window, 0 );

        if ( hostWnd && hostWnd.hasOwnProperty( 'splmHostMethodAsync' ) ) {
            /**
             * The 'host' does not like 'nil' payloads. Use an empty string instead.
             */
            if( hostUtils.isNil( payload ) ) {
            ...
            _logCallActivity( 'Client->Host', 'splmHostMethodAsync, expect promise return', serviceDesc, payload );

            var descriptor = JSON.stringify( serviceDesc );
            return hostWnd.splmHostMethodAsync( descriptor, payload );
        }
...
export let getCanTalkWithHost = function() {
    if( !_haveChecked ) {
        ...
        if( _debug_logHandShakeActivity ) {
            exports.log( getCurrentTime() + 'getCanTalkWithHost: ' + 'Can talk to the host: ' + _canTalk + ' hostVersion: ' + exports.hostversion );
        }
    }
 ...
 */
export let loadClientConfiguration = function( hostConfig ) {
    /**
     ...
     */
    var hostingConfig = hostConfig ?? cfgSvc.getCfgCached( 'hosting' );
...
var splmInterop_HostServiceListUpdate = function( action, jsonList, sendList ) {
    try {
        ...
        if( _debug_logHostCallsToClient || _debug_logHostCallsToClientDetails ) {
            exports.log( getCurrentTime( ) + 'Host->Client' + ' (' + 'splmInterop_HostServiceListUpdate' + '):' + '\n' +
        ...
        if( sendList ) {
            ...
            if( _debug_logHostCallsToClient || _debug_logHostCallsToClientDetails ) {
                exports.log( getCurrentTime( ) + 'Host->Client' + ' (' + 'splmHost_WebSideServiceListUpdate' + '):' + '*** Response ***' + '\n' +
...
var splmInterop_RequestWebServiceList = function() {
    ...
    if( _debug_logHostCallsToClient || _debug_logHostCallsToClientDetails ) {
        exports.log( getCurrentTime( ) + 'Host->Client' + ' (' + 'splmInterop_RequestWebServiceList' + '):' + '*** Response ***' + '\n' +
            JSON.stringify( entries, null, 2 ) );
    }
...
var splmInterop_WebMethod = function( descJson, payload ) {
    ...
    try {
        ...
        if( serviceDesc ) {
            ...
            if( clientSvcToCall ) {
                _logCallActivity( getCurrentTime( ) + 'Host->Client', 'splmInterop_WebMethod', serviceDesc, payload );

                var response = clientSvcToCall.handleMethodCall( payload );

                _logCallActivityResponse( getCurrentTime( ) + 'Host->Client', 'splmInterop_WebMethod', serviceDesc, response );
...
var splmInterop_WebEvent = function( descJson, payload ) {
    try {
        ...
        if( serviceDesc ) {
            ...
            if( clientSvcToCall ) {
                _logCallActivity( getCurrentTime( ) + 'Host->Client', 'splmInterop_WebEvent', serviceDesc, payload );
