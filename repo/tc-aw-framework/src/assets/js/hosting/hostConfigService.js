...
/**
 * {ObjectMap} host Setting object map from host application
 */
var _hostSettings = {};
...
function _setNonHostedDefaults() {
    ...
    _uiOptions[ hostConfigKeys.ALLOW_CACHE_HOST_DATA ] = false;
    ...
    _uiOptions[ hostConfigKeys.DISABLE_HS_ASYNC_SOA ] = true;
    ...
    _uiOptions[ hostConfigKeys.HOST_SESSION_GROUP ] = '';
    _uiOptions[ hostConfigKeys.HOST_SESSION_LOCALE ] = '';
    _uiOptions[ hostConfigKeys.HOST_SESSION_ROLE ] = '';
    _uiOptions[ hostConfigKeys.HOST_SESSION_SECURITY_TOKEN ] = '';
    _uiOptions[ hostConfigKeys.HOST_SESSION_USER_NAME ] = '';
    ...
    _uiOptions[ hostConfigKeys.SHOW_APP_SWITCHER ] = false;
...
function _setHostedDefaults() {
    ...
    _uiOptions[ hostConfigKeys.DISABLE_HS_ASYNC_SOA ] = false;
    _uiOptions[ hostConfigKeys.ENABLE_MCAD_INTEGRATION_HOSTED_COMMANDS ] = false;
    _uiOptions[ hostConfigKeys.HAS_FULL_SCREEN_SUPPORT ] = false;
    _uiOptions[ hostConfigKeys.HIDE_AVATAR ] = false;
    _uiOptions[ hostConfigKeys.HIDE_SEARCH_COMMAND_BUTTON ] = false;
    _uiOptions[ hostConfigKeys.HOST_CLONE_STABLE_ID_PATH_TOP_DOWN ] = false;
    _uiOptions[ hostConfigKeys.HOST_DISCRIMINATOR ] = null;
    _uiOptions[ hostConfigKeys.HOST_SESSION_GROUP ] = '';
    ...
    for( var i = 0; i < settings.length; i++ ) {
        ...
        if( !_.isEmpty( pair ) ) {
            ...
            if( !_.isEmpty( key ) && !_.isEmpty( value ) ) {
                switch ( key ) {
                    case hostConfigKeys.ALLOW_CACHE_HOST_DATA:
                        if ( value ) {
                            _hostSettings = settings;
                            if ( _.isNil( localStorage.get( exports.HOST_CONFIG_SETTINGS ) ) ) {
                                localStorage.publish( exports.HOST_CONFIG_SETTINGS, JSON.stringify( settings ) );
                            }
                        }
                    ...
                    case hostConfigKeys.SHOW_APP_SWITCHER:
                    ...
                    case hostConfigKeys.HOST_SESSION_GROUP:
                    case hostConfigKeys.HOST_SESSION_LOCALE:
                    case hostConfigKeys.HOST_SESSION_ROLE:
                    case hostConfigKeys.HOST_SESSION_SECURITY_TOKEN:
                    case hostConfigKeys.HOST_SESSION_USER_NAME:
                        /**
...
function _useCachedHostConfig() {
    ...
    let settingsArray = [];
    if ( configSettings ) {
        let settings = JSON.parse( configSettings );
        if ( settings ) {
    ...
    if( _.isEmpty( settingsArray ) ) {
        logger.info( 'hostConfigService._useCachedHostConfig: No host configuration settings, use default settings' );
    }
    deferred.resolve( _process_2014_07_Settings( settingsArray ) );
 ...
 * @returns {Promise} Resolved with the {UiOptions} with properties set from the 'host' (or defaulted).
 */
function _useHostConfiguration_2014_07() {
    // Request the configuration from the host
    const hostConfigJsonString = localStorage.get( exports.HOST_CONFIG_SETTINGS );
    if ( hostConfigJsonString ) {
        return new Promise( ( resolve ) => resolve( _process_2014_07_Settings( JSON.parse( hostConfigJsonString ) ) ) );
    }
    return hostCoreSvc2.createRequestHostConfigProxy().callHostMethodAsync().then( function( hostConfig ) {
...
function _onInitialization() {
    ...
    } else if( hostConfigJsonString ) {
        promise = new Promise( ( resolve ) => resolve( _process_2014_07_Settings( JSON.parse( hostConfigJsonString ) ) ) );
    ...
    if( promise ) {
        return promise.then( function( uiOptions ) {
            ...
            if( _debug_logHostCallsToClientDetails ) {
                hostInteropSvc.log( hostInteropSvc.getCurrentTime( ) + 'hostConfigService._onInitialization: _uiOptions:' + '\n' +
            ...
            if ( _uiOptions[ hostConfigKeys.DISABLE_HS_ASYNC_SOA] ) {
                appCtxSvc.ctx.disableHostSOAAfterLogin = true;
            } else {
                appCtxSvc.ctx.disableHostSOAAfterLogin = false;
            }
            ...
            if( _uiOptions[ hostConfigKeys.SEND_TO_SUPPORTED_TYPES_PREFERENCE ] ) {
                const preferenceSubscribe = eventBus.subscribe( 'authentication.complete', function() {
...
/**
 * Key for host configuration setting in local storage
 */
export let HOST_CONFIG_SETTINGS = 'HostConfigSettings';
...
export let mapThemeName = function( themeName ) {
    ...
    } else if( themeName === 'Dark' || themeName === 'dark' || themeName === 'com.siemens.splm.clientfx.ui.darkTheme' || themeName === 'ui-darkTheme' ) {
        return 'ui-darkTheme';
    }
    return 'ui-lightTheme';
};
...
/**
 * Update theme in host configuration setting
 * 
 * @param {String} theme - Theme name given by host application 
 */
export const setTheme = function( theme ) {
    if ( Object.keys( _hostSettings ).length !== 0 ) {
        _uiOptions[ hostConfigKeys.PREFERRED_THEME ] = theme;
        const item = _hostSettings.find( item => item.Key ===  hostConfigKeys.PREFERRED_THEME );
        if ( item ) {
            item.Value = _uiOptions[ hostConfigKeys.PREFERRED_THEME ];
            localStorage.publish( exports.HOST_CONFIG_SETTINGS, JSON.stringify( _hostSettings ) );
        }
    }
};
