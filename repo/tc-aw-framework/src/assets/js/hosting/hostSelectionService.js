 ...
 */
const isPacked = ( item ) => item && item.props && item.props.awb0IsPacked && item.props.awb0IsPacked.dbValues && _.isEqual( item.props.awb0IsPacked.dbValues[ 0 ], '1' );

/**
 * Helper function uses to check if the object can be packed.
 ...
 * @returns  {boolean} True if the object is packed.
 */
const canBeUnpacked = ( ) => appCtxSvc.getCtx( 'occmgmtContext.productContextInfo.uid' ) && appCtxSvc.getCtx( 'selected' ).modelType.typeHierarchyArray.indexOf( 'Awb0Element' ) > -1;
...
export let initialize = function() {
    eventBus.subscribe( 'appCtx.register', handleSelectionEvent );
    eventBus.subscribe( 'appCtx.update', handleSelectionEventForAlignedDesigns );

    function handleSelectionEvent( eventData ) {
        if( eventData.name === 'mselected' ) {
            processSelectionEvent( false );
        }
    }

    function handleSelectionEventForAlignedDesigns( eventData ) {
        if( eventData.name === 'aw_aligned_designs_product' ) {
            processSelectionEvent( true );
        }
    }

    function processSelectionEvent( processAllignedDesignSelection ) {
        /**
        ...
        if( !hostingState || !hostingState.ignoreSelection || processAllignedDesignSelection ) {
            ...
            if( selectedObjs ) {
                ...
                // If the object contains packed lines, we add those packed elements to selectedObjects
                if( canBeUnpacked && selectedObjs.some( isPacked ) ) {
                    // Send an event with the selection list
                    // nxCommandHandlers.js will unpack it for us so we don't add extra dependencies in this module
                    eventBus.publish( 'hosting.packedElementsRequest', selectedObjs );
                    eventBus.subscribe( 'hosting.packedElementsResponse', function( eventData ) {
                        if( eventData && eventData.selectedUIDs && eventData.selectedModelObjs ) {
                            _processSelection( eventData.selectedUIDs, eventData.selectedModelObjs );
                        }
                    } );
                } else {
                    var selectedUIDs = [];
