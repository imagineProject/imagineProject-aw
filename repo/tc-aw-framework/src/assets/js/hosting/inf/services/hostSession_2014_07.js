 ...
 */
HostSessionInfoProxy.prototype.callHostMethodAsync = function( inputData ) {
...
export let getHostSessionUserInfo = function() {
    if ( hostInteropSvc ) {
        ...
        if ( sessionInfo ) { return AwPromiseService.instance.resolve( sessionInfo.UserName && sessionInfo.IsSessionActive ); }
    }
...
export let isHostSessionAccessible = function() {
    ...
    if( hostInteropSvc.getCanTalkWithHost() ) {
        let sessionInfo = hostInteropSvc.getSessionInfo();
        if( sessionInfo && sessionInfo.IsSessionActive ) {
            return true;
        }
