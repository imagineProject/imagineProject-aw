                ...
                dms.loadObjects( objsToLoadArray ).then( function() {
                    ...
                    if( _debug_logRefreshAcvitity ) {
                        logger.info( 'Loading properties: ' + '\n' +
                            ...
                            'objsToRefresh: ' + objsToRefresh.length + '\n' +
                            'updatedUIDsToAnnounce: ' + updatedUIDsToAnnounce.length + '\n' +
                            'createdUIDsToAnnounce: ' + createdUIDsToAnnounce.length );
                    }
                    ...
                    // Register policy based on Dataset type
                    let actionPolicyId = null;
                    var isRegisterPolicy = objsToRefresh && objsToRefresh.length > 0 && objsToRefresh[0].modelType.parentTypeName === _DatasetType;
                    if( isRegisterPolicy ) {
                        var policy = _propertyPolicy;
                        actionPolicyId = propertyPolicySvc.register( policy, 'aw_prop_Policy' );
                    }
