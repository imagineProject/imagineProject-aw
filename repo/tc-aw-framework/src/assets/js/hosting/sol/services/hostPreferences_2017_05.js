...
/**
 * Creates a response proxy and sends out a response message.
 * @param {Array} pairs 
 */
const sendResponse = function( pairs ) {
    ...
    try {
        ...
        preferenceSvc.getMultiStringValues( prefNames ).then( function( response ) {
            ...
            _.forEach( response, function( value, name ) {
                ...
                loadedPreferences[ name ] = true;
            } );

            _.forEach( prefNames, function( pref ) {
                if( !loadedPreferences[ pref ] ) {
                    prefsToLoad.push( pref );
                }
            } );

            if( _.isEmpty( prefsToLoad ) ){
                sendResponse( pairs );
            } else {
                preferenceSvc.getMultiStringValues( _.clone( prefsToLoad ), true ).then( function() {

                    preferenceSvc.getMultiStringValues( prefsToLoad ).then( function( response ) {
