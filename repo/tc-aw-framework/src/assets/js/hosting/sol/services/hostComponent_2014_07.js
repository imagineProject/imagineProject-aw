    ...
    if( componentId ) {
        ...
        if( hostedComponent ) {
            /**
              ...
              */
            appCtxSvc.registerCtx( 'aw_hosting_state.currentHostedComponentId', componentId );
            appCtxSvc.registerCtx( 'aw_hosting_state.currentEmbeddedLocationView', embeddedLocationView );
            ...
            if( hostedComponent.commandId ) {
                ...
                exports.setupComponentContext( _contextObjUidsMap[ componentId ], AwStateService.instance.params,
                    ...
                    function( params ) {
                        ...
                        const host_type = appCtxSvc.getCtx( 'aw_host_type' );
                        if ( host_type === hostConfigValues.HOST_TYPE_NX ) {
                            appCtxSvc.registerCtx( 'locationContext.ActiveWorkspace:Location', [ 'com.siemens.splm.clientfx.tcui.xrt', host_type, 'hostedComponent', location ].join( '.' ) );
                        }

                        if( appCtxSvc.ctx.plcomponent_site ) {
                            appCtxSvc.registerCtx( 'state.params', params );
                            AwStateService.instance.params = params;
                            appCtxSvc.registerCtx( 'host_component_setup', true );
                        } else {
    ...
    if( !_.isEmpty( validUids ) ) {
        return dms.loadObjects( validUids ).then( function() {
            ...
            if( modelObj ) {
                ...
                appCtxSvc.registerCtx( 'hostSelected', vmo );
