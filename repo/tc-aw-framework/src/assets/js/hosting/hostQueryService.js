...
import hostAuthenticatorSvc from 'js/hosting/hostAuthenticatorService';
...
/**
 * This {HostQueryHandler} calls hostAuthenticator signOut when host is Authenticator.
 *
 * @constructor
 * @memberof hostQueryService
 *
 * @extends {hostQueryFactoryService.HostQueryHandler}
 */
const CleanUpHostingCachedDataQueryHandler = function() {
    hostQueryFactorySvc.getHandler().call( this );
};

CleanUpHostingCachedDataQueryHandler.prototype = hostQueryFactorySvc.extendHandler();

/**
 * Handle an incoming query from the 'host'.
 *
 * @memberof hostQueryService.CleanUpHostingCachedDataQueryHandler
 *
 * @param {HostQueryMessage} inputMessage - The input message from the 'host'.
 *
 * @return {HostQueryMessage} The {HostQueryMessage} to send back to 'host' containing any details such as
 * was sign out method called.
 */
CleanUpHostingCachedDataQueryHandler.prototype.handleQuery = function( inputMessage ) {
    const responseData = hostQueryFactorySvc.createEditableData();
    if( AppCtxService.ctx.aw_hosting_host_auth === true ) {
        hostAuthenticatorSvc.signOut();
        responseData.setData( 'isCleanUpCalled', true );
        return hostQueryFactorySvc.createResponseMessage( inputMessage, [ responseData ] );
    }
    responseData.setData( 'isCleanUpCalled', false );
    return hostQueryFactorySvc.createResponseMessage( inputMessage, [ responseData ] );
};
...
/**
 * Return a new instance of CleanUpHostingCachedDataQueryHandler class.
 *
 * @memberof hostQueryService.
 *
 * @returns {CleanUpHostingCachedDataQueryHandler} New initialized instance of this class.
 */
export const createCleanUpHostingCachedDataQueryHandler = function() {
    return new CleanUpHostingCachedDataQueryHandler();
};
...
export default exports = {
    ...
    createCleanUpHostingCachedDataQueryHandler,
    getQueryHandler,
    sendQueriesToHost,
