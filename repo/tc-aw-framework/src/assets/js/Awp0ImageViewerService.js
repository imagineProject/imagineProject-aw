 ...
 */
export const awImageViewerRenderFn = function( { elementRefList, viewModel, subPanelContext } ) {
    let headerPropList = [];
    let headerVisible;
    let viewerHeaderElem;
    let loadingDiv;
    let img;
    let imgViewerRef = elementRefList.get( 'awp0ImageViewer' );

    if( imgViewerRef.current ) {
        imgViewerRef.current.style.height = viewModel.data.viewerHeight;
    }
    ...
    return (
        <div id='aw-image-viewer' className='aw-image-viewer' ref={imgViewerRef}>
