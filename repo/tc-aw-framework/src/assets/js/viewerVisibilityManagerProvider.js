...
export default class ViewerVisibilityManager extends ViewerVisibilityCoreManager {
    ...
    setUpVisibilityListener( viewerContextData, viewerView ) {
        ...
        function _handleVisibilityOfOccurrencesFromViewer( occsFromViewer, visibilityToSet, isStateChange ) {
            ...
            _.forEach( occsFromViewer, function( occurrence ) {
                if( occurrence.type === 1 || occurrence.type === 4 ) {
            ...
            let visibilityData = {
                ...
                invisibleExceptionPartitionIds: Object.values( self.invisibleExceptionPartitionIds )
            };
    ...
    setPartitionPartsVisibility( partitionCsidChains, isVisible, notifyChange ) {
        ...
        if( _.includes( betaPrefValues, 'enableServerless' ) ) {
            return; //PVW DO NOT HAVE ANY PARTITIONS DATA.
        }
    ...
    _notifyViewerVisibilityChanged( occurrencesFromViewer, visibilityToSet, isStateChange ) {
        ...
        if( this.viewerVisibilityChangedListeners.length > 0 ) {
            _.forEach( this.viewerVisibilityChangedListeners, function( observer ) {
                observer( occurrencesFromViewer, visibilityToSet, isStateChange );
            } );
        }
    }
    ...
    /**
     * Updates visibility state cache
     * @param {[String]} invisibleCsids Array of invisible csid strings
     * @param {[String]} invisibleExceptionCsids Array of invisible exception csid strings
     */
    updateVisibleStateCache( invisibleCsids, invisibleExceptionCsids, invisiblePartitionIds, invisibleExceptionPartitionIds ) {
        if ( invisibleCsids && Array.isArray( invisibleCsids ) && invisibleCsids.length > 0 ) {
            invisibleCsids.forEach( csid => {
                if ( !this.invisibleCsids.includes( csid ) ) {
                    this.invisibleCsids.push( csid );
                }
            } );
        }
        if ( invisibleExceptionCsids && Array.isArray( invisibleExceptionCsids ) && invisibleExceptionCsids.length > 0 ) {
            invisibleExceptionCsids.forEach( csid => {
                if ( !this.invisibleExceptionCsids.includes( csid ) ) {
                    this.invisibleExceptionCsids.push( csid );
                }
            } );
        }
        if ( invisiblePartitionIds && Array.isArray( invisiblePartitionIds ) && invisiblePartitionIds.length > 0 ) {
            invisiblePartitionIds.forEach( prtnId => {
                if ( !this.invisiblePartitionIds.includes( prtnId ) ) {
                    this.invisiblePartitionIds.push( prtnId );
                }
            } );
        }
        if ( invisibleExceptionPartitionIds && Array.isArray( invisibleExceptionPartitionIds ) && invisibleExceptionPartitionIds.length > 0 ) {
            invisibleExceptionPartitionIds.forEach( prtnId => {
                if ( !this.invisibleExceptionPartitionIds.includes( prtnId ) ) {
                    this.invisibleExceptionPartitionIds.push( prtnId );
                }
            } );
        }
    }
    ...
    restoreViewerVisibility( invisibles, invisibleExceptions, invisiblePartitions, invisibleExceptionPartitions ) {
        ...
        this.updateVisibleStateCache( invisibles, invisibleExceptions, invisiblePartitions, invisibleExceptionPartitions );
     ...
     * @returns {Promise} promise thats resolved when Clean 3D View in done
     */
