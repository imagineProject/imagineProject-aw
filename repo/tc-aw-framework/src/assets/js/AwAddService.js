...
import AddObjectPrimarySub from 'viewmodel/AddObjectPrimarySubViewModel';
...
export const awAddRenderFunction = ( {
    ...
    // default policy is being overridden by subPanelContextIn.policy.
    // Ideally it should be merged. This is causing issue with configuredRev behavior which expects revision_list prop.
    // This worked fine in 6.3 as subPanelContext did not have policy.
    // Extract the policy from subPanelContextIn
    if( typeof subPanelContextIn.policy === 'object' ) {
        let subPanelPolicy = subPanelContextIn.policy || { types: [] };

        // Merge the policies
        let mergedPolicy = {
            types: defaultPolicy.types.map( defaultType => {
                let subPanelType = ( subPanelPolicy.types || [] ).find( type => type.name === defaultType.name );
                if ( subPanelType ) {
                    return {
                        name: defaultType.name,
                        properties: [ ...defaultType.properties, ...subPanelType.properties ]
                    };
                }
                return defaultType;
            } )
        ...
        // Add any additional types from subPanelPolicy that are not in defaultPolicy
        ( subPanelPolicy.types || [] ).forEach( subPanelType => {
            if ( !( defaultPolicy.types || [] ).find( type => type.name === subPanelType.name ) ) {
                mergedPolicy.types.push( subPanelType );
            }
        } );
        context = { ...context, policy: mergedPolicy };
    }


    return <AddObjectPrimarySub subPanelContext={context}></AddObjectPrimarySub>;
};
...
export const getAddTabModels = ( vmDef, prop, data ) => {
    ...
    return [ new DerivedStateResult( {
        ...
        compute: ( renderContext, visibleTabs, defaultTab, optionalTabs ) => {
            ...
            let optionalTabsData = optionalTabs ? Object.values( optionalTabs ) : [];
