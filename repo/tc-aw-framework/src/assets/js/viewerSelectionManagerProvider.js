...
const SYNC_SELECTION_WITH_ACE_TREE = 'syncSelectionWithAceTree';
...
class ViewerSelectionManager {
    ...
    initialize() {
        this.viewerView.selectionMgr.addSelectionListener( occurrences => {
            ...
            if( occurrences ) {
                ...
                if( !_.isEmpty( occCSIDChains ) || _.isEmpty( occurrences ) ) {
     ...
     */
    selectPartsInViewerUsingCsidWithPartitions( csidChainArray, partitionChainArray, partitionsMemberCSIDArray ) {
        var occurrences = [];
        ...
        if( Array.isArray( partitionsMemberCSIDArray ) && partitionsMemberCSIDArray.length > 0 ) {
            csidChainArray = [ ...csidChainArray, ...partitionsMemberCSIDArray ];
        }
    ...
    checkIfModelObjectSelectionsAreEqual( modelObjectsArray ) {
        ...
        if( !currentlySelectedModelObjects || _.xorBy( modelObjectsArray, currentlySelectedModelObjects,
            ViewerSelectionManager.returnUIDFromModelObjects ).length !== 0 ) {
            return false;
        }
        return true;
    }
    ...
    notifyViewerSelectionChanged( occCSIDChains, bomLineSRUIDs ) {
        ...
        if( this.viewerSelectionChangedListeners.length > 0 ) {
            _.forEach( this.viewerSelectionChangedListeners, observer => {
                observer( occCSIDChains, bomLineSRUIDs );
            } );
        }
    }
    ...
    setContext( csidChainArray ) {
        ...
        if( this.viewerContextData.getThreeDViewManager() ) {
