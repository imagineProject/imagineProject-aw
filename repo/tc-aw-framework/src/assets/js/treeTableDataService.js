...
/**
 * Copy properties to the selection model objects.
 *
 * @param {Object} uwDataProvider The data provider object.
 */
const copyPropertiesToSelectionModel = function( uwDataProvider ) {
    if ( uwDataProvider?.selectionModel?.selectionData ) {
        const selectedObjects = uwDataProvider.getSelectedObjects();
        const selectionModelSelected = uwDataProvider.selectionModel.selectionData.selected;
        if ( selectedObjects?.length > 0 ) {
            for ( let i = 0; i < selectedObjects.length; i++ ) {
                for ( let j = 0; j < selectionModelSelected.length; j++ ) {
                    if ( !selectionModelSelected[j].props && selectedObjects[i].uid === selectionModelSelected[j].uid ) {
                        selectionModelSelected[j].props = selectedObjects[i].props;
                        break;
                    }
                }
            }
...
export let loadTreeTablePropertiesOnInitialLoad = function( vmNodes, declViewModel, uwDataProvider, context, contextKey, updateColumnPropsCallback, overriddenPropertyPolicy ) {
    ...
    let _propertyLoadResult = awTableTreeSvc.createPropertyLoadResult( vmNodes );
...
function getChildNodes( propertyLoadInput ) {
    ...
    _.forEach( propertyLoadInput.propertyLoadRequests, function( propertyLoadRequest ) {
        _.forEach( propertyLoadRequest.childNodes, function( childNode ) {
            ...
            if ( childNode.fetchRemainingProps && cdmSvc.isValidObjectUid( childNode.uid ) ) {
                allChildNodes.push( childNode );
            }
        } );
    } );
...
export let loadTreeTableProperties = function() {
    ...
    if( uwDataProvider && !_.isUndefined( uwDataProvider.columnConfigLoadingInProgress ) ) {
        let _propertyLoadResult = {};
        if ( declViewModel.dataProviders && declViewModel.dataProviders.objNavTreeDataProvider ) {
            _propertyLoadResult.updatedNodes = getChildNodes( propertyLoadInput );
        }
...
function _loadProperties( propertyLoadInput, propertyLoadContext, contextKey, declViewModel, uwDataProvider, updateColumnPropsCallback, overriddenPropertyPolicy ) {
    ...
    let allChildNodes = getChildNodes( propertyLoadInput );
    ...
    let _propertyLoadResult = awTableTreeSvc.createPropertyLoadResult( allChildNodes );
    ...
    return tcVmoService.getTableViewModelProperties( allChildNodes, propertyLoadContextLcl, overriddenPropertyPolicy ).then(
        function( response ) {
            if( response && !declViewModel.isDestroyed() ) {
                ...
                var columnConfigResult = updateColumnPropsCallback.callUpdateColumnPropsAndNodeIconURLsFunction( propColumns, allChildNodes, contextKey, response, uwDataProvider );
                _propertyLoadResult.columnConfig = columnConfigResult;
                copyPropertiesToSelectionModel( uwDataProvider );
            }
