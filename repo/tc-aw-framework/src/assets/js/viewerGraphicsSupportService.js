 ...
 */
export let getRenderSupportInfoFromJscom = () => {
    ...
    return window.JSCom.Health.HealthUtils.getServerHealthInfo( frameAdapterService.getConnectionUrl() ).then( ( gpuInfo ) => {
        if( Array.isArray( gpuInfo.poolManagers ) && gpuInfo.poolManagers.length > 0 ) {
            for ( const poolMgrKey in gpuInfo.poolManagers ) {
                const poolMgr = gpuInfo.poolManagers[poolMgrKey];
                if( poolMgr.graphicsCapable > 0 ) {
...
export let getSupportedRenderLocation = async( modelObject, isSameProductOpenedAsPrevious ) => {
    let modelUid = viewerContextService.getModelObjectUid( modelObject );
        ...
        if( !isSameProductOpenedAsPrevious ) {
            ...
            if( !renderSupportInfo ) {
                if( !viewerContextService.isServerless() ) {
                    return getRenderSupportInfoFromJscom().then( ( renderSupportInfo ) => {
                ...
                //not saving to session if we are in serverless mode as we are setting SSR to false and CSR to true ( if hardware is supported)
                return _getSupportedRenderLocation( renderSupportInfo, renderLocationFromIndexedDb, renderLocationFromTcPref );
            }
            return _getSupportedRenderLocation( renderSupportInfo, renderLocationFromIndexedDb, renderLocationFromTcPref );
        }
