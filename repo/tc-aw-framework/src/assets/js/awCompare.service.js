 ...
 */
export const getViewModelObjects = function( modelObjects, columns ) {
    if( !columns ) {
    ...
    const propNames = columns.map( function( col ) {
        return col.propertyName || col.propDescriptor.propertyName;
    } );

    const viewModelObjects = modelObjects.map( function( modelObject ) {
        let vmo = viewModelObjectService.constructViewModelObjectFromModelObject( modelObject );
        Object.defineProperty( vmo, 'displayName', {
            get() {
                if( this.props.object_string ) {
                    return this.props.object_string.uiValues[ 0 ];
                } else if( this.props.object_name ) {
                    return this.props.object_name.uiValues[ 0 ];
                }
                return '';
            }
        } );
    ...
    return tcViewModelObjectService.getViewModelProperties( viewModelObjects, propNames ).then( function( result ) {
        if( result && result.objects && result.logicalObjects ) {
            putClsData( result.objects, result.logicalObjects );
            return {
 ...
 */
export let getTypesAndCallSoa = function( soaInput, modelObjects, columnProvider ) {
    ...
    return tcDataManagementService.baseGetOrResetUIColumnConfigs( soaInput ).then( function( result ) {
        result.types = types;
        return result;
    } );
};
...
export let putClsData = function( viewModelObjects, objects ) {
    ...
    for( var obj in objects ) {
        var clsProp = {};
        for( var currentProp in objects[ obj ].props ) {
            if( objects[ obj ].props[ currentProp ].propertyName.startsWith( 'CLS_ATTR:' ) ) {
                if( objects[ obj ] && objects[ obj ].props[ currentProp ] && objects[ obj ].props[ currentProp ].uiValues &&
                    objects[ obj ].props[ currentProp ].dbValues ) {
                    clsProp[ objects[ obj ].props[ currentProp ].propertyName ] = objects[ obj ].props[ currentProp ];
                }
            }
        }
        vmoProps[ objects[ obj ].uid ] = clsProp;
    }
    for( var vmo in viewModelObjects ) {
        var clsPropList = vmoProps[ viewModelObjects[ vmo ].uid ];
        for( var prop in clsPropList ) {
            viewModelObjects[ vmo ].props[clsPropList[prop].propertyName] = clsPropList[prop];

            if( clsPropList[ prop ].uiValues && clsPropList[ prop ].uiValues.length === 1 ) {
                viewModelObjects[ vmo ].props[ prop ].uiValue = viewModelObjects[ vmo ].props[ prop ].uiValues[ 0 ];
            }
            if( clsPropList[ prop ].dbValues && clsPropList[ prop ].dbValues.length === 1 ) {
