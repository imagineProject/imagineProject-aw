...
export const createNameValuePropertyComponent = function( vmDef, props ) {
    return [ new DerivedStateResult( {
        ctxParameters: [],
        additionalParameters: [ props.nameValuePropertyData, props.columns, props.firstPageUids, props.objects, props.editContextKey, props.operationName, props.enablePropEdit ],
        compute: ( renderContext, nameValuePropertyData, columns, firstPageUids, objects, editContextKey, operationName, enablePropEdit ) => {
            ...
            const viewModel = abstractTablePropertyService.createDynamicTablePropertyViewModel( uniqueName, objectSetSource, parentUid, columns, firstPageUids, objects, editContextKeyIn,
                props.selectionData, operationName, sortCriteria, enablePropEdit );
            ...
            const Component = createComponent( viewModel, ( props ) => {
                ...
                const commandContext = {
                    ...
                    isColumnFilterApplied: viewModel.dataProviders[providerName]?.isColumnFilterApplied
                };

                const header = <div className='aw-widgets-nameValueLabel'>{displayName}:</div>;

                if( props.dynamicTableStyleRef && props.dynamicTableStyleRef.current && props.dynamicTableStyleRef.current.style ) {
                    if( viewModel.data.tableWidth ) {
                        let tableWidth = viewModel.data.tableWidth;
                        if ( !isNaN( tableWidth ) ) {
                            tableWidth = `${tableWidth}px`;
                        }
                        props.dynamicTableStyleRef.current.style.width = tableWidth;
                    }
                    if( viewModel.data.tableHeight ) {
                        let tableHeight = viewModel.data.tableHeight;
                        if ( !isNaN( tableHeight ) ) {
                            tableHeight = `${tableHeight}px`;
                        }
                        props.dynamicTableStyleRef.current.style.maxHeight = tableHeight;
                    }
                }
                ...
                return (
                    <div className='sw-component aw-xrt-tablePropContent aw-xrt-tablePropertyContainer'>
                        ...
                        <div className='aw-layout-flexColumn aw-base-scrollPanel' ref={props.dynamicTableStyleRef}>
...
export const awNameValuePropertyRenderFunction = function( props ) {
    const { nameValuePropertyData, selectionData, editContextKey, dpRef, elementRefList, ctxMin: { dynamicTable } } = props;
    const AwDynamicNameValueProperty = dynamicTable[ 0 ];
    const dynamicTableStyleRef = elementRefList.get( 'dynamicTableStyle' );
    return <AwDynamicNameValueProperty nameValuePropertyData={nameValuePropertyData} selectionData={selectionData} editContextKey={editContextKey} dpRef={dpRef}
        dynamicTableStyleRef={dynamicTableStyleRef}></AwDynamicNameValueProperty>;
};
