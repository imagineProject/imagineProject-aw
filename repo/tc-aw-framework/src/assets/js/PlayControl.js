// Copyright 2025 Siemens Product Lifecycle Management Software Inc.
/**
...
export function newInstance( viewerContainer ) {
    ...
    function seekStart( e ) {
        // start seeking from anywhere on the timeline (mouse or touch)
        var x = eventOffsetX( e );
        var t = xToT( x - 6 );

        seeking = true;
        e.preventDefault();

        // If user double-clicks or clicks near a tick, allow adjusting that tick
        ...
        // ensure we capture move/end even if pointer leaves the timeline
        var doc = container.ownerDocument;
        doc.addEventListener( 'mousemove', seekMove );
        doc.addEventListener( 'mouseup', seekEnd );
        doc.addEventListener( 'touchmove', seekMove );
        doc.addEventListener( 'touchend', seekEnd );

        // Immediately update time to where user clicked
        setCurrentTime( Math.max( Math.min( t, duration ), 0 ), true );
    }
    ...
    function seekMove( e ) {
        if( seeking ) {
            e.preventDefault();
            var x = eventOffsetX( e );
            var t = Math.max( Math.min( xToT( x - 6 ), duration ), 0 );
    ...
    function seekEnd() {
        ...
        // remove document-level listeners added during seek
        var doc = container.ownerDocument;
        doc.removeEventListener( 'mousemove', seekMove );
        doc.removeEventListener( 'mouseup', seekEnd );
        doc.removeEventListener( 'touchmove', seekMove );
        doc.removeEventListener( 'touchend', seekEnd );
    }

    /**
     * Get pointer X relative to timeLine (supports mouse and touch)
     * @param {Event} e - the event
     * @returns {Number} offsetX relative to timeLine
     */
