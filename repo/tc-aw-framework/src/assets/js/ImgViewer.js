...
export function newInstance( viewerContainer ) {
        ...
        if( url.match( /\.svg$/i ) ) {
            loadBlob( url, 'image/svg+xml;charset=utf-8', function( blobUrl ) {
                ...
                img.onerror = function() {
                    // we should replace the broken image with a placeholder here
                    console.log( 'Error loading SVG image from URL:' + url );
                };
                img.src = blobUrl;
            } );
     ...
     * @returns {Object} the vpfit global variable
     */
     ...
     *
     * @param {Object} e - the event
     * @returns {Object} the return value of panStart
     */
     ...
     *
     * @param {Object} e - the event
     * @returns {Object} the return value of panMove
     */
    ...
    function fixSvg( svgDoc ) {
        var svgHeads = svgDoc.match( /<svg[^>]*>/g );
        if( svgHeads ) {
            var head = svgHeads[0];
            var width = head.match( /width="[0-9.]+[a-zA-Z]*"/ );
            var height = head.match( /height="[0-9.]+[a-zA-Z]*"/ );
            var viewBox = head.match( /viewBox="([0-9. ]+)"/ );
            var isMultiPageSvg = svgHeads.length > 1;
            if( isMultiPageSvg ) {
                // Possible multi-page SVG, one way to tell is if every other child SVG element has a viewBox attribute
                for( var i = 1; i < svgHeads.length; ++i ) {
                    var vbCheck = svgHeads[i].match( /viewBox="([0-9. ]+)"/ );
                    if( !vbCheck ) {
                        isMultiPageSvg = false;
                        break;
                    }
                }
                if( isMultiPageSvg ) {
                    // We have to massage the SVG data to have it display properly
                    return fixMultiPageSvg( svgDoc, svgHeads );
                }
            }

            // fix case where there is a viewBox attribute but no height or width
    ...
    /**
     * Fix multi page SVG by replacing child SVG elements with "g" (group) elements.
     *
     * @param {String} svgDoc - the SVG document in text format
     * @param {Array} svgHeads - array of the document's SVG elements
     * @returns {String} the fixed SVG document
     */
    function fixMultiPageSvg( svgDoc, svgHeads ) {
        // first ensure that the top level SVG element has the namespace attribute
        let newHead = svgHeads[0];
        let nsAttr = svgHeads[0].match( /xmlns="http:\/\/www\.w3\.org\/2000\/svg"/ );
        if( !nsAttr ) {
            newHead = svgHeads[0].replace( /<svg /, '<svg xmlns="http://www.w3.org/2000/svg" ' );
        }

        // get the heights and widths of each page; we will stack them vertically
        let finalWidth = 0;
        let finalHeight = 0;
        let allHeights = [];
        let units = 'px';
        for( var i = 1; i < svgHeads.length; ++i ) {
            let viewBoxAttr = svgHeads[i].match( /viewBox="([0-9. ]+)"/ );

            if( viewBoxAttr ) {
                let vboxVals = getNumbersFromString( viewBoxAttr[0] );
                // width will be index #2, height #3
                if( vboxVals.length === 4 ) {
                    let vbWidthVal = parseFloat( vboxVals[2] );
                    let vbHeightVal = parseFloat( vboxVals[3] );
                    if( vbWidthVal > finalWidth ) {
                        finalWidth = vbWidthVal;
                    }
                    finalHeight += vbHeightVal;
                    allHeights.push( vbHeightVal );
                }
            } else {
                let widthAttr = svgHeads[i].match( /width="[0-9.]+[a-zA-Z]*"/ );
                if( widthAttr ) {
                    let widthVal = getNumbersFromString( widthAttr[0] );
                    if( widthVal > finalWidth ) {
                        finalWidth = widthVal;
                    }
                }
                let heightAttr = svgHeads[i].match( /height="[0-9.]+[a-zA-Z]*"/ );
                if( heightAttr ) {
                    let heightVal = getNumbersFromString( heightAttr[0] );
                    finalHeight += heightVal;
                    allHeights.push( heightVal );
                }
            }
        }
        ...
        // now replace or add the width, height, and viewBox attributes to the head SVG element
        let widthAttr = svgHeads[0].match( /width="[0-9.]+[a-zA-Z]*"/ );
        let heightAttr = svgHeads[0].match( /height="[0-9.]+[a-zA-Z]*"/ );
        let viewBoxAttr = svgHeads[0].match( /viewBox="([0-9. ]+)"/ );

        let replaceStr = widthAttr ? widthAttr[0] : />/;
        let endStr = widthAttr ? '' : '>';
        newHead = newHead.replace( replaceStr, ' width="' + finalWidth + units + '" ' + endStr );
        replaceStr = heightAttr ? heightAttr[0] : />/;
        endStr = heightAttr ? '' : '>';
        newHead = newHead.replace( replaceStr, ' height="' + finalHeight + units + '" ' + endStr );
        replaceStr = viewBoxAttr ? viewBoxAttr[0] : />/;
        endStr = viewBoxAttr ? '' : '>';
        newHead = newHead.replace( replaceStr, ' viewBox="0 0 ' + finalWidth + ' ' + finalHeight + '" ' + endStr );
        let newSvgDoc = svgDoc.replace( svgHeads[0], newHead );

        // now we will replace the child <svg> elements with <g> elements
        // after the first 'g' element, we will add a translation command to place it vertically under the previous 'page'
        let currentHeight = 0;
        for( var j = 1; j < svgHeads.length; ++j ) {
            currentHeight += j === 1 ? 0 : allHeights[j - 2];
            let groupStr = j === 1 ? '<g>' : '<g ' + ' transform="translate(0,' + currentHeight + ')">';
            newSvgDoc = newSvgDoc.replace( svgHeads[j], groupStr );
        }

        // have to also replace the closing </svg> tags
        newSvgDoc = newSvgDoc.replace( /<\/svg>/g, '</g>' );
        // that will also replace the head </svg>
        let lastIdx = newSvgDoc.lastIndexOf( '</g>' );
        newSvgDoc = newSvgDoc.slice( 0, lastIdx - 1 );
        newSvgDoc += '</svg>';
    ...
    /**
     * Extracts one or more number values from a string.
     * @param {String} attrValue the string value
     * @returns {*} undefined if there are no matches, the number if there is only one value, and an array of numbers if there are multiple values
     */
    function getNumbersFromString( attrValue ) {
        let strNumValues = attrValue.match( /[0-9.]+/g );
        if( !strNumValues ) {
            return undefined;
        } else if ( strNumValues.length === 1 ) {
            return parseFloat( strNumValues[0] );
        }

        let numArray = [];
        for( let i = 0; i < strNumValues.length; ++i ) {
            numArray.push( parseFloat( strNumValues[i] ) );
        }
        return numArray;
    }
