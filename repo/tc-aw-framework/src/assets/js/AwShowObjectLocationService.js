...
export const syncOpenedObject = async( viewModelObject, eventData ) => {
    ...
    const sourceUids = viewModelObjectSvc.processSourceUids( viewModelObject );

    if( affectedObjected ) {
        for( let i in affectedObjected ) {
            if( affectedObjected[ i ].uid === viewModelObject.uid || Object.values( sourceUids ).includes( affectedObjected[ i ].uid ) ) {
    ...
    let affectedVmo = viewModelObjectSvc.updateViewModelObjectFromModelObjects( viewModelObject, affectedObjected );
    let updatedVmo = affectedVmo ? affectedVmo : viewModelObject;

    const displayName = await getDisplayName( updatedVmo );
    return {
        viewModelObject : { ...updatedVmo },
        displayName
    };
};
...
export const handleObjectDeletedEvent = async( viewModelObject, showObjContext, eventData ) => {
    ...
    if( isOpenedObjectDeleted ) {
        const errorMsg = await getLocalizedTextFromKey( 'XRTMessages.objectNotFound' );
        const displayName = await getLocalizedTextFromKey( 'XRTMessages.ShowObjectLocationTitle' );
        return { showObjContext: { errorMsg, displayName } };
    }
    return { showObjContext };
};

export const unmount = ( preSaveActionID ) => {
    _activeParams = [];
...
const synchronizeState = ( showObjContext, viewModelObject, data ) => {
    ...
    if ( changedParams.hasOwnProperty( 'page' ) || changedParams.hasOwnProperty( 'pageId' ) ) {
        if ( appCtxService?.ctx?.state?.urlAttributes?.relationsTree  && showObjContext.activeTab.hasChildTabs ) {
            const selectedTabExists = showObjContext.subLocationTabs?.some(
                ( tab ) => tab.id === changedParams.pageId
            );
            if ( !selectedTabExists ) { // selected tab doesn't exist means it is part of active Tab(e.g. Details), it will be handled by same.
                return;
            }
        }
...
const _processDeclStyleSheetResponse = ( viewModelJsonString, selectedObject ) => {
    ...
    if( appCtxService?.ctx?.state?.urlAttributes?.relationsTree ) {
        delete xrtViewModel.data._headers;
    }
    ...
    _.forEach( xrtViewModel.dataProviders, function( dataProvider ) {
        if( dataProvider?.action && xrtViewModel.actions[ dataProvider.action ]?.inputData ) {
            let searchInput = tcDataManagementService.getSearchInputFromActionInputData( xrtViewModel.actions[ dataProvider.action ].inputData );
            if( searchInput ) {
...
const openNewModelObject = ( uid, data ) => {
    ...
    if( !modelObject ) {
        return dataManagementService.loadObjects( [ uid ] ).then( function() {
            ...
            return getLocalizedTextFromKey( 'XRTMessages.FailureLoadingSummary' ).then( ( errorMsg )=>{
                return getLocalizedTextFromKey( 'XRTMessages.ShowObjectLocationTitle' ).then( ( displayName )=>{
...
const getDisplayName = async( modelObject ) => {
    ...
    return getLocalizedTextFromKey( 'XRTMessages.ShowObjectLocationTitle' );
};
...
const refreshModelObject = async( modelObject, data ) => {
    ...
    return xrtParserService.getDeclStyleSheet( 'SUMMARY', pageId, modelObject ).then(
        async function( response ) {
            ...
            let uid = modelObject ? modelObject.uid : Object.keys( xrtViewModel.data.objects )[ 0 ];
            let viewModelObject =  xrtUtils.prepareXrtViewModelObject( uid, xrtViewModel, xrtViewModel.data?.operationName, null, null );
...
const getSelectTabApi = ( updatedSubLocationTabs, displayName, xrtViewModel, activeTab, viewModelObject, data, provider ) => {
    let api = ( id, tabTitle, tabsModel ) => {
        if( !id && !tabTitle ) {
            selectTab( { subLocationTabs: updatedSubLocationTabs, displayName, xrtViewModel, activeTab, api, provider: provider }, viewModelObject, undefined, data ).then( function(
                updatedObject ) {
    ...
    if( visiblePages ) {
        tabs = visiblePages.map( ( page, index ) => {
            return {
                ...
                hasChildTabs: page.hasChildTabs // This flag ensures show object location does not handle pageId sync with URL and vice versa for this Tab, it will be handled by that specific tab.
            };
        } );
    }
    if( tabs ) {
...
const getLatestTabsDetailsFromXRTVM = ( xrtViewModel ) => {
    Object.values( xrtViewModel.dataProviders ).forEach( ( dp ) => {
        dp.inputData = {
    ...
    visiblePages = visiblePages.filter( ( visiblePage )=>{
        if( !appCtxService?.ctx?.state?.urlAttributes?.relationsTree && visiblePage.pageNameToken ) {
            return visiblePage.pageNameToken !== 'AwRelationsSublocation';
        }
        return true;
    } );
    let pageToSelect;
    if( appCtxService?.ctx?.state?.urlAttributes?.relationsTree ) {
        ( { visiblePages, pageToSelect } = processPagesBasedOnType( visiblePages ) );
    }
    let subLocationTabs = buildSublocationTabs( visiblePages );
    return getUpdatedSublocTabsAndActiveTab( subLocationTabs, pageToSelect );
};

const  processPagesBasedOnType = ( visiblePages ) => {
    let xrtBucket = [];
    let pageNameTokenBucket = [];
    // If 1st page is of type pageNameToken and then ensure it is getting selected irrespective of its position with Details tab
    let pageToSelect = !AwStateService.instance.params.pageId && visiblePages?.[0]?.pageNameToken ? visiblePages[0].titleKey : undefined;
    visiblePages?.map( ( visiblePage )=>{
        if( visiblePage.pageNameToken ) {
            pageNameTokenBucket.push( visiblePage );
        }else{
            xrtBucket.push( visiblePage );
        }
    } );
    if ( xrtBucket.length > 0 ) {
        let XRTMessages = localeSvc.getLoadedText( 'XRTMessages' );
        let detailsSublocationTab = {
            displayTitle: XRTMessages.Details,
            pageNameToken: 'AwDetailsSublocation',
            title: XRTMessages.Details,
            titleKey: 'tc_xrt_Details',
            hasChildTabs: true
        };
        pageNameTokenBucket.unshift( detailsSublocationTab );
    }
    // This is temporary change and should be removed once this is contributed from Server XRT via LCS-1146754
    // Starts here
    let vmo = clientDataModel.getObject( AwStateService.instance.params.uid );
    const targetTypes = [ 'ItemRevision', 'Item' ];
    const hasTargetType = targetTypes.some( type => vmo.modelType.typeHierarchyArray.includes( type ) );
    if( hasTargetType && pageNameTokenBucket.filter( ( page ) => page.pageNameToken === 'AwRelationsSublocation' ).length === 0 ) {
        let XRTMessages = localeSvc.getLoadedText( 'XRTMessages' );
        let relationsSublocationTab = {
            displayTitle: XRTMessages.Relations,
            pageNameToken: 'AwRelationsSublocation',
            title: XRTMessages.Relations,
            titleKey: 'tc_xrt_RelationsTree'
        };
        pageNameTokenBucket.push( relationsSublocationTab );
    }
    if( hasTargetType && pageNameTokenBucket.filter( ( page ) => page.pageNameToken === 'AwDetailsSublocation' ).length === 0 ) {
        let XRTMessages = localeSvc.getLoadedText( 'XRTMessages' );
        let relationsSublocationTab = {
            displayTitle: XRTMessages.Details,
            pageNameToken: 'AwDetailsSublocation',
        ...
        pageNameTokenBucket.unshift( relationsSublocationTab );
    }
    // Ends here
    return { visiblePages: pageNameTokenBucket, pageToSelect: pageToSelect };
};

const getUpdatedSublocTabsAndActiveTab = ( subLocationTabs, pageToSelect ) => {
    let localSublocTabs = Array.from( subLocationTabs );
    const stateParams = AwStateService.instance.params;
    let tabId = getMatchingTabIndex( pageToSelect || stateParams.pageId, stateParams.page, localSublocTabs );
...
export const getSubLocationContext = ( vmDef, props ) => {
...
export const syncUrlWithActiveTab = ( activeTab ) => {
    if (  appCtxService?.ctx?.state?.urlAttributes?.relationsTree && activeTab && !activeTab.hasChildTabs && activeTab.id !== AwStateService.instance?.params?.pageId ) {
        AwStateService.instance.go( '.', { pageId: activeTab.id } );
    }
};

export const registerPreSaveAction = ( ) => {
    let preSaveActionID;
    let activeEditHandler = editHandlerService.getActiveEditHandler();
    if( activeEditHandler && activeEditHandler.registerPreSaveAction ) {
