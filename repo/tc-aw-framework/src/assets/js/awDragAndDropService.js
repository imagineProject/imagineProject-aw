...
const getXrtTableUid = ( dragAndDropParams, props ) => {
    if( dragAndDropParams.targetObjects && dragAndDropParams.targetObjects.length > 0 && props.vmo ) {
        if( dragAndDropParams.targetObjects[0].uid !== props.vmo.uid ) {
            return dragAndDropParams.targetObjects[0].uid;
        }
    }
    if( props && props.objectSetData.source ) {
        return props.vmo.uid + '.' + dragAndDropParams.dataProvider.name + '_' + dragAndDropParams.dataProvider._modelId;
    }
    return null;
};
...
var _deselectAllAndPasteSourceObjects = function( targetElement, targetVMO, sourceObjects, callbackAPIs, xrtTableUid, dragAndDropParams, props ) {
    if( sourceObjects && sourceObjects.length > 0 ) {
        ...
        if( !validSourceTypes || _.isEmpty( validSourceTypes ) ) {
            ...
            let adaptedObjs = isXrtListOrTableContainer( dragAndDropParams, dragAndDropParams.props ) ? adapterSvc.getAdaptedObjectsSync( [ targetVMO ] ) : null;
            let targetObject = adaptedObjs && adaptedObjs[0] ? adaptedObjs[0] : targetVMO;
            _pasteSourceObjects( targetObject, sourceObjects, callbackAPIs, validSourceTypes, dragAndDropParams );
        }
    }
};
...
var _setValidDropTypesOnTarget = function( dragAndDropParams, props ) {
    ...
    if( targetVMO ) {
        let uid = targetVMO.uid;
        if(  dataProvider && dataProvider.validSourceTypes  ||  props.objectSetData && props.objectSetData.source  ) {
            uid = getXrtTableUid( dragAndDropParams, props );
        }
        if( checkValidTypes( uid ) ) {
            return;
        }
        if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
            adapterSvc.getAdaptedObjects( [ targetVMO ] ).then( function( adaptedObjs ) {
        ...
        } else if( dataProvider && dataProvider.validSourceTypes ) {
        ...
        } else if( props.objectSetData && props.objectSetData.source ) {
 ...
 */
var _pasteFiles = function( pasteInput, objectSetState ) {
    const pasteFiles = () => {
        return pasteFileHandler.pasteFilesWithHandler( pasteInput, objectSetState ).then( function( response ) {
...
const executeFileDrop = ( targetVMO, callbackAPIs, pasteRelation2SourceObjectsMap, dragAndDropParams ) => {
    ...
    _pasteFiles( pasteInput, dragAndDropParams?.props?.objectSetState ).then( function( result ) {
...
const _pasteSourceObjects = function( targetVMO, sourceObjects, callbackAPIs, validSourceTypes, dragAndDropParams ) {
     ...
     * type.
     */
    let sourceType2RelationMap = _createSourceType2RelationsMap( validSourceTypes );
    let actionSearchInput = tcDataManagementService.getSearchInputFromActionInputData( dragAndDropParams.dataProvider?.action?.inputData );
    let confRevFlag = actionSearchInput?.searchCriteria ? actionSearchInput.searchCriteria.showConfiguredRev : undefined;
    const isPasteOnObjectSetView = appCtxSvc.getCtx( 'pselected' )?.uid === targetVMO.uid;

    /**
     * configured revision should be evaluated when pasting on the primary object i.e.
     * PWA object or directly on objectset view. If pasting on another object inside an objectset,
     * then the target object is not the primary object (or opened object). In that case,
     * objectSetData.showConfiguredRev is not applicable as it is specific to the primary object
     *
     * Use case eg: Pasting a document on an item which is under Contents section of a folder
     */
    if( isPasteOnObjectSetView ) {
    ...
    let pasteRelation2SourceObjectsMap = {};

    if( sourceObjects ) {
        for( let i = 0; i < sourceObjects.length; i++ ) {
            let sourceObject = sourceObjects[ i ];

            /**
             * Get all the ancestor types for this 'source' type and see if one of them is valid.
             */
            let sourceType = sourceObject.type;
            let sourceModelType = cmm.getType( sourceType );
            if( sourceModelType ) {
                let sourceTypeHeirarchy = sourceModelType.typeHierarchyArray;
                for( let j = 0; j < sourceTypeHeirarchy.length; j++ ) {
                    let sourceParentType = sourceTypeHeirarchy[ j ];
                    let relationType = sourceType2RelationMap[ sourceParentType ];
                    if( relationType ) {
                        let sourceObjectsForType = pasteRelation2SourceObjectsMap[ relationType ];
                        if( !sourceObjectsForType ) {
                            sourceObjectsForType = [];
                            pasteRelation2SourceObjectsMap[ relationType ] = sourceObjectsForType;
                        }
 ...
 */
const isValidObjectToDrop = function( dragAndDropParams, xrtTableUid, props ) {
...
const getAdaptedObjectsWithAlternateId = ( targetVMOs ) => {
    return adapterSvc.getAdaptedObjects( [ targetVMOs[ 0 ] ] ).then( function( adaptedObjs ) {
        if( targetVMOs[ 0 ].alternateID ) {
            adaptedObjs[0].alternateID = targetVMOs[ 0 ].alternateID;
        }
        return adaptedObjs;
    } );
};

export let onDropObjectSetTree = ( props, dragAndDropParams ) => {
    let event = dragAndDropParams.event;
    event.preventDefault();
    var targetElement = dragAndDropParams.targetElement;
    if( !targetElement ) {
    ...
    var missingSourceUIDs = [];
    let targetVMOs = dragAndDropParams.targetObjects;
    if( props.vmo && dragAndDropParams.dataProvider && props.objectSetData.source ) {
        targetVMOs =  !dragAndDropParams.targetObjects || !dragAndDropParams.targetObjects.length  ? [ props.vmo ] : targetVMOs;
    }
    var sourceUids = awDragAndDropUtils.getCachedSourceUids();
    if( sourceUids && sourceUids.length > 0 ) {
        ...
        if( missingSourceUIDs.length === 0 ) {
            if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMOs[ 0 ].modelType ) ) {
                getAdaptedObjectsWithAlternateId( [ targetVMOs[ 0 ] ] ).then( function( adaptedObjs ) {
                    pasteSourceObjectsOnObjectSetTree( adaptedObjs[ 0 ], sourceObjects, dragAndDropParams.callbackAPIs, dragAndDropParams );
                } );
            } else {
                pasteSourceObjectsOnObjectSetTree( targetVMOs[ 0 ], sourceObjects, dragAndDropParams.callbackAPIs, dragAndDropParams );
            }
            awDragAndDropUtils._clearCachedData();
        } else {
            dms.loadObjects( missingSourceUIDs ).then( function() {
        ...
        let targetObject = dragAndDropParams.targetObjects[ 0 ];
        if( targetObject.props && targetObject.props.awp0Secondary && targetObject.props.awp0Secondary.dbValue
            && cdm.isValidObjectUid( targetObject.props.awp0Secondary.dbValue ) ) {
            targetObject = viewModelObjectSvc.createViewModelObject( cdm
                .getObject( targetObject.props.awp0Secondary.dbValue ) );
        }
        if( targetObject.type === 'PseudoFolder' ) {
            dragAndDropParams.updatedObjectForPseudoFolder = xrtObjectSetSvc.getTargetObjectForPseudoFolder( targetObject );
        }
    } else {
        var sourceFiles = event.dataTransfer.files;
        if( sourceFiles && sourceFiles.length > 0 ) {
            if( isDropFileContainerOrDragOverChoose( event ) ) {
                dragAndDropParams.callbackAPIs.updateFileData( event );
            } else {
                dropFilesOnObjectSetTree( sourceFiles, dragAndDropParams );
            }
        } else {
            awDragAndDropUtils._clearCachedData();
        }
...
const pasteSourceObjectsOnObjectSetTree = function( targetVMO, sourceObjects, callbackAPIs, dragAndDropParams ) {
    let pasteRelation2SourceObjectsMap = [];
    if( dragAndDropParams && dragAndDropParams.props && dragAndDropParams.props.objectSetData && dragAndDropParams.props.objectSetData.source ) {
        pasteRelation2SourceObjectsMap = xrtObjectSetSvc.getModelTypeRelationListMap( dragAndDropParams.props.objectSetData.source );
    }
    if( targetVMO.type === 'PseudoFolder' ) {
        let parentOfPseudoFolder = xrtObjectSetSvc.getTargetObjectForPseudoFolder( targetVMO );
        targetVMO = parentOfPseudoFolder.targetObjectForPseudoFolder;
        pasteRelation2SourceObjectsMap = xrtObjectSetSvc.getModelTypeRelationListMap( parentOfPseudoFolder.relationType );
    } else {
        pasteRelation2SourceObjectsMap = xrtObjectSetSvc.getModelTypeRelationListMap( '' );
    }
    callbackAPIs.setSelection( targetVMO );
...
const isDropFileContainerOrDragOverChoose = function( event ) {
    var targetElement = null;
    if( event && event.target && event.target.classList && event.target.classList.contains( 'aw-widgets-chooseordropfile' ) ) {
        targetElement = event.target;
    } else {
        var cdfContainer = ngUtils.closestElement( event.target, '.aw-widgets-chooseordropfile' );
        if( cdfContainer ) {
            targetElement = cdfContainer;
        }
    }
    return targetElement;
};

const dropFilesOnObjectSetTree = function( sourceFiles, dragAndDropParams ) {
    let targetVMO = dragAndDropParams.targetObjects[ 0 ];
    if( sourceFiles && sourceFiles.length > 0 ) {
        if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
            getAdaptedObjectsWithAlternateId( [ targetVMO ] ).then( function( adaptedObjs ) {
                dragAndDropParams.callbackAPIs.clearSelection( adaptedObjs );
            } );
        }
        dragAndDropParams.callbackAPIs.clearSelection( targetVMO );
        if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
            getAdaptedObjectsWithAlternateId( [ targetVMO ] ).then( function( adaptedObjs ) {
                pasteSourceFilesOnObjectSetTree( adaptedObjs[ 0 ], sourceFiles, dragAndDropParams );
            } );
        } else {
            pasteSourceFilesOnObjectSetTree( targetVMO, sourceFiles, dragAndDropParams );
        }
    }
    awDragAndDropUtils._clearCachedData();
};

const pasteSourceFilesOnObjectSetTree = function( targetVMO, sourceFiles, dragAndDropParams ) {
    let pasteRelation2SourceObjectsMap = {};
    let modelTypeRelationListMap = [];
    if( dragAndDropParams && dragAndDropParams.props && dragAndDropParams.props.objectSetData && dragAndDropParams.props.objectSetData.source ) {
        modelTypeRelationListMap = xrtObjectSetSvc.getModelTypeRelationListMap( dragAndDropParams.props.objectSetData.source );
    }
    if( targetVMO.type === 'PseudoFolder' ) {
        let parentOfPseudoFolder = xrtObjectSetSvc.getTargetObjectForPseudoFolder( targetVMO );
        targetVMO = parentOfPseudoFolder.targetObjectForPseudoFolder;
        targetVMO.isFileDroppedOnPseudoFolder = true;
        modelTypeRelationListMap = xrtObjectSetSvc.getModelTypeRelationListMap( parentOfPseudoFolder.relationType );
    } else {
        modelTypeRelationListMap = xrtObjectSetSvc.getModelTypeRelationListMap( '' );
    }
    dragAndDropParams.callbackAPIs.setSelection( targetVMO );
    _.forEach( modelTypeRelationListMap, function( value, key ) {
        pasteRelation2SourceObjectsMap[ value ] = sourceFiles;
    } );
    executeFileDrop( targetVMO, dragAndDropParams.callbackAPIs, pasteRelation2SourceObjectsMap, dragAndDropParams  );
};

export let dragEnterActionOnObjectSetTree = ( props, dragAndDropParams ) => {
    if( dragAndDropParams.event ) {
        dragAndDropParams.event.preventDefault();
    }
};

export let dragOverActionOnObjectSetTree = ( props, dragAndDropParams ) => {
    if( dragAndDropParams.event ) {
        dragAndDropParams.callbackAPIs.highlightTarget( {
            isHighlightFlag: true,
            targetElement: dragAndDropParams.targetElement
...
exports = {
    ...
    onDropObjectSetTree,
    dragEnterActionOnObjectSetTree,
    dragOverActionOnObjectSetTree,
    dragStart,
    dragEnd,
    dragOver,
    dragEnter,
