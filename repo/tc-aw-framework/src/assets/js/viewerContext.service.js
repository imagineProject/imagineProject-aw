...
/**
 * Viewer has Visual Report Legend enabled
 */
export let VIEWER_IS_VISUAL_REPORT_LEGEND_VISIBLE = 'isVisualReportLegendVisible';
 ...
 */
export let SECTION_PANEL_CLOSED = 'sectionPanelClosed';
...
export let PVW_CONTEXT_MENU = 'pvwContextMenu';

export let PVW_SECTION_CONTEXT_MENU = 'pvwSectionContextMenu';

export let VIEWER_SECTION_CMD_FROM_PVW = 'viewerSectionCmdFromPvw';

export let PMI_ENTITY_HIDE_FROM_PVW_CONTEXT_MENU = 'pmiEntityHideFromPvwContextMenu';
/**
...
/**
 * Session Snapshot applided
 */
export let SESSION_SNAPSHOT_APPLIED = 'SessionSnapshotApplied';
/**
 ...
 * Create Markup end
 */
export let VIEWER_CREATE_MARKUP_END = 'ViewerCreateMarkupEnd';

/**
 * used as topic to delete from keyboard
 */
export let VIS_DELETE_FROM_KEYBOARD = 'visDeleteFromKeyboard';

export let GET_ASSETS_INFO = 'getAssetsInfo';

/**
 * Switch render location to SSR
 */
export let VIEWER_RENDER_IN_SSR = 'switchToSSR';

/**
 * Viewer pref mouse gesture value for NX
 */
export let NX_MOUSE_GESTURE = 'NX';
/**
 * Viewer pref mouse gesture value for TC web client
 */
const TC_WEBCLIENT_MOUSE_GESTURE = 'AW';
/**
 * viewer pref path of AWV0MouseGestureMapping
 *
 */
const VIEW_PREF_MOUSE_GESTURE = 'viewerPreference.AWV0MouseGestureMapping';

/**
 * remove analysis
 */
const EXECUTE_REMOVE_ANALYSIS = 'executeRemoveAnalysis';

/**
 * viewer volume select
 */
const VIEWER_VOLUME_SELECT = 'volumeSelect';

export const VIS_SECTION_PLANE_TRANSLATE_MODE = 'sectionTranslateMode';
 ...
 * @param {Object} psLoader product structure loader instance
 * @return {Object} Registered viewer context
 */
export let registerViewerContext = function( viewerCtxNamespace, viewerType, contextData, is2D, viewerAtomicData, psLoader, bookmarkProviderInstance ) {
    ...
    var viewerCtxData = viewerCtxDataProvider.getViewerContextData( viewerCtxNamespace,
        viewerTypeToBeRegistered, contextData, is2D, viewerAtomicData, psLoader, bookmarkProviderInstance );
 ...
 *
 */
export let handleBrowserUnload = function() {
    _.forOwn( registeredViewerContext, ( registeredObj ) => {
        if( typeof registeredObj.close === 'function' ) {
            if( exports.isServerless() ) {
                registeredObj.getPsLoader().deregisterForBomEventListener();
            }
            registeredObj.close();
        }
    } );
...
export let activateViewerCommandPanel = function( commandId, location, context, closeWhenCommandHidden ) {
    if( context && context.viewerContextData ) {
        ...
        if( context.viewerContextData.getValueOnViewerAtomicData( VIEWER_IS_NAV_MODE_AREA_SELECT ) === true && exports.isTeamcenterWcGestureSettingEnabled( context.viewerContextData ) ) {
            exports.setNavigationMode( context.viewerContextData, context.viewerContextData.getValueOnViewerAtomicData( VIEW_PREF_NAVMODE_PATH ) );
        }
    }
...
/**
 * Sets Default Navigation action LMB on the basis of mouse gesture setting
 * @param {Object} viewerContextData viewer Context data
 */
export let setDefaultActionNavigation = ( viewerContextData ) => {
    if( viewerContextData.getValueOnViewerAtomicData( exports.VIEWER_IS_NAV_MODE_AREA_SELECT ) === true ) {
        if( exports.isTeamcenterWcGestureSettingEnabled( viewerContextData ) ) {
            exports.setNavigationMode( viewerContextData, viewerContextData.getValueOnViewerAtomicData( VIEW_PREF_NAVMODE_PATH ) );
        } else if( exports.isNxGestureSettingEnabled( viewerContextData ) ) {
            viewerContextData.getNavigationManager().disableDefaultAction();
            viewerContextData.updateViewerAtomicData( VIEWER_IS_NAV_MODE_AREA_SELECT, false );
        }
    } else if( exports.isNxGestureSettingEnabled( viewerContextData ) && !viewerContextData.getValueOnViewerAtomicData( VIEWER_IS_SUB_COMMANDS_TOOLBAR_ENABLED ) ) {
        exports.setNavigationMode( viewerContextData, 'AREA_SELECT' );
    }
};
...
export let closeSubCommandsToolbar = function( viewerCtxData ) {
    ...
    if( exports.isNxGestureSettingEnabled( viewerCtxData ) && viewerCtxData.getValueOnViewerAtomicData( exports.VIEWER_IS_NAV_MODE_AREA_SELECT ) === false ) {
        exports.setNavigationMode( viewerCtxData, 'AREA_SELECT' );
    }
};

/**
 * Checks if Nx Gesture setting is enabled
 * @param {Object} viewerContextData viewer context data
 * @returns {Boolean} Nx gesture setting enabled or not
 */
export let isNxGestureSettingEnabled = function( viewerContextData ) {
    return viewerContextData.getValueOnViewerAtomicData( exports.VIEW_PREF_MOUSE_GESTURE ) === exports.NX_MOUSE_GESTURE;
};

/**
 * Checks if team center web client gesture setting is enabled
 * @param {Object} viewerContextData viewer context data
 * @returns {Boolean} team center web client gesture setting enabled or not
 */
export let isTeamcenterWcGestureSettingEnabled = function( viewerContextData ) {
    return viewerContextData.getValueOnViewerAtomicData( exports.VIEW_PREF_MOUSE_GESTURE ) === TC_WEBCLIENT_MOUSE_GESTURE;
};
 ...
 *
 * @param {Object} viewerContextData - viewer Context Data
 * @param {String} selectedStyle - Transparent/PartColor
 */
export let styleSelection = function( viewerContextData, selectedStyle ) {
    ...
    } else if ( selectedStyle === 'PartColor' ) {
        viewerContextData.getSelectionManager().setViewerSelectionDisplayStyle(
            viewerPreferenceService.SelectionDisplayStyle.HIGHLIGHT );
        viewerContextData.getSelectionManager().setViewerContextDisplayStyle(
            viewerPreferenceService.ContextDisplayStyle.NONE );
    }  else if ( selectedStyle === 'Halo' ) {
        viewerContextData.getSelectionManager().setViewerSelectionDisplayStyle(
            viewerPreferenceService.SelectionDisplayStyle.HALO_EFFECT );
        viewerContextData.getSelectionManager().setViewerContextDisplayStyle(
            viewerPreferenceService.ContextDisplayStyle.NONE );
    }
    viewerPreferenceService.setSelectionDisplayPreference( selectedStyle.toString(), null, viewerContextData );
    let prefName = 'AWC_visSelectionDisplay';
    prefService.setStringValue( prefName, [ selectedStyle.toString() ] );
};
...
export let setNavigationMode = function( viewerContextData, navigationModeStr ) {
    ...
    if( navigationMode === 'AREA_SELECT' && viewerContextData.getValueOnViewerAtomicData( VIEWER_IS_NAV_MODE_AREA_SELECT ) === true && exports.isTeamcenterWcGestureSettingEnabled(
        viewerContextData ) ) {
        if( viewerContextData.getValueOnViewerAtomicData( VIEW_PREF_NAVMODE_PATH ) === null ) {
            navigationMode = 'ROTATE';
        } else {
            navigationMode = viewerContextData.getValueOnViewerAtomicData( VIEW_PREF_NAVMODE_PATH );
        }
        viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( null );
    }
    if( viewerContextData && !_.isUndefined( viewerContextData.getNavigationManager() ) ) {
    ...
    if( navigationMode !== 'AREA_SELECT' ) {
        ...
        let escapeKeyFncName = viewerContextData.getViewerKeyboardMgr().getEscapeKeyFunctionName();
        if( exports.isTeamcenterWcGestureSettingEnabled( viewerContextData ) && !_.isNull( escapeKeyFncName ) && escapeKeyFncName.includes( 'handleEscapeKeyboardAreaSelectIn3d' ) ) {
            viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( null );
        }
    }
    ...
    if( navigationMode !== 'AREA_SELECT' ) {
        ...
        if( exports.isTeamcenterWcGestureSettingEnabled( viewerContextData ) ) {
            viewerContextData.getViewerKeyboardMgr().setEscapeKeyFunction( handleEscapeKeyboardAreaSelectIn3d.bind( { viewerContextData: viewerContextData } ) );
        }
    }
};
/**
 * handler for escape keyboard listener in area select
 * @this handleEscapeKeyboardAreaSelectIn3d
 */
let handleEscapeKeyboardAreaSelectIn3d = function() {
    exports.setNavigationMode( this.viewerContextData, 'AREA_SELECT' );
};
...
/**
 * Execute VolumeSelect command.
 *
 * @function executeVolumeSelect
 *
 *  @param {Object} viewerContextData Viewer context data
 */
export let executeVolumeSelect = function( viewerContextData ) {
    if( viewerContextData.getValueOnViewerAtomicData( VIEWER_VOLUME_SELECT ) === true ) {
        exports.executeVolumeSelectCancel( viewerContextData );
    } else {
        exports.closePanelsWhenVolumeSelectIsActive( viewerContextData );
        viewerContextData.updateViewerAtomicData( VIEWER_VOLUME_SELECT, true );
        viewerContextData.getVolumeManager().enableVolumeCubeManipulator( true );
    }
    exports.setDefaultActionNavigation( viewerContextData );
};

/**
 * Execute VolumeSelect Cancel command.
 *
 * @function executeVolumeSelect
 *
 *  @param {Object} viewerContextData Viewer context data
 */
export let executeVolumeSelectCancel = function( viewerContextData ) {
    viewerContextData.getVolumeManager().cancelVolumeCubeManipulator( );
};


/**
 * Execute VolumeSelect Accept command.
 *
 * @function executeVolumeSelect
 *
 *  @param {Object} viewerContextData Viewer context data
 */
export let executeVolumeSelectAccept = function( viewerContextData ) {
    viewerContextData.updateViewerAtomicData( VIEWER_VOLUME_SELECT, false );
    viewerContextData.getVolumeManager().enableVolumeCubeManipulator( false );
};

/**
 * Closing Panels and other commands when Volume select is invoked
 *
 * @function closePanelsWhenVolumeSelectIsActive
 *
 * @param {Object} viewerContextData Viewer context data
 */

export let closePanelsWhenVolumeSelectIsActive = function( viewerContextData ) {
    if( viewerContextData.getValueOnViewerAtomicData( VIEWER_IS_SUB_COMMANDS_TOOLBAR_ENABLED ) ) {
        exports.closeSubCommandsToolbar( viewerContextData );
    } else {
        let dialogToolAndInfo = appCtxService.getCtx( 'activeToolsAndInfoCommand_dialog' );
        if( dialogToolAndInfo && dialogToolAndInfo.commandId !== 'Awv0ViewerSettings' ) {
            viewerContextData.closeActiveDialog();
        }
    }
};
...
export let executeViewOrientationCommand = function( viewerContextData, camOrientation ) {
    var cameraOrientation = viewerOrientationService.getViewOrientation( camOrientation );
 ...
 */
export let executeFitCommand = function( viewerContextData, occmgmtContext, commandContext ) {
    ...
    AwTimeoutService.instance( function() {
        viewerContextData.getVisibilityManager().removeAnalysisResult().then( () => {
            ...
            if( viewerContextData.get3DMarkupManager() ) {
                viewerContextData.get3DMarkupManager().removeAllToolkitMarkups();
            }
            viewerContextData.getViewerAtomicDataSubject().notify( exports.EXECUTE_REMOVE_ANALYSIS );
        } ).catch( error => {
...
/**
 * Set NX mouse gesture
 * @param {Object} viewerCtxData viewer context data
 */
export let setNXMouseGesture = ( viewerCtxData ) => {
    if( viewerCtxData && viewerCtxData.getValueOnViewerAtomicData( VIEW_PREF_MOUSE_GESTURE ) === NX_MOUSE_GESTURE && viewerCtxData.getValueOnViewerAtomicData( 'renderLocation' ) === 'CSR' ) {
        viewerCtxData.updateViewerAtomicData( VIEWER_IS_NAV_MODE_AREA_SELECT, true );
        viewerCtxData.getNavigationManager().setNXMouseGesture();
    }
};

/**
 * Set AW mouse gesture
 * @param {Object} viewerCtxData viewer context data
 */
export let setAWMouseGesture = ( viewerCtxData ) => {
    if( viewerCtxData && viewerCtxData.getValueOnViewerAtomicData( VIEW_PREF_MOUSE_GESTURE ) === TC_WEBCLIENT_MOUSE_GESTURE && viewerCtxData.getValueOnViewerAtomicData( 'renderLocation' ) === 'CSR' ) {
        viewerCtxData.updateViewerAtomicData( VIEWER_IS_NAV_MODE_AREA_SELECT, false );
        let navigationMode = viewerPreferenceService.getPreferenceValue( viewerPreferenceService.VIS_NAVIGATION_MODE, viewerCtxData );
        viewerCtxData.getNavigationManager().setAWMouseGesture( viewerPreferenceService.ViewerNavigationModes[ navigationMode ] );
    }
};

/**
 * Get model object uid
 * @param {Object} modelObject model object
 * @returns {String} unique uid of model object passed
 */
export let getModelObjectUid = ( modelObject ) => {
    let modelUid = null;
...
/**
 * Returns serverless mode enabled or not from beta preferences
 * @returns {Boolean} - returns serverless mode enabled or not
 */
export let isServerless = ()=>{
    let betaPrefValues = viewerPreferenceService.getViewerBetaPref();
    return  _.includes( betaPrefValues, 'enableServerless' );
};

/**
 * Return topElement object from occmgmtContext passed
 * @param {Object} occmgmtContext - Occurrence management context
 * @return {Object} - topElement object
 */
export let getTopElementObject = function( occmgmtContext ) {
    let topElement = null;
    if( occmgmtContext && occmgmtContext.currentState && occmgmtContext.currentState.t_uid ) {
        topElement = cdm.getObject( occmgmtContext.currentState.t_uid );
    } else if ( occmgmtContext ) {
        topElement = occmgmtContext.topElement || occmgmtContext.rootElement;
    }
    return topElement;
};
...
export default exports = {
    ...
    VIEWER_AREA_SELECT,
    PVW_CONTEXT_MENU,
    PVW_SECTION_CONTEXT_MENU,
    VIS_DELETE_FROM_KEYBOARD,
    GET_ASSETS_INFO,
    VIEWER_SECTION_CMD_FROM_PVW,
    PMI_ENTITY_HIDE_FROM_PVW_CONTEXT_MENU,
    VIEWER_CAPTURE_SNAPSHOT_BEGIN,
    SESSION_SNAPSHOT_APPLIED,
    VIEWER_CREATE_SECTION_BEGIN,
    VIEWER_CREATE_MARKUP_BEGIN,
    VIEWER_CREATE_MARKUP_END,
    VIEWER_IS_NAV_MODE_AREA_SELECT,
    VIEWER_RENDER_IN_SSR,
    NX_MOUSE_GESTURE,
    VIEW_PREF_MOUSE_GESTURE,
    EXECUTE_REMOVE_ANALYSIS,
    VIEWER_VOLUME_SELECT,
    VIS_SECTION_PLANE_TRANSLATE_MODE,
    ...
    setDefaultActionNavigation,
    isNxGestureSettingEnabled,
    isTeamcenterWcGestureSettingEnabled,
    setNXMouseGesture,
    setAWMouseGesture,
    getModelObjectUid,
    executeVolumeSelect,
    executeVolumeSelectCancel,
    executeVolumeSelectAccept,
    closePanelsWhenVolumeSelectIsActive,
    isServerless,
    getTopElementObject
};
