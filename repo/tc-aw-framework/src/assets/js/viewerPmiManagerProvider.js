...
const GEOANALYSIS_PMI_PVW_IDS_FROM_CONTEXT_MENU = 'pvwIdsForHiddenPmiFromContextMenu';
/**
...
var _createPMIEntityObj = function( idx, entity ) {
    ...
    var uniqueIdP = entity.visObject.getVisPropertyValue( 'UniqueId' );
    AwPromiseService.instance.all( [ nameP, typeP, visibleP, selectedP, uniqueIdP ] ).then( function( args ) {
        var name = args[ 0 ];
        var type = args[ 1 ];
        ...
        var uniqueId = args[4];
        deferred.resolve( [ idx, name, type, visibilityStr, entity.visObject.resourceID, selected, uniqueId ] );
    } );
...
class ViewerPmiManager {
    ...
    setupAtomicDataTopicsPanelReveal( pmiToolState ) {
        ...
        this.viewerContextData.getViewerAtomicDataSubject().subscribe( viewerContextService.PMI_ENTITY_HIDE_FROM_PVW_CONTEXT_MENU, this );
    }
    ...
    update( topic, data ) {
        ...
        }else if( topic === viewerContextService.PMI_ENTITY_HIDE_FROM_PVW_CONTEXT_MENU ) {
            this.updatePmiToolState( GEOANALYSIS_PMI_PVW_IDS_FROM_CONTEXT_MENU, data.pvwObject );
        }
    }
    ...
    updateCtxWithCurrentSelection() {
        ...
        if( _.isEmpty( viewerSelectionCSIDS ) ) {
            ...
            if( !viewerSelectionCSIDS.slice( -1 )[ 0 ] || !viewerSelectionModels.slice( -1 )[ 0 ] ) {
                return;
            }
    ...
    unsubscribeAtomicDataTopicsPanelClose() {
        ...
        this.viewerContextData.getViewerAtomicDataSubject().unsubscribe( viewerContextService.PMI_ENTITY_HIDE_FROM_PVW_CONTEXT_MENU, this );
    }
}
...
export default {
    ...
    GEOANALYSIS_PMI_PVW_IDS_FROM_CONTEXT_MENU,
    getPmiManager,
    getModelViewManager
};
