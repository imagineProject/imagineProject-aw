...
export let importOptions = function( isRunInBackground, conflict ) {
    let importOptions = [];
    importOptions.push( 'RoundTripImport' );

    if ( isRunInBackground ) {
        importOptions.push( 'RunInBackground' );
    }
...
export let getMessageTextAction = function( partialErrors ) {
    let errorMessageText;
    let errorCode;
    if( partialErrors && partialErrors.length > 0 && partialErrors[0].errorValues && partialErrors[0].errorValues.length > 0 ) {
        errorCode = partialErrors[0].errorValues[partialErrors[0].errorValues.length - 1].code;
        errorMessageText = partialErrors[0].errorValues[partialErrors[0].errorValues.length - 1].message;
    }
    return {
        errorcode:errorCode,
