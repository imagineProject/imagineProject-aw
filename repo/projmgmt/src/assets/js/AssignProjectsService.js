...
export let addToAvailableProjects = function( data, vmo ) {
    ...
    updateAvailableList.unshift( vmo );
    ...
    data.dataProviders.availableProjects.update( updateAvailableList, updateAvailableList.length );

    //Usecase - Update the endIndex which is used as startIndex for next soa call
    //available project list is updated as a project is added to availableProject dataprovider when remove from members of project.
    //modelObjects.length < memberModelObjects.length - condition is used to decide that it is removing project from members of project.
    //projectList.indexOf( vmo.uid ) < 0 condition is true when it is removing project from members of tab which is newly added through the panel. 
    //projectList.indexOf( vmo.uid ) < 0 condition is false when vmo project is from saved assigned project, in that case no need to update endIndex.
    result.endIndex = data.endIndexAvailableProject;    
    var projectList = appCtxSvc.getCtx( 'assignedProjectList' );
    if(viewModelObjectsAvailList.length !== updateAvailableList.length && projectList.indexOf( vmo.uid ) < 0){
        result.endIndex = data.endIndexAvailableProject + 1; 
    }
...
/**
  ...
  *  
  * Usecase - 
  * This function is remove a project vmo from member of project list.
  * if it is owning project then it remove from owning project ctx as well and marked isOwningProjectUpdated true
  * In this method we are modifying the endIndex for availableproject list as a project is added to availableProjectlist when remove from members of project list.
  * EndIndex is +1 only when project which is remove from member of project is not previous saved members of projects.
  */
...
function removeFromMemberOfProjects( data, vmo ) {
    ...
    if( appCtxSvc.getCtx( 'owningProject' ) && vmo.uid === appCtxSvc.getCtx( 'owningProject' ).uid ) {
        ...
        var viewModelOwningObjects = data.dataProviders.availableOwningProgram.viewModelCollection.loadedVMObjects;
        var owningModelObjects = _.clone( viewModelOwningObjects );
        owningModelObjects.unshift( vmo );
        if( data.filterBoxOwningProgram.dbValue !== '' ) {
            projectMgmntUtils.setFilterText( data.filterBoxOwningProgram.dbValue );
            var arrByID = owningModelObjects.filter( projectMgmntUtils.filterByName );
            owningModelObjects = arrByID;
        }        
        data.dataProviders.availableOwningProgram.update( owningModelObjects );
        return {
...
function removeFromAvailableProjects( data, vmo ) {
    ...
    //Usecase - Update the endIndex which is used as startIndex for next soa call
    //available project list is updated as a project is removed from availableProject dataprovider and added to members of.
    //modelObjects.length !== availModelObjects.length - condition is used to decide that it is removing project from available project list.
    //projectList.indexOf( vmo.uid ) < 0 condition is true when when vmo project is not from saved assigned project, update endIndex.
    //projectList.indexOf( vmo.uid ) < 0 condition is false it is same vmo project present in assigned project list, then no need to change endIndex.
    //one project is removed from available project so need to change endIndex for pagination.
    var endIndex = data.endIndexAvailableProject;    
    var projectList = appCtxSvc.getCtx( 'assignedProjectList' );
    if(modelObjects.length !== availModelObjects.length && projectList.indexOf( vmo.uid ) < 0){
        endIndex = data.endIndexAvailableProject - 1; 
    }
    data.dispatch( { path:'data.endIndexAvailableProject', value:endIndex } );
...
export let replaceOwningProject = function( data, vmo ) {
    ...
    if( appCtxSvc.getCtx( 'owningProject' ) && appCtxSvc.getCtx( 'owningProject' ) !== '' ) {
        ...
        var availModelObjects = _.clone( viewModelObjects );
        availModelObjects.unshift( appCtxSvc.getCtx( 'owningProject' ) );
...
/**
 * This method is call upon unmount of project assign/remove panel.
 * It is unregister the owningProject ctx & assignedProjectList.
 * Ctx owningProject & assignedProjectList is register when panel loads and unregister it when panel close.
 */
export let unregisterCtx = function( ) {
    appCtxSvc.unRegisterCtx( 'owningProject' );
    appCtxSvc.unRegisterCtx( 'assignedProjectList' );
};
