...
var m_expandSourceRelated = 'EXPAND_SOURCE_RELATED';
var m_relatedObjectsData = 'Rb0RelatedObjectsData';
...
export let getExpandSourceRelatedName = function() {
    return m_expandSourceRelated;
};

export let getRelatedObjectsDataName = function() {
    return m_relatedObjectsData;
};
 ...
 */
const getFilterCategoryFromPropertyName = function( targetCategories, sourceFilterPropName ) {
    ...
    let returnCategory; // No need to initialize it to 'undefined'

    _.forEach( targetCategories, ( category ) => {
...
export let processSearchDataAndGetChartPoints = function( searchResultFilters, filterCategories, filterMap, reportConfig ) {
    ...
    if( internalNameData.type === 'DateFilter' ) {
        dataPointsChart = dataPointsChart.filter( function( dataPoint ) {
            if( dataPoint.internalExtension === '_0Z0_year_month_day' && dataPoint.selected ) {
    ...
    var displayName = getLocalisedNameOfChartOn( reportConfig.ChartPropName, searchFilterName, filterCategories );
...
export const checkDashboardContainsReportdef = ( tabObjects, reportNameKey )=>{
    let reportFound = false;
    let foundTabObject = null;
    const tabObjectUids = [];
    _.forEach( tabObjects, function( tabObject ) {
        tabObjectUids.push( tabObject.uid );
    } );
    return dmSvc.getProperties( tabObjectUids, [ 'rb0DashboardTemplatesList' ] ).then( function() {
        _.forEach( tabObjects, function( tabObject ) {
            let reportList = [];
            tabObject = cdm.getObject( tabObject.uid ) ? cdm.getObject( tabObject.uid ) : tabObject;
            if( tabObject.props?.rb0DashboardTemplatesList?.dbValues?.length ) {
                reportList = tabObject.props.rb0DashboardTemplatesList.dbValues;
            }
            if( reportList && reportList.indexOf( reportNameKey ) !== -1 ) {
                reportFound = true;
                foundTabObject = tabObject;
            }
        } );
        appCtxService.updatePartialCtx( 'showAddToDashboardCommand', !reportFound );
        return { reportFound, foundTabObject };
    } );
};

export let checkForDashboardConfigCommand = function( selectedReport, selectedObject, preferenceName, tabObjects ) {
    if( selectedReport ) {
        ...
        let reportList = [];
        let showAddToDashboardCommand = true;
        if( tabObjects?.length && tabObjects[0] ) {
            return checkDashboardContainsReportdef( tabObjects, reportNameKey ).reportFound;
        } else if( appCtxService.ctx.dashboardObjects?.objects ) {
            return checkDashboardContainsReportdef( appCtxService.ctx.dashboardObjects.objects, reportNameKey ).reportFound;
        } else if( preferenceName ) {
            reportList = appCtxService.ctx.preferences[ preferenceName ];
            showAddToDashboardCommand = !( reportList && reportList.indexOf( reportNameKey ) !== -1 );
            appCtxService.updatePartialCtx( 'showAddToDashboardCommand', showAddToDashboardCommand );
        } else {
            reportList = appCtxService.getCtx( getCtxForReportsPreference() );
            showAddToDashboardCommand = !( reportList && reportList.indexOf( reportNameKey ) !== -1 );
            appCtxService.updatePartialCtx( 'showAddToDashboardCommand', showAddToDashboardCommand );
        }
    }
...
export const checkForManageDashboardCommand = () => {
    let currentGroup = appCtxService.getCtx( 'userSession.props.group.displayValues[0]' );
    let currentRole = appCtxService.getCtx( 'userSession.props.role.displayValues[0]' );
    let showManageDashboardCommand = false;
    let grpRoles = appCtxService.getCtx( m_preferences + 'REPORT_Create_Dashboard_Allowed_GroupRoles' );
    _.forEach( grpRoles, function( grpRole ) {
        let group = grpRole.split( ':' )[0];
        let role = grpRole.split( ':' )[1];
        if( group === currentGroup && role === currentRole ) {
            showManageDashboardCommand = true;
        }
    } );
    appCtxService.updatePartialCtx( 'showManageDashboardCommand', showManageDashboardCommand );
};
...
export let getLocalisedNameOfChartOn = ( chartName, chartInternalName, filterCategories )=>{
    var displayName = Array.isArray( chartName ) ? chartName[ 0 ] : chartName;
    var internalName = Array.isArray( chartInternalName ) ? chartInternalName[ 0 ] : chartInternalName;
    const internalNameArray = internalName?.split( '.' );
    internalName = internalNameArray ? internalNameArray[internalNameArray.length - 1] : '';
    _.forEach( filterCategories, category=>{
        const splitArray = category.internalName.split( '.' );
        const filterName = splitArray ? splitArray[splitArray.length - 1] : '';
        filterName === internalName ? displayName = category.displayName : '';
    } );
    return displayName;
};
export let getSelectedRepObject = ( selectedReport ) => {
    ...
    } else if( appCtxService.ctx.pselected?.type === 'ReportDefinition' ) {
        return appCtxService.ctx.pselected;
    } else if( appCtxService.ctx.ReportsContext?.selected?.type === 'ReportDefinition' ) {
        return appCtxService.ctx.ReportsContext.selected;
    }
    return selectedReport;
};
export let getUIDForSelectObject = ( selectedReport, reportSourceObjectUid ) => {
    if( selectedReport?.props?.rd_type?.dbValues[ 0 ] === '1' ) {
        if( reportSourceObjectUid?.length > 0 ) {
            return reportSourceObjectUid;
        }
        var obj = exports.getUnderlyingObject( appCtxService.ctx.selected );
        return obj.uid;
    }
    return undefined;
};
...
export default exports = {
    ...
    getUnderlyingObject,
    getReportUpdateTime,
    checkDashboardContainsReportdef,
    checkForDashboardConfigCommand,
    checkForManageDashboardCommand,
    convertJsonToString,
    getLocalisedNameOfChartOn,
    getExpandSourceRelatedName,
    getRelatedObjectsDataName,
