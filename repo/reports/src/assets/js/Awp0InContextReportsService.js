...
var OFFICE_TEMPLATE = 'Office Template';
...
export let getListOfReportStylesheetAction = function( data, selectedReport ) {
    if( data !== null ) {
        ...
        var source_Name = selectedObject.props.rd_source.dbValues[ 0 ];
        if( source_Name === 'Teamcenter' && selectedObject.props.rd_style_sheets?.dbValues ) {
        ...
        } else if( source_Name === OFFICE_TEMPLATE ) {
...
export let processResponseToStoreReportDefs = function( response, data ) {
    ...
    let ReportDefinitions = JSON.parse( response.searchResultsJSON ).objects.map( function( rDef ) {
        return response.ServiceData.modelObjects[ rDef.uid ];
    } );
    _.forEach( ReportDefinitions, ( repDefObj ) => {
        if( repDefObj.type === 'ReportDefinition' && ( repDefObj.props.rd_source.dbValues[ 0 ] === 'Teamcenter' || repDefObj.props.rd_source.dbValues[ 0 ] === 'Office Template' || repDefObj.props.rd_source.dbValues[ 0 ] === 'Active Workspace' ) ) {
            _inCtxReportDefs.push( repDefObj );
...
export let prepareLanguageList = function( response ) {
    if( response.languageList ) {
        ...
        for( var ii = 0; ii < response.languageList.length; ii++ ) {
            if( response.languageList[ ii ].languageCode.startsWith( currentLocaleCode ) ) {
        ...
        if( reportLocale !== undefined ) {
            for( var jj = 0; jj < reportLocale.length; jj++ ) {
                ...
                for( var k = 0; k < response.languageList.length; k++ ) {
                    if( response.languageList[ k ].languageCode === value && !response.languageList[ k ].languageCode.startsWith( currentLocaleCode ) ) {
            ...
            if( languageList.length <= 1 ) {
                for( var i = 0; i < response.languageList.length; i++ ) {
                    if( !response.languageList[ i ].languageCode.startsWith( currentLocaleCode ) ) {
            ...
            for( let i = 0; i < response.languageList.length; i++ ) {
                if( !response.languageList[ i ].languageCode.startsWith( currentLocaleCode ) ) {
                    languageList.push( response.languageList[ i ] );
                }
            }
 ...
 */
export let getSelectedObjects = function( selectedObjects ) {
    let ctxObjs = [];
    let iterateObjects;
    if( selectedObjects ) {
        iterateObjects = selectedObjects;
    } else {
        iterateObjects = appCtxService.ctx.mselected;
    }
    iterateObjects.forEach( element => {
        let underlyingCtx = reportsCommSrvc.getUnderlyingObject( element );
 ...
 */
self.updateDisplayedReport = function( fileTicket, sublocationState, selected ) {
    reportstabpageservice.setReportsParameter( selected, fileTicket );
    let nwSublocationState = sublocationState.getValue();
    nwSublocationState.updateHTMLReport = !nwSublocationState.updateHTMLReport;
    sublocationState.update( nwSublocationState );
};
...
self.isReportTabExist = function() {
    return $( '[name=\'tc_xrt_Rb0Reports\']' ).length > 0 || $( '[data-locator=\'tab-tc_xrt_Rb0Reports\']' ).length > 0;
};
...
self.getReportsTabScope = function( reportsPage, data, ctx, commandContext ) {
    ...
    var reportsTab = null;
    if( ctx.panelContext && ctx.panelContext.showObjectContext && ctx.selected && ctx.panelContext.openedObject?.uid === ctx.selected.uid ) {
...
const setupExecutingReportOnCtx = function( dataProviders, subPanelContext, containsSublocationState ) {
    // Check if dataProviders is null or undefined before accessing its properties
    if( dataProviders?.objectsDataProvider?.viewModelCollection?.loadedVMObjects?.length ) {
        if( containsSublocationState ) {
            let subLocationState = subPanelContext.pageContext.sublocationState.getValue();
            subLocationState.selectedSourceObject = dataProviders.objectsDataProvider.viewModelCollection.loadedVMObjects[ 0 ];
            subPanelContext.pageContext.sublocationState.update( subLocationState );
        } else {
            appCtxService.updatePartialCtx( 'selectedSourceObject', dataProviders.getInContextReportDefinitions.selectedObjects[ 0 ] );
        }
    } else {
        if( containsSublocationState ) {
            let subLocationState = subPanelContext.pageContext.sublocationState.getValue();
            delete subLocationState.selectedSourceObject;
            subPanelContext.pageContext.sublocationState.update( subLocationState );
        } else {
            appCtxService.unRegisterCtx( 'selectedSourceObject' );
        }
    }

    // Safe check for dataProviders before accessing nested properties
    if ( dataProviders?.getInContextReportDefinitions?.selectedObjects?.[0]?.props?.rd_type?.dbValue === 1 &&
        dataProviders?.getInContextReportDefinitions?.selectedObjects?.[0]?.props?.rd_source?.dbValue === 'Active Workspace' ) {
        if( containsSublocationState ) {
            let subLocationState = subPanelContext.pageContext.sublocationState.getValue();
            subLocationState.selectedReportDefinition = dataProviders.getInContextReportDefinitions.selectedObjects[ 0 ];
            subPanelContext.pageContext.sublocationState.update( subLocationState );
        } else {
            appCtxService.updatePartialCtx( 'selectedReportDefinition', dataProviders.getInContextReportDefinitions.selectedObjects[ 0 ] );
        }
    } else {
        if( containsSublocationState ) {
            let subLocationState = subPanelContext.pageContext.sublocationState.getValue();
            delete subLocationState.selectedReportDefinition;
            subPanelContext.pageContext.sublocationState.update( subLocationState );
        } else {
...
let navigateToReportsTab = ( subLocation = {} ) => {
    const newSublocationCtx = { ...subLocation.getValue() };
    newSublocationCtx.secondaryActiveTabId = 'tc_xrt_Rb0Reports';
    subLocation.update && subLocation.update( newSublocationCtx );
};

let setupSplitViewForReports = ( commandContext, spliView, ctxObj )=>{
    commandContext.pageContext = {};
    //get ctx.occmgmtContext & ctx.occmgmtContext2
    let leftCtxKey = spliView.viewKeys[ 0 ];
    let rightCtxKey = spliView.viewKeys[ 1 ];
    let leftsideSelection = appCtxService.getCtx( leftCtxKey + '.treeDataProvider.selectionModel.selectionData.selected[0]' );
    if( !leftsideSelection ) {
        leftsideSelection = appCtxService.getCtx( leftCtxKey + '.topElement' );
    }
    let rightsideSelection = appCtxService.getCtx( rightCtxKey + '.treeDataProvider.selectionModel.selectionData.selected[0]' );
    if( !rightsideSelection ) {
        rightsideSelection = appCtxService.getCtx( rightCtxKey + '.topElement' );
    }
    let lCtxObj = reportsCommSrvc.getUnderlyingObject( leftsideSelection );
    let rCtxObj = reportsCommSrvc.getUnderlyingObject( rightsideSelection );
    if( lCtxObj.uid === ctxObj.uid ) {
        commandContext.pageContext.sublocationState = commandContext.leftSublocationState;
    } else if( rCtxObj.uid === ctxObj.uid ) {
        commandContext.pageContext.sublocationState = commandContext.rightSublocationState;
    }
    if( lCtxObj.uid === ctxObj.uid && rCtxObj.uid === ctxObj.uid && commandContext.rightSublocationState.isSublocationActive ) {
        commandContext.pageContext.sublocationState = commandContext.rightSublocationState;
    }
};
...
export let openFileInNewTab = function( selected, fileTicket, data, commandContext ) {
    ...
    if( ctxObj && ctxObj.uid ) {
        // set active pageContext.sublocationState in case of comparison mode
        let spliView = appCtxService.getCtx( 'splitView' );

        if( spliView && commandContext?.leftSublocationState && commandContext?.rightSublocationState ) {
            setupSplitViewForReports( commandContext, spliView, ctxObj );
        }

        // Ensure commandContext is not null before accessing its properties
        let containsSublocationState = commandContext?.pageContext?.sublocationState;
        setupExecutingReportOnCtx( data.dataProviders, commandContext, containsSublocationState );

        // Set selected object and its associated file ticket parameters
        reportstabpageservice.setReportsParameter( ctxObj || selected, fileTicket );
        ...
        if( ( reportsTab || pageName === data.i18n.reportsPage ) && containsSublocationState ) {
            if( reportsTab && reportsTab.selectedTab === true || pageName === data.i18n.reportsPage ) {
                self.updateDisplayedReport( fileTicket, commandContext?.pageContext?.sublocationState, ctxObj || selected );
            } else {
            ...
            if( data.dataProviders && data.dataProviders.getInContextReportDefinitions.selectedObjects[ 0 ].props.rd_type.dbValue === 1 && data.dataProviders.getInContextReportDefinitions.selectedObjects[ 0 ].props.rd_source.dbValue === 'Active Workspace' ) {
            ...
            if( data.isTabAvailable ) {
                eventBus.publish( 'reportsTab.reset' );
            }
...
/**
 * Populates the criteria names and values for an item report based on the selected object's properties.
 *
 * @param {Object} data - The data object containing report parameters.
 * @param {Array} criteriaNames - The array to which criteria names will be added.
 * @param {Array} criteriaValues - The array to which criteria values will be added.
 * @param {Object} selectedObject - The selected object containing properties to determine report criteria.
 */
function getItemReportCriteria( data, criteriaNames, criteriaValues, selectedObject ) {
    if( selectedObject.props.rd_parameters.dbValues !== null &&
        ...
        const localTextBundle = localeSvc.getLoadedText( 'ReportMessages' );
        if( data.packedbom.dbValue === localTextBundle.trueVal ) {
            criteriaValues.push( 'TRUE' );
        } else if( data.packedbom.dbValue === localTextBundle.falseVal ) {
...
export let evaluateCriteriaAndCallGenerateReport = function( data ) {
    ...
    if( data !== null ) {
        if( data.dataProviders === undefined && data.selectedReportDef.props.rd_type.dbValues[ 0 ] === '0' ) {
            ...
            //This check added for query clauses which contains IS_NULL and IS_NOT_NULL operators.
            if( data.rb0ReportQueryAttributes.props && Object.entries( data.rb0ReportQueryAttributes.props ).length === 0 ) {
                isCriteriaEntered = true;
            }
...
export let getReportOptionNames = function( data, selectedObject ) {
    ...
    if( data !== null && selectedObject !== null ) {
        var source_Name = selectedObject.props.rd_source.dbValues[ 0 ];
        if( source_Name === OFFICE_TEMPLATE ) {
            reportOptionNames.push( 'officeLive' );
            reportOptionValues.push( exports.convertDisplayLocaleValuetoString( data.doLiveIntegration.dbValue ) );
        }
        if( source_Name === 'Teamcenter' ) {
...
export let loadReportCriteria = function( context, data, runReportAsync ) {
    ...
    if( context.selectedReport.props.rd_parameters.dbValues[ 0 ] === 'BOM_REPORT' ) {
        const localTextBundle = localeSvc.getLoadedText( 'ReportMessages' );
        packbomProps = listBoxService.createListModelObjectsFromStrings( [ localTextBundle.trueVal, localTextBundle.falseVal ] );
    }
    var customRepConfig = {
        customRepProps:[],
        propConfigs: false
