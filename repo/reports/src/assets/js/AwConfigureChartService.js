...
const AwReportChartExistWhen = ExistWhen( AwReportChart );
var exports = {};
export const awConfigureChartRenderFunction = ( props ) => {
    ...
    if( props.chartConfig.ChartType === 'column' ) {
        chartEmptyIcon = 'graEmptyBarChartFaded';
    } else if( props.chartConfig.ChartType === 'pie' ) {
        chartEmptyIcon = 'graEmptyPieChartFaded';
    } else{
        chartEmptyIcon = 'graEmptyLineChartFaded';
    }
    return (
        <div className='h-12'>
            ...
            <div className='sw-row aw-reports-chartWrapper'>
                {
                    reportsState.reportParameters?.ReportDefProps?.allChartsList[id]?.ChartPropName !== '' ?
                        <AwReportChartExistWhen
                            existWhen={!reportsState.reloadChart}
                            className={chartView}
 ...
 */
let getChartByPropertiesList = function( searchContext ) {
...
export let getChartPropList = ( reportsState ) => {
    var chartPropList = getChartByPropertiesList( reportsState.searchIncontextInfo );
    var nwReportsState = reportsState.getValue();
...
export let changeTitle = ( title, chartProperty ) => {
    ...
    nwTitle.uiValues = [ chartProperty.uiValue ];
    nwTitle.dbValue = chartProperty.uiValue;
    nwTitle.dbValues = [ chartProperty.uiValue ];
    nwTitle.valueUpdated = true;
    return nwTitle;
};
/**
...
export const getThumbnailChartList = ( reportsState, { thumbnailChartList, thumbnail, editModeThumbnail, addToDashboard, i18n } ) => {
    ...
    if( !nwReportsState.selectedReport ) {
        ...
        _.forEach( validCharts, ( chart, index ) => {
            chart.ChartPropInternalName !== '' && thumbnailChartList.push( { propDisplayValue: i18n.chart + ' ' + ( index + 1 ) + ': ' + reportsCommonService.getLocalisedNameOfChartOn( chart.ChartPropName, chart.ChartPropInternalName, reportsState.searchInfo.categories ), propInternalValue: 'ReportChart' + ( index + 1 ) } );
        } );
        ...
        while( rd_params.indexOf( reportChart + ( i + 1 ) + '_0' ) > -1 ) {
            ...
            ReportChart.ChartPropInternalName = JSON.parse( rd_paramValues[paramIndex + 1] );
            validCharts.push( ReportChart );
            i++;
        }
        _.forEach( validCharts, ( chart, index ) => {
            thumbnailChartList.push( { propDisplayValue: i18n.chart + ' ' + ( index + 1 ) + ': ' + reportsCommonService.getLocalisedNameOfChartOn( chart.ChartPropName, chart.ChartPropInternalName, reportsState.searchInfo.categories ), propInternalValue: 'ReportChart' + ( index + 1 ) } );
        } );
        thumbnailChartList.push( { propDisplayValue: i18n.tablePanel, propInternalValue: 'ReportTable1' } );
        ...
        nwAddToDashboard.uiValue = nwAddToDashboard.dbValue;
        nwAddToDashboard.value = nwAddToDashboard.dbValue;
    }
