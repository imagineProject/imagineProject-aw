...
export let getReportDefinitionValList = function( response, searchData, preferenceName, i18n ) {
    ...
    i18n && _.forEach( reportDefinitions, ( value, index ) => {
        if( value.props.Fnd0Applicable_Assignment ) {
            let ownerIdx = value.props.Fnd0Applicable_Assignment.dbValues.findIndex( obj=> obj === value.props.owning_user.dbValues[0] );
            let owner = value.props.Fnd0Applicable_Assignment.uiValues[ownerIdx];
            ...
            let uiValue = '';
            let idx = 0; let  cnt = 0;
            while( idx < parseInt( value.props.Fnd0Applicable_Assignment.uiValues.length ) && cnt < 5 ) {
                if( value.props.Fnd0Applicable_Assignment.uiValues[ idx ] !== owner ) {
                    if( uiValue === '' ) {
                        uiValue = ' ' + value.props.Fnd0Applicable_Assignment.uiValues[ idx ];
                    } else {
                        uiValue = uiValue + ', ' + value.props.Fnd0Applicable_Assignment.uiValues[ idx ];
            ...
            property.uiValue = value.props.Fnd0Applicable_Assignment.uiValues.length > 5 ? String( uiValue ) + ' ...' : String( uiValue );
            property.uiValues = [ property.uiValue ];
            if( ownerIdx > -1 && value.props.Fnd0Applicable_Assignment.uiValues.length === 1 ) {
                property.dbValue = 'private';
                property.dbValues = [ 'private' ];
                property.uiValue = i18n.private;
                property.uiValues = [ i18n.private ];
            }
            reportDefinitions[ index ].props.Fnd0reports_SharedWith = property;
        }
    } );
    ...
    var finalList = owningUserList.concat( otherUserList );
    // filter list based on the searchstring
    reportDefinitions = getFilteredList( finalList, searchData );
    reportCommnSrvc.setupReportPersistCtx( preferenceName );
    if( searchData ) {
        let newSearchData = searchData.getValue();
        newSearchData.totalFound = reportDefinitions.length;
    ...
    return { filteredList:reportDefinitions, reportDefList:finalList };
};

export let setSharedWithList = ( propertyProp, property, i18n, owner ) => {
    let nwpropertyProp = { ...propertyProp };
    let uiValue = '';
    // Check for private or public
    if( property.uiValues.length === 0 ) {
        uiValue = i18n.public;
    } else if( property.dbValues.length === 1 && property.dbValues[0] === owner.dbValues[0] ) {
        uiValue = i18n.private;
    }else{
        let uiValues = property.uiValues;
        uiValues.splice( property.dbValues.findIndex( value => value === owner.dbValues[0] ), 1 );
        uiValues.sort( ( data1, data2 ) => { return data1.localeCompare( data2 ); } );
        uiValue = uiValues.join( ', ' );
    }
...
let getFilteredList = function( searchResults, searchData ) {
    ...
    '' ) {
        ...
        for( var i = 0; i < searchResults.length; i++ ) {
            if( searchResults[ i ].props.rd_name.uiValues[ 0 ].toLowerCase().replace( /\s/g, '' ).indexOf( searchData.criteria.searchString.toLowerCase().replace( /\s/g, '' ) ) >= 0 ) {
                filterResults.push( searchResults[i] );
            }
        }
...
export let updateDataProvider = function( searchData, searchResults, dataProvider ) {
    let vmos = getFilteredList( searchResults, searchData );
    if( vmos && searchData ) {
        const newSearchData = searchData.getValue();
        newSearchData.totalFound = vmos.length;
        searchData.update( newSearchData );
        dataProvider.viewModelCollection.update( vmos, vmos.length );
    }
};
