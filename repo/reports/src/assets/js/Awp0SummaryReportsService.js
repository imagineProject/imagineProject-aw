...
/**
 * Reveals the conditional panel based on the selected report definition object and data.
 *
 * @param {Object} selectedReportDefinitionObject - The selected report definition object.
 * @param {Object} data - The data object containing panel information.
 * @param {Object} runReportAsync - The object containing the asynchronous report execution flag.
 * @returns {Object} An object containing custom report properties, the updated runReportAsync object, and the selected report definition object.
 */
export let conditionalPanelReveal = function( selectedReportDefinitionObject, data, runReportAsync ) {
    if( !selectedReportDefinitionObject?.props ) { return; }
    ...
    if( selectedReportDefinitionObject && data._internal.panelId === 'Awp0ReportsSummaryExecution' ) {
        ...
        return {
            ...
            selectedReportDef: selectedReportDefinitionObject
        };
    }
};
...
export let getImanQueryObject = function( selectedReportDefinitionObject ) {
    ...
    if( selectedReportDefinitionObject !== null ) {
        ...
        tcVmoService.getViewModelProperties( objs, propNames ).then( function() {
            if( objs?.length && objs[ 0 ]?.props?.qry_src_tc_qry?.dbValues?.length ) {
...
export let getRealProperties = function( modelObject ) {
    ...
    _.forEach( modelObject.props, function( prop ) {
        ...
        //FIX for LCS-1058357, Given the property is a date, set the rendering hint to 'datetime'
        //Otherwise, set the rendering hint to 'checkboxoptionlov'
        //valueType 2 is date type
        if( prop.propertyDescriptor.valueType === 2 ) {
            prop.renderingHint = 'datetime';
        } else if( prop.lovApi ) {
            prop.renderingHint = 'checkboxoptionlov';
